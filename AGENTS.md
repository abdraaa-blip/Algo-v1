...
- Toujours croiser le design avec **`config/algo-system-rules.ts`**. Règles Cursor : `.cursor/rules/algo-qa-intelligent.mdc` (**alwaysApply**) · `.cursor/rules/algo-cicd-guardian.mdc` (**alwaysApply** — gardien pipeline GitHub / Vercel, interdits `|| true` / CI dupliquée).
