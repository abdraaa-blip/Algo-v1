...
class AlgoOrchestratorClass {
  ...
  async fetchSource(source: DataSource): Promise<void> {
    ...
    if (state.inProgress) {
      console.log(`[ALGO Orchestrator] ${source} fetch already in progress, skipping`)
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          console.warn(`[ALGO Orchestrator] ${source} endpoint not found (404) - skipping`)
          if (cached.isCached) {
            this.publishDataEvent(source, cached.data, 'cache')
          }
          return
   ...
   */
  private getEndpoint(source: DataSource, scope: string, language: string): string {
    const base = API_ENDPOINTS[source as keyof typeof API_ENDPOINTS] || `/api/${source}`
   ...
   */
  private publishDataEvent(source: DataSource, data: unknown, dataSource: 'live' | 'cache' | 'stale'): void {
    ...
    if (eventType && eventType !== 'data:scores:updated') {
      AlgoEventBus.publish(eventType as Exclude<typeof eventType, 'data:scores:updated'>, {
        data: Array.isArray(data) ? data : (data as { data?: unknown[] })?.data || [],
        source: dataSource,
