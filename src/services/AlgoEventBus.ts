...
type EventCallback<T extends AlgoEventType> = (
  payload: AlgoEventPayload[T],
) => void;
...
class AlgoEventBusClass {
  ...
  private wildcardSubscribers: Array<{
    id: string;
    callback: AnyEventCallback;
  }> = [];
  private lastEventState: Map<
    AlgoEventType,
    ...
    if (receiveLastState) {
      ...
      if (lastState) {
        try {
          ...
          console.error(
            `[ALGO EventBus] Error delivering last state for ${eventType}:`,
            error,
          );
        }
      }
   ...
   */
  publish<T extends AlgoEventType>(
    eventType: T,
    payload: AlgoEventPayload[T],
  ): void {
    ...
    for (const sub of subscribers) {
      try {
        ...
        console.error(
          `[ALGO EventBus] Error in subscriber for ${eventType}:`,
          error,
        );
      }
    }
   ...
   */
  getLastState<T extends AlgoEventType>(
    eventType: T,
  ): AlgoEventPayload[T] | null {
