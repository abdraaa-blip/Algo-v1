...
class CircuitBreakerClass {
  ...
  recordFailure(serviceName: string, error?: Error): void {
    ...
    stats.failures = stats.failures.filter(t => now - t < config.failureWindow)
    
    // Check if we should open the circuit
    if (stats.state === 'CLOSED' && stats.failures.length >= config.failureThreshold) {
      stats.state = 'OPEN'
      stats.openedAt = now
      console.warn(`[CircuitBreaker] ${serviceName} circuit OPENED after ${config.failureThreshold} failures`, error?.message)
    } else if (stats.state === 'HALF_OPEN') {
      // Failed during test, go back to OPEN
      stats.state = 'OPEN'
      stats.openedAt = now
      console.warn(`[CircuitBreaker] ${serviceName} failed during recovery test, circuit OPENED`)
    }
  }
...
CircuitBreaker.configure('youtube', { failureThreshold: 3, resetTimeout: 60000 })
CircuitBreaker.configure('tmdb', { failureThreshold: 3, resetTimeout: 60000 })
CircuitBreaker.configure('newsapi', { failureThreshold: 5, resetTimeout: 120000 })
CircuitBreaker.configure('reddit', { failureThreshold: 3, resetTimeout: 30000 })
CircuitBreaker.configure('lastfm', { failureThreshold: 3, resetTimeout: 60000 })
CircuitBreaker.configure('spotify', { failureThreshold: 3, resetTimeout: 60000 })
CircuitBreaker.configure('twitch', { failureThreshold: 3, resetTimeout: 30000 })
CircuitBreaker.configure('github', { failureThreshold: 5, resetTimeout: 120000 })
