...
class AlgoCacheClass {
  ...
  private async initIndexedDB(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      try {
        ...
        request.onerror = () => {
          console.warn('[ALGO Cache] IndexedDB not available, using memory only')
          resolve(false)
        }
   ...
   */
  async get<T>(source: CacheSource, scope: string): Promise<{
    data: T | null
    isStale: boolean
    isCached: boolean
    age: number
   ...
   */
  getDataFreshness(source: CacheSource, scope: string): 'fresh' | 'stale' | 'old' | 'unknown' {
    const key = this.getKey(source, scope)
  ...
  getMetrics() {
    ...
    for (const [, entry] of this.memoryCache) {
      entriesPerSource[entry.source] = (entriesPerSource[entry.source] || 0) + 1
    }
