...
function acquireBulkhead(serviceName: string): boolean {
  ...
  if (bulkhead.current >= bulkhead.max) {
    console.warn(`[Resilience] Bulkhead full for ${serviceName} (${bulkhead.current}/${bulkhead.max})`)
    return false
  }
...
export async function resilientFetch<T>(
  ...
  if (!CircuitBreaker.canRequest(serviceName)) {
    ...
    if (options.cacheKey) {
      const cached = await AlgoCache.get<T>(options.cacheKey.source, options.cacheKey.scope)
      if (cached.data) {
  ...
  try {
    ...
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
      try {
        ...
        // Cache the result
        if (options.cacheKey) {
          await AlgoCache.set(options.cacheKey.source, options.cacheKey.scope, result)
        }
        ...
        if (attempt < config.maxRetries) {
          ...
          console.log(`[Resilience] ${serviceName} attempt ${attempt + 1} failed, retrying in ${Math.round(delay)}ms`)
          await sleep(delay)
        }
      }
    }
    ...
    if (options.cacheKey) {
      const cached = await AlgoCache.get<T>(options.cacheKey.source, options.cacheKey.scope)
      if (cached.data) {
        console.log(`[Resilience] All retries failed for ${serviceName}, serving cache`)
        return cached.data
      }
    }
    ...
    // Use fallback
    if (options.fallback) {
      console.log(`[Resilience] All retries failed for ${serviceName}, using fallback`)
      return options.fallback()
    }
 ...
 */
export function getBestDataSource(feedType: 'news' | 'videos' | 'movies' | 'music'): string[] {
  ...
  return available.sort((a, b) => {
    ...
    const scoreA = healthA?.status === 'healthy' ? 0 : healthA?.status === 'degraded' ? 1 : 2
    const scoreB = healthB?.status === 'healthy' ? 0 : healthB?.status === 'degraded' ? 1 : 2
