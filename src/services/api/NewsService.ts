...
const newsCache: Map<string, { data: NewsArticle[]; expiresAt: Date }> = new Map()
...
function extractTags(title: string): string[] {
  const stopWords = new Set([
    ...
    'a', 'the', 'an', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by',
    'est', 'sont', 'dans', 'sur', 'pour', 'par', 'que', 'qui', 'quoi'
  ])
...
function guessCategory(title: string): string {
  ...
  if (/tech|ia|ai|digital|startup|innovation|robot|cyber|apple|google|microsoft/.test(titleLower)) return 'Tech'
  if (/sport|football|match|ligue|nba|tennis|rugby|jeux olympiques|psg|real|barca/.test(titleLower)) return 'Sport'
  if (/film|cinema|serie|netflix|disney|marvel|acteur|actrice/.test(titleLower)) return 'Entertainment'
  if (/politique|president|gouvernement|election|ministre|vote|assemblee/.test(titleLower)) return 'Politique'
  if (/economie|bourse|marche|entreprise|finance|euro|dollar|inflation/.test(titleLower)) return 'Economie'
  if (/musique|concert|artiste|album|spotify|rappeur|chanteuse/.test(titleLower)) return 'Musique'
  if (/science|recherche|decouverte|etude|espace|nasa|climat/.test(titleLower)) return 'Science'
  if (/sante|covid|hopital|medecin|medicament|vaccin/.test(titleLower)) return 'Sante'
...
async function fetchNewsFromAPI(country: string = 'fr', isBreaking: boolean = false): Promise<NewsArticle[]> {
 ...
 */
export async function getNews(country: string = 'fr'): Promise<NewsServiceResponse> {
 ...
 */
export async function searchNews(query: string, country: string = 'fr'): Promise<NewsServiceResponse> {
  const response = await getNews(country)
 ...
 */
export async function getNewsByCategory(category: string, country: string = 'fr'): Promise<NewsServiceResponse> {
  const response = await getNews(country)
