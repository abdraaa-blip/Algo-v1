...
function determineMomentum(
  popularity: number,
): "exploding" | "rising" | "stable" | "fading" {
...
function generateTrendingReason(film: Film): string {
  ...
  if (film.momentum === "exploding")
    reasons.push("Buzz massif sur les reseaux");
...
function extractBuzzKeywords(title: string, genres: string[]): string[] {
  ...
  if (
    ...
    title.toLowerCase().includes("avenger")
  ) {
...
function generateContentIdeas(film: Film): string[] {
  ...
  if (film.genres.some((g) => g.toLowerCase().includes("action"))) {
    ideas.push(
      `Top 5 des scenes d'action les plus memorables de "${film.title}"`,
    );
  ...
  if (
    film.genres.some(
      (g) =>
        g.toLowerCase().includes("comedie") ||
  ...
  if (
    film.genres.some(
      (g) =>
        g.toLowerCase().includes("horreur") ||
  ...
  if (film.rating >= 8) {
    ...
    ideas.push(
      `"${film.title}" est-il le meilleur ${film.type === "movie" ? "film" : "série"} de l'année ?`,
    );
  ...
  // Universal ideas
  ideas.push(
    `Mon avis sur "${film.title}" en ${film.type === "movie" ? "2" : "5"} minutes`,
  );
...
export async function searchFilms(query: string): Promise<FilmServiceResponse> {
  ...
  const filtered = trending.data.filter(
    (film) =>
 ...
 */
export async function getFilmsByType(
  type: "movie" | "series",
): Promise<FilmServiceResponse> {
  const trending = await getTrendingFilms();
