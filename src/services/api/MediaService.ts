...
function calculateVideoViralScore(
  views: number,
  likes: number,
  comments: number,
  hoursAgo: number,
...
function calculateMusicViralScore(
  playCount: number,
  listeners: number,
): number {
  let score = 50;
...
function guessVideoCategory(title: string, channelTitle: string): string {
  ...
  if (
    /gaming|fortnite|minecraft|gta|valorant|league|cod|playstation|xbox/.test(
      text,
    )
  )
    return "Gaming";
  if (/music|official|mv|clip|lyrics|song|feat|ft\./.test(text))
    return "Musique";
...
async function fetchVideosFromAPI(country: string = "FR"): Promise<Video[]> {
  ...
  return data.data.map((video: Record<string, unknown>) => {
    const publishedAt =
      (video.publishedAt as string) || new Date().toISOString();
    ...
    return {
      ...
      thumbnail:
        (video.thumbnail as string) || (video.thumbnailUrl as string) || "",
      publishedAt,
      viewCount: views,
      likeCount: likes,
...
async function fetchMusicFromAPI(
  ...
  const tracks: Track[] = (data.data?.tracks || []).map(
    (track: Record<string, unknown>, index: number) => ({
  ...
  const artists: Artist[] = (data.data?.artists || []).map(
    (artist: Record<string, unknown>, index: number) => ({
...
export async function getTrendingVideos(
  country: string = "FR",
): Promise<MediaServiceResponse<Video>> {
...
export async function getTrendingTracks(
  country: string = "france",
): Promise<MediaServiceResponse<Track>> {
...
export async function getTrendingArtists(
  country: string = "france",
): Promise<MediaServiceResponse<Artist>> {
...
export function generateVideoContentIdeas(video: Video): string[] {
  ...
  if (video.viralScore > 85) {
    ideas.push(
      `La formule secrete de "${video.title}" - ${video.viewCount.toLocaleString()} vues!`,
    );
  }
