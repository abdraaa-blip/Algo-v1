...
const trendCache: Map<
  string,
  {
    data: TrendItem[];
    sources: TrendServiceResponse["sources"];
    expiresAt: Date;
  }
> = new Map();
...
function guessCategory(name: string, source: string): string {
  ...
  if (/tech|ai|ia|apple|google|microsoft|startup/.test(nameLower))
    return "Tech";
  if (/film|movie|serie|netflix|disney|marvel|trailer/.test(nameLower))
    return "Cinema";
  if (/music|song|album|concert|artiste|rapper/.test(nameLower))
    return "Musique";
  if (/sport|football|nba|match|ligue|psg/.test(nameLower)) return "Sport";
  if (/politique|president|election|gouvernement/.test(nameLower))
    return "Politique";
...
function transformYouTubeToTrend(
  videos: Array<{
    title: string;
    viewCount: number;
    url: string;
...
function transformRedditToTrend(
  posts: Array<{
    title: string;
    score: number;
    url: string;
...
function transformGitHubToTrend(
  repos: Array<{
    name: string;
    stars: number;
    url: string;
  ...
  return repos
    ...
    .map(
      (t): TrendItem => ({
        ...t,
        explanation: t.explanation || generateExplanation(t),
      }),
    );
}
  ...
  try {
    ...
    if (response.data.youtube) {
      ...
      sources.push({
        name: "YouTube",
        status: "success",
        count: response.data.youtube.length,
      });
    }
    ...
    if (response.data.reddit) {
      ...
      sources.push({
        name: "Reddit",
        status: "success",
        count: response.data.reddit.length,
      });
    }
    ...
    if (response.data.github) {
      ...
      sources.push({
        name: "GitHub",
        status: "success",
        count: response.data.github.length,
      });
    }
