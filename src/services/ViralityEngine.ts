...
export function calculateViralScore(metrics: {
  ...
  const engagementScore = Math.min(
    25,
    Math.log10(Math.max(1, likes + shares + comments)) * 5,
  );
  ...
  const growthBonus = Math.min(
    15,
    growthRate > 0 ? Math.sqrt(growthRate) * 2 : 0,
  );
  ...
  let value = Math.round(
    viewScore + engagementScore + mentionScore + growthBonus + freshnessBonus,
  );
  ...
  const dataPoints = [views, likes, shares, comments, mentions].filter(
    (v) => v > 0,
  ).length;
  ...
  if (previousValues.length >= 2) {
    const prevVelocity =
      ((previousValues[previousValues.length - 1] -
        previousValues[previousValues.length - 2]) /
        intervalMinutes) *
      60;
...
export function generateRecommendations(
  ...
  if (momentum.type === "accelerating") {
    recommendations.push(
      "Croissance exponentielle détectée — surfer sur la vague",
    );
  ...
  if (freshness.peakWindow) {
    recommendations.push(
      `Fenêtre optimale : ${freshness.remainingMinutes || 60} minutes restantes`,
    );
...
export function analyzeVirality(
  ...
  const publishedAt =
    typeof metrics.publishedAt === "string"
  ...
  // Predict peak
  const peakScore = Math.min(
    100,
    score.value + (momentum.velocity > 0 ? momentum.velocity * 2 : 0),
  );
  const timeToPeak =
    momentum.velocity > 0
      ? Math.floor((60 / Math.max(1, momentum.velocity)) * (100 - score.value))
      : 0;
...
export const VIRALITY_LABELS_FR: Record<
  ViralityLevel,
