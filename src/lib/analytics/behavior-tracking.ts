...
function saveEvents() {
  try {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify(eventQueue.slice(-MAX_STORED_EVENTS)),
    );
  } catch (error) {
...
function updatePreferences(interaction: ContentInteraction) {
  try {
    ...
    const prefs: UserPreferences = stored
      ? JSON.parse(stored)
      : {
          topCategories: {},
          topPlatforms: {},
    ...
    if (interaction.duration) {
      prefs.avgDwellTime =
        (prefs.avgDwellTime * (prefs.contentCount - 1) + interaction.duration) /
        prefs.contentCount;
    }
    ...
    prefs.topCategories[category] =
      (prefs.topCategories[category] || 0) * 0.95 + actionScore;
