...
export const ALGO_UI_EMPTY = {
  ...
  message:
    "Le radar ne capte rien pour le moment. Élargis ta recherche ou reviens plus tard.",
} as const;

export const ALGO_UI_OFFLINE = {
  title: "Connexion instable",
  message:
    "Vérifie le réseau puis réessaie : les données peuvent être en retard.",
...
export const ALGO_BILLING = {
  ...
  checkoutUnavailable:
    "Paiement : en attente de configuration côté serveur (clés Stripe + prix).",
  checkoutRequiresLogin:
    "Connecte-toi (compte ALGO) pour ouvrir le paiement Stripe.",
  checkoutError:
    "Impossible d'ouvrir le paiement pour l'instant. Réessaie plus tard.",
  planLabelFree: "Plan actuel : gratuit",
  planLabelPro: "Plan actuel : Pro (Stripe)",
  proCheckoutDone:
    "Ton abonnement est actif côté profil. Renouvellement et factures : portail Stripe (lien e-mail ou dashboard Stripe).",
  portalCta: "Gérer l'abonnement (Stripe)",
  portalError:
    "Impossible d'ouvrir le portail pour l'instant. Réessaie plus tard.",
  successNote:
    "Paiement initié ou réussi côté Stripe : le passage en Pro dans l'app suivra la connexion webhook (à brancher).",
