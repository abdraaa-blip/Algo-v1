...
function timingLabelFor(timing: TimingStatus) {
  if (timing === 'now') return fillLocaleStrings({ fr: 'Publier maintenant', en: 'Post now' })
  if (timing === 'too_late') return fillLocaleStrings({ fr: 'Fenêtre passée', en: 'Too late' })
  return fillLocaleStrings({ fr: 'Trop tôt', en: 'Too early' })
}

export function adaptCreatorPageInsightToFull(input: CreatorPageInsightInput): Insight {
  const platforms = (input.bestPlatform?.length ? input.bestPlatform : ['TikTok']).map(stringToPlatform)
  return {
