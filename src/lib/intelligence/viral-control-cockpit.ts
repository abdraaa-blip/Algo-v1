...
import {
  ...
  type RadarDeltaHint,
} from "@/lib/intelligence/radar-series-hint";
...
export function deriveVelocityAcceleration(points: RadarHistoryPointDTO[]): {
  ...
  if (sorted.length < 2)
    return { velocityPerHour: null, accelerationPerHour2: null };
  ...
  if (
    !Number.isFinite(dtH0) ||
    dtH0 <= 0 ||
    !Number.isFinite(dtH1) ||
    dtH1 <= 0
  ) {
    return { velocityPerHour: v1, accelerationPerHour2: null };
  }
  ...
  return {
    velocityPerHour: v1,
    accelerationPerHour2: Number.isFinite(acc) ? acc : null,
  };
}

function engagementProxyFromSnapshot(
  snapshot: GlobalIntelligenceSnapshot,
): ViralControlCockpitPayload["engagementProxy"] {
  ...
  const commentsIndex = clamp(
    Math.round(base * 0.78 + e * 18 - f * 12),
    0,
    100,
  );
...
function buildTrendVsGlobal(
  ...
  const avgCat =
    snapshot.categories.length > 0
      ? snapshot.categories.reduce((s, c) => s + c.score, 0) /
  ...
  if (Math.abs(alignDelta) < 4) {
    return {
      ...
      detailFr:
        "Ta catégorie dominante suit la moyenne des scores thématiques du radar.",
    };
  }
  if (alignDelta > 0) {
    return {
...
export function buildViralControlCockpitPayload(input: {
  ...
  const top = [...snapshot.categories].sort((a, b) => b.score - a.score)[0];
  const { velocityPerHour, accelerationPerHour2 } =
    deriveVelocityAcceleration(radarPoints);
  ...
  if (actions.length === 0) {
    actions.push({
      ...
      detailFr:
        "Sans métriques fines par post, garde un test court avant de scaler la prod.",
      priority: "soon",
    });
  }
