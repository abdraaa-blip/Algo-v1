...
export function collectOpsStatus() {
  ...
  const openCircuits = Object.entries(circuits).filter(([, stat]) => stat.state === 'OPEN')
  ...
  const recommendedActions = [
    ...(alerts.some((a) => a.id === 'learning-drift')
      ...
      : []),
    ...(alerts.some((a) => a.id === 'resilience-open-circuit') ? ['slow_external_polling'] : []),
    ...(alerts.some((a) => a.id === 'autonomy-killswitch') ? ['manual_ops_review_required'] : []),
  ]
