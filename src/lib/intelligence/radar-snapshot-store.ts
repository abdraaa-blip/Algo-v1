...
export function recordRadarSnapshotIfDue(
  ...
  const capped =
    pruned.length > MAX_POINTS_PER_REGION
      ? pruned.slice(-MAX_POINTS_PER_REGION)
      : pruned;
...
export function getRadarHistoryMemory(
  region: string,
  days: number,
...
async function loadRadarPointsFromDedicatedTable(
  ...
  for (const row of data as Array<{
    ...
    if (!Number.isFinite(row.virality_score) || !Number.isFinite(confidence))
      continue;
    out.push({
      ...
      confidence,
      anomalyCount:
        typeof row.anomaly_count === "number" &&
        Number.isFinite(row.anomaly_count)
...
async function loadRadarSnapshotsFromAnalyticsLegacy(
  ...
  for (const row of data) {
    ...
    const at = String(
      props.capturedAt || (row as { created_at?: string }).created_at || "",
    );
    if (!at) continue;
    const viralityScore =
      typeof props.viralityScore === "number"
        ? props.viralityScore
        : Number(props.viralityScore);
    const confidence =
      typeof props.confidence === "number"
        ? props.confidence
        : Number(props.confidence);
    const anomalyCount =
      typeof props.anomalyCount === "number"
        ? props.anomalyCount
        : Number(props.anomalyCount);
    if (!Number.isFinite(viralityScore) || !Number.isFinite(confidence))
      continue;
    out.push({
      at: new Date(at).toISOString(),
      viralityScore: Math.round(viralityScore),
      confidence,
      anomalyCount: Number.isFinite(anomalyCount)
        ? Math.max(0, Math.round(anomalyCount))
        : 0,
    });
  }
  return dedupeRadarPointsByAt(out);
...
async function loadRadarSnapshotsFromSupabase(
  region: string,
  limit: number,
): Promise<RadarHistoryPointDTO[]> {
  const fromTable = await loadRadarPointsFromDedicatedTable(region, limit);
  const fromLegacy = await loadRadarSnapshotsFromAnalyticsLegacy(
    region,
    Math.min(limit, 200),
  );
