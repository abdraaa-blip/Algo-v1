...
export type GlobalIntelligenceSnapshot = Awaited<
  ReturnType<typeof buildGlobalIntelligence>
>;

/**
 * Même logique que GET /api/intelligence/predictive (hors cache HTTP) · utilisée par le cron radar.
 */
export async function buildPredictiveIntelligenceBundle(input: {
  region: string;
  locale: string;
}): Promise<{
  ...
  const topCategory = [...snapshot.categories].sort(
    (a, b) => b.score - a.score,
  )[0];
  ...
  const anomalyPenalty = snapshot.anomalies.some(
    (a) => a.type === "friction" && a.severity === "high",
  )
    ? 8
    : 0;
  const predictedViralityScore = clamp(
    Math.round(baseVirality + engagementAdj - frictionPenalty - anomalyPenalty),
  );
  const confidence =
    predictedViralityScore >= 70
      ? 0.78
      : predictedViralityScore >= 50
        ? 0.64
        : 0.52;
