...
async function fetchFinanceSignals(): Promise<{
  count: number;
  source: string;
  items: string[];
}> {
  ...
  const coins = Array.isArray(json?.coins)
    ? (json?.coins as Array<{ item?: { name?: string } }>)
    : [];
  const items = coins
    .map((c) => c.item?.name)
...
async function fetchScienceSignals(): Promise<{
  count: number;
  source: string;
  items: string[];
}> {
  ...
  const json = await safeFetchJson(url);
  const msg = (json?.message || null) as {
...
async function fetchEconomicSignals(
  region: string,
): Promise<{ count: number; source: string; items: string[] }> {
  ...
  const arr = Array.isArray(json)
    ...
    : [];
  const rows = Array.isArray(arr[1])
    ? (arr[1] as Array<Record<string, unknown>>)
    : [];
  const items = rows
    .slice(0, 6)
    .map((r) => {
...
async function fetchExternalSocialSignals(): Promise<{
  count: number;
  source: string;
  items: string[];
}> {
  const reddit = await safeFetchJson(
...
async function fetchCommerceSignals(): Promise<{
  count: number;
  source: string;
  items: string[];
}> {
  ...
  const coins = Array.isArray(market?.coins)
    ? (market.coins as Array<{ item?: { name?: string } }>)
    : [];
  const items = coins
    .map((c) => c.item?.name)
    .filter((v): v is string => Boolean(v))
...
function buildCategoryScores(inputs: {
  ...
  const categories = Object.entries(scoreKeywords).map(([name, keys]) => {
    ...
    const adj = Math.round(
      base + inputs.engagementRate * 20 - inputs.frictionRate * 30,
    );
    const score = Math.max(0, Math.min(100, adj));
    const momentum: "up" | "stable" | "down" =
      score >= 65 ? "up" : score >= 45 ? "stable" : "down";
    return {
      name,
      score,
      momentum,
...
function detectAnomalies(params: {
  ...
  if (params.frictionRate > 0.2) {
    anomalies.push({
      ...
      severity: "high",
      message:
        "Friction utilisateur élevée : prioriser la stabilité UX / perf.",
    });
  }
...
function inferProductOpportunities(
  titles: string[],
): GlobalIntelligenceSnapshot["opportunities"] {
  const productHints = [
    "headphone",
    "camera",
    "ring light",
  ...
  const mapped = found.map((k) => ({
    ...
    confidence: 0.62,
    rationale:
      "Fréquence croissante sur titres sociaux / vidéo + potentiel UGC.",
  }));
  return mapped;
}
...
export async function buildGlobalIntelligence(params?: {
  ...
  const newsItems = Array.isArray(newsJson?.data)
    ? (newsJson?.data as Json[])
    : [];
  const trendItems = Array.isArray(trendsJson?.data)
    ? (trendsJson?.data as Json[])
    : [];
  const videoItems = Array.isArray(videosJson?.data)
    ? (videosJson?.data as Json[])
    : [];
  const engagementRate = toNum(
    (analyticsJson?.adaptiveSignals as Json | undefined)?.engagementRate,
    0,
  );
  const frictionRate = toNum(
    (analyticsJson?.adaptiveSignals as Json | undefined)?.frictionRate,
    0,
  );
  ...
  const newsTitles = newsItems
    ...
    .filter(Boolean);
  const socialTitles = trendItems
    .map((t) => String(t.title || t.name || ""))
    .filter(Boolean);
  const videoTitles = videoItems
    .map((v) => String(v.title || ""))
    .filter(Boolean);
  ...
  const opportunities: GlobalIntelligenceSnapshot["opportunities"] = [
    {
      ...
      rationale:
        "Catégories avec momentum positif détectées dans les flux agrégés.",
    },
    {
      type: "timing",
      ...
      confidence: 0.66,
      rationale:
        "Combinaison engagement/friction et volume social pour ajuster le timing de publication.",
    },
    ...inferProductOpportunities([...socialTitles, ...videoTitles]),
    {
      ...
      rationale:
        "Recent research bursts can seed high-trust narratives and explainers.",
    },
    {
      type: "timing",
      ...
      rationale:
        "Macro volatility windows can shift user attention and conversion timing.",
    },
    {
      type: "product",
      ...
      rationale:
        "Cross-market rising entities indicate monetizable attention clusters.",
    },
  ];
  ...
  return {
    ...
    sources: {
      news: {
        count: newsTitles.length,
        source: String(newsJson?.source || "mixed"),
      },
      social: {
        count: socialTitles.length,
        source: String(trendsJson?.source || "mixed"),
      },
      videos: {
        count: videoTitles.length,
        source: String(videosJson?.source || "mixed"),
      },
      ...
      socialExternal: {
        count: socialExternal.count,
        source: socialExternal.source,
      },
