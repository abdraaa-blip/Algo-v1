...
export async function fetchGitHubTrending() {
  try {
    const r = await fetch(
      "https://api.github.com/search/repositories?q=created:>2024-01-01&sort=stars&order=desc&per_page=15",
      {
        signal: AbortSignal.timeout(5000),
...
export async function fetchAllSources() {
  const [yt, rd, hn, gh] = await Promise.allSettled([
