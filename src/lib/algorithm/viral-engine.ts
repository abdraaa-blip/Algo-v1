...
export function calculateVelocity(
  ...
  const timeDelta =
    (lastPoint.timestamp - firstPoint.timestamp) / (60 * 60 * 1000); // en heures
  ...
  const v1 =
    (midPoint.value - firstPoint.value) /
    ((midPoint.timestamp - firstPoint.timestamp) / (60 * 60 * 1000));
  const v2 =
    (lastPoint.value - midPoint.value) /
    ((lastPoint.timestamp - midPoint.timestamp) / (60 * 60 * 1000));
...
export function computeViralScore(signals: ViralSignal[]): ViralScore {
  ...
  const avgVelocity =
    signals.reduce((acc, s) => acc + s.velocity, 0) / signals.length;
  const velocityScore = Math.min(
    100,
    (avgVelocity / 1000) * 100 + (maxVelocity > 5000 ? 20 : 0),
  );

  // 2. ACCELERATION SCORE · Accélération = explosion imminente
  const avgAcceleration =
    signals.reduce((acc, s) => acc + s.acceleration, 0) / signals.length;
  const accelerationBonus =
    avgAcceleration > 0 ? Math.min(30, avgAcceleration / 100) : 0;

  // 3. CROSS-PLATFORM SCORE · Multi-plateforme = viral confirmé
  const platforms = new Set(signals.map((s) => s.source));
  const crossPlatformScore = Math.min(
    100,
    (platforms.size / 7) * 100 + (platforms.size >= 4 ? 20 : 0),
  );

  // 4. SENTIMENT SCORE · Émotions fortes = engagement
  const avgSentiment =
    signals.reduce((acc, s) => acc + Math.abs(s.sentiment), 0) / signals.length;
  const avgIntensity =
    signals.reduce((acc, s) => acc + s.emotionalIntensity, 0) / signals.length;
  ...
  const now = Date.now();
  const avgAge =
    signals.reduce((acc, s) => acc + (now - s.timestamp), 0) / signals.length;
  ...
  const predictedPeak =
    avgAcceleration > 0
      ...
      : avgAcceleration < -50
        ? -1
        : 48; // Accélération négative = pic passé
  ...
  const confidence = Math.min(
    ...
    (signals.length / 10) * 0.5 + // Plus de signaux = plus de confiance
      ...
      (1 -
        (Math.max(...signals.map((s) => s.velocity)) -
          Math.min(...signals.map((s) => s.velocity))) /
          10000) *
        0.2,
  );
...
export function predictViralPotential(
  ...
  if (signals.some((s) => s.acceleration > 100))
  ...
  const estimatedPeakTime = new Date(
    now.getTime() + hoursUntilPeak * 60 * 60 * 1000,
  );

  // Volume estimé au pic
  const currentVolume = signals.reduce((acc, s) => acc + s.volume, 0);
  const growthFactor = 1 + score.velocity / 100;
  const estimatedPeakVolume = Math.round(
    currentVolume * Math.pow(growthFactor, hoursUntilPeak),
  );

  // Fenêtre optimale de publication
  const optimalStart = new Date(
    now.getTime() + Math.max(0, hoursUntilPeak - 6) * 60 * 60 * 1000,
  );
  const optimalEnd = new Date(
    now.getTime() + Math.max(1, hoursUntilPeak - 1) * 60 * 60 * 1000,
  );
  ...
  if (score.tier === "S")
    reasoning.push("Signal tier S - Potentiel viral exceptionnel");
  if (score.velocity > 70)
    reasoning.push("Vélocité très élevée - Croissance explosive");
  if (score.crossPlatform > 60)
    reasoning.push("Présence multi-plateforme - Propagation confirmée");
  if (signals.some((s) => s.acceleration > 50))
    reasoning.push("Accélération positive - Momentum en hausse");
  if (score.sentiment > 70) reasoning.push("Engagement émotionnel fort");
  if (probability < 0.3)
    reasoning.push("Signaux faibles - Surveiller avant d'agir");
...
export class SignalAggregator {
  ...
  private subscribers: Set<(signals: Map<string, ViralSignal[]>) => void> =
    new Set();
  ...
  getTopSignals(
    limit: number = 20,
  ): { keyword: string; score: ViralScore; signals: ViralSignal[] }[] {
    const results: {
      keyword: string;
      score: ViralScore;
      signals: ViralSignal[];
