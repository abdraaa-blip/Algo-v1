...
export type InteractionType = 'view' | 'click' | 'save' | 'share' | 'like' | 'comment' | 'follow' | 'dismiss'
...
export function trackInteraction(interaction: Interaction): void {
  ...
  if (interaction.category) {
    prefs.categories[interaction.category] = (prefs.categories[interaction.category] || 0) + weight
  }
  
  // Update platform preference
  if (interaction.platform) {
    prefs.platforms[interaction.platform] = (prefs.platforms[interaction.platform] || 0) + weight
  }
  
  // Update content type preference
  if (interaction.contentType) {
    prefs.contentTypes[interaction.contentType] = (prefs.contentTypes[interaction.contentType] || 0) + weight
  }
  
  // Track keyword
  if (interaction.trendKeyword && !prefs.keywords.includes(interaction.trendKeyword)) {
  ...
  if (interaction.trendKeyword) {
    if (interaction.type === 'view' && !prefs.viewedTrends.includes(interaction.trendKeyword)) {
      prefs.viewedTrends.unshift(interaction.trendKeyword)
      prefs.viewedTrends = prefs.viewedTrends.slice(0, 500)
    }
    if (interaction.type === 'save' && !prefs.savedTrends.includes(interaction.trendKeyword)) {
      prefs.savedTrends.unshift(interaction.trendKeyword)
    }
    if (interaction.type === 'share' && !prefs.sharedTrends.includes(interaction.trendKeyword)) {
...
export function calculateRelevanceScore(
  ...
  trendWords.forEach(word => {
    if (userWords.has(word) || prefs.keywords.some(k => k.toLowerCase().includes(word))) {
  ...
  const trendType = trend.platforms.includes('youtube') || trend.platforms.includes('tiktok') 
    ? 'video' 
  ...
  const total = categoryMatch + platformMatch + keywordSimilarity + timingBonus + noveltyBonus
...
export function getPersonalizedRecommendations(
  ...
  trends.forEach(trend => {
    ...
    // Platform match
    const matchingPlatforms = trend.platforms.filter(p => topPlatforms.includes(p))
    ...
    const matchingKeywords = topKeywords.filter(k => 
      trendWords.some(w => k.toLowerCase().includes(w) || w.includes(k.toLowerCase()))
    )
