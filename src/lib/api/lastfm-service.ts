...
import {
  getDemoLastFmArtists,
  getDemoLastFmTracks,
} from "@/lib/data/offline-media-demos";
...
export async function fetchTopTracks(
  ...
  try {
    ...
    const tracks: RealTrack[] = tracksData.map(
      (
        track: {
          name: string;
        ...
        return {
          ...
          artist: artistName,
          artistUrl:
            typeof track.artist === "object" ? track.artist?.url || "" : "",
          url: track.url,
...
export async function fetchTopArtists(
  ...
  try {
    ...
    const artists: RealArtist[] = artistsData.map(
      (
        artist: {
          name: string;
          url: string;
