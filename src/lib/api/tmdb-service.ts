...
function getImageUrl(path: string | null, size: 'w200' | 'w300' | 'w500' | 'w780' | 'original' = 'w500'): string {
  if (!path) return 'https://ui-avatars.com/api/?name=No+Image&background=1a1a2e&color=fff&size=500'
  return `${TMDB_IMAGE_BASE}/${size}${path}`
}

function mapGenres(ids: number[] | undefined | null, isTV: boolean = false): string[] {
  if (!ids || !Array.isArray(ids)) return []
  const genreMap = isTV ? TV_GENRES : MOVIE_GENRES
  return ids.map(id => genreMap[id] || 'Unknown').filter(g => g !== 'Unknown')
}
 ...
 */
export async function fetchTrendingMovies(timeWindow: 'day' | 'week' = 'week', region: string = 'FR'): Promise<CachedData<RealMovie[]>> {
 ...
 */
export async function fetchTrendingTV(timeWindow: 'day' | 'week' = 'week', region: string = 'FR'): Promise<CachedData<RealTVShow[]>> {
 ...
 */
export async function fetchTrendingPeople(timeWindow: 'day' | 'week' = 'week'): Promise<CachedData<RealCelebrity[]>> {
 ...
 */
export async function fetchNowPlaying(region: string = 'FR'): Promise<CachedData<RealMovie[]>> {
 ...
 */
export async function fetchUpcoming(region: string = 'FR'): Promise<CachedData<RealMovie[]>> {
...
const movieDetailsCache: Map<string, { data: MovieDetails; expiresAt: Date }> = new Map()
const tvDetailsCache: Map<string, { data: TVDetails; expiresAt: Date }> = new Map()
...
export async function fetchMovieDetails(tmdbId: number): Promise<MovieDetails | null> {
  ...
  try {
    ...
    const [detailsRes, creditsRes, videosFrRes, videosEnRes] = await Promise.all([
      fetch(`${TMDB_BASE_URL}/movie/${tmdbId}?api_key=${TMDB_API_KEY}&language=fr-FR`, {
        signal: AbortSignal.timeout(8000)
      }),
      fetch(`${TMDB_BASE_URL}/movie/${tmdbId}/credits?api_key=${TMDB_API_KEY}&language=fr-FR`, {
        signal: AbortSignal.timeout(8000)
      }),
      fetch(`${TMDB_BASE_URL}/movie/${tmdbId}/videos?api_key=${TMDB_API_KEY}&language=fr-FR`, {
        signal: AbortSignal.timeout(8000)
      }),
      fetch(`${TMDB_BASE_URL}/movie/${tmdbId}/videos?api_key=${TMDB_API_KEY}&language=en-US`, {
        signal: AbortSignal.timeout(8000)
      })
    ])
    ...
    const videosFr = videosFrRes.ok ? await videosFrRes.json() : { results: [] }
    const videosEn = videosEnRes.ok ? await videosEnRes.json() : { results: [] }
    
    // Combine French and English videos, prefer French
    const allVideos = [...(videosFr.results || []), ...(videosEn.results || [])]
    ...
    interface VideoResult { type: string; site: string; key: string; official?: boolean }
    const trailer = allVideos.find((v: VideoResult) => v.type === 'Trailer' && v.site === 'YouTube') 
      || allVideos.find((v: VideoResult) => v.type === 'Teaser' && v.site === 'YouTube')
    ...
    const director = credits.crew?.find((c: CrewMember) => c.job === 'Director')?.name
 ...
 */
export async function fetchTVDetails(tmdbId: number): Promise<TVDetails | null> {
  ...
  try {
    ...
    const [detailsRes, creditsRes, videosFrRes, videosEnRes] = await Promise.all([
      fetch(`${TMDB_BASE_URL}/tv/${tmdbId}?api_key=${TMDB_API_KEY}&language=fr-FR`, {
        signal: AbortSignal.timeout(8000)
      }),
      fetch(`${TMDB_BASE_URL}/tv/${tmdbId}/credits?api_key=${TMDB_API_KEY}&language=fr-FR`, {
        signal: AbortSignal.timeout(8000)
      }),
      fetch(`${TMDB_BASE_URL}/tv/${tmdbId}/videos?api_key=${TMDB_API_KEY}&language=fr-FR`, {
        signal: AbortSignal.timeout(8000)
      }),
      fetch(`${TMDB_BASE_URL}/tv/${tmdbId}/videos?api_key=${TMDB_API_KEY}&language=en-US`, {
        signal: AbortSignal.timeout(8000)
      })
    ])
    ...
    interface VideoResult { type: string; site: string; key: string }
    const trailer = allVideos.find((v: VideoResult) => v.type === 'Trailer' && v.site === 'YouTube') 
      || allVideos.find((v: VideoResult) => v.type === 'Teaser' && v.site === 'YouTube')
      || allVideos.find((v: VideoResult) => v.site === 'YouTube')
    
    console.log('[TMDB] TV Videos found:', allVideos.length, 'Trailer:', trailer?.key || 'None')
    
    interface CreatedBy { name: string }
    const creator = details.created_by?.[0]?.name || details.created_by?.map((c: CreatedBy) => c.name).join(', ')
...
export async function fetchAllTrending(): Promise<{
  ...
  const sources = new Set([moviesResult.source, tvResult.source, peopleResult.source])
