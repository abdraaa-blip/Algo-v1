...
function setRateLimited(api: 'newsApi' | 'youtubeApi'): void {
  rateLimitState[api] = { limited: true, until: Date.now() + RATE_LIMIT_BACKOFF_MS }
...
export async function fetchGoogleTrends(country: string): Promise<CachedData<RealTrend>> {
  ...
  try {
    const newsConfig = GOOGLE_NEWS_COUNTRY_CODES[country.toLowerCase()] || GOOGLE_NEWS_COUNTRY_CODES.fr
...
function extractTrendsFromNews(xml: string, country: string): RealTrend[] {
  ...
  const stopWords = ['the', 'a', 'an', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'le', 'la', 'les', 'un', 'une', 'des', 'du', 'de', 'et', 'en', 'au', 'aux']
  ...
  while ((match = itemRegex.exec(xml)) !== null && index < 15) {
    ...
    const words = cleanTitle.split(/[\s\-:,]+/).filter(w => w.length > 2 && !stopWords.includes(w.toLowerCase()))
    ...
    const traffic = hoursSincePublish < 1 ? '500K+' : hoursSincePublish < 3 ? '200K+' : hoursSincePublish < 6 ? '100K+' : '50K+'
...
function generateEstimatedTrends(country: string): CachedData<RealTrend> {
  ...
  const countryTopics: Record<string, Array<{ title: string; desc: string; traffic: string }>> = {
    ...
    US: [
      ...
      { title: 'AI Regulation', desc: 'New government policies', traffic: '450K+' },
      { title: 'Stock Market', desc: 'S&P 500 movements', traffic: '680K+' },
      { title: 'Taylor Swift Tour', desc: 'Concert dates and tickets', traffic: '890K+' },
      { title: 'Supreme Court', desc: 'Latest rulings', traffic: '320K+' },
      { title: 'Marvel Phase 6', desc: 'New movie announcements', traffic: '750K+' },
      { title: 'Tesla Updates', desc: 'Elon Musk news', traffic: '580K+' },
      { title: 'TikTok Ban', desc: 'Legal developments', traffic: '920K+' },
    ],
    GB: [
      { title: 'Premier League', desc: 'Match results and standings', traffic: '850K+' },
    ...
    NG: [
      { title: 'Afrobeats Charts', desc: 'Top songs this week', traffic: '650K+' },
    ...
    MA: [
      ...
      { title: 'Ramadan 2026', desc: 'Horaires et programmes', traffic: '380K+' },
      { title: 'Tourisme Maroc', desc: 'Destinations tendance', traffic: '250K+' },
      { title: 'Economie Maroc', desc: 'Actualites economiques', traffic: '180K+' },
      { title: 'Musique Marocaine', desc: 'Nouveaux artistes', traffic: '320K+' },
      { title: 'Tech Maroc', desc: 'Startups et innovation', traffic: '150K+' },
      { title: 'Cinema Marocain', desc: 'Films et festivals', traffic: '200K+' },
      { title: 'Mode Casablanca', desc: 'Fashion week', traffic: '280K+' },
    ],
    SN: [
      { title: 'CAN 2026', desc: 'Coupe d Afrique des Nations', traffic: '520K+' },
      { title: 'Politique Senegal', desc: 'Actualites gouvernement', traffic: '280K+' },
      { title: 'Musique Senegalaise', desc: 'Mbalax et hip-hop', traffic: '350K+' },
      { title: 'Dakar Fashion', desc: 'Mode africaine', traffic: '180K+' },
      { title: 'Tech Dakar', desc: 'Innovation et startups', traffic: '120K+' },
      { title: 'Lutte Senegalaise', desc: 'Combats et resultats', traffic: '450K+' },
      { title: 'Economie CEDEAO', desc: 'Actualites regionales', traffic: '150K+' },
      { title: 'Series Senegalaises', desc: 'TV et streaming', traffic: '290K+' },
    ],
  }
  ...
  if (hour >= 18 && hour <= 23) {
    shuffled.unshift({ title: 'Prime Time TV', desc: 'Emissions du soir en direct', traffic: '600K+' })
  }
  if (isWeekend) {
    shuffled.unshift({ title: 'Weekend Plans', desc: 'Sorties et evenements', traffic: '450K+' })
  }
...
export async function fetchAllTrends(): Promise<CachedData<RealTrend>> {
  const results = await Promise.all(TRENDS_COUNTRIES.map(c => fetchGoogleTrends(c)))
...
const GOOGLE_NEWS_COUNTRY_CODES: Record<string, { hl: string; gl: string; ceid: string }> = {
...
async function fetchGoogleNewsRSS(country: string): Promise<CachedData<RealNewsArticle>> {
  const config = GOOGLE_NEWS_COUNTRY_CODES[country.toLowerCase()] || GOOGLE_NEWS_COUNTRY_CODES.fr
...
let cachedFallbackNews: { data: RealNewsArticle[]; generatedAt: number } | null = null
...
export async function fetchRealNews(country: string): Promise<CachedData<RealNewsArticle>> {
  ...
  if (isRateLimited('newsApi')) {
    if (cached) return { ...cached, source: 'fallback' }
    return { data: getFallbackNews(), fetchedAt: new Date().toISOString(), expiresAt: new Date().toISOString(), source: 'fallback' }
  }
  
  // Use deduplication to prevent concurrent requests for the same country
  return deduplicatedFetch(cacheKey, () => fetchRealNewsInternal(country, cached))
}

async function fetchRealNewsInternal(country: string, cached: CachedData<RealNewsArticle> | undefined): Promise<CachedData<RealNewsArticle>> {
  const cacheKey = `news_${country}`
  ...
  try {
    ...
    const articles: RealNewsArticle[] = data.articles.map((article: {
      title: string
      description: string
      url: string
      urlToImage: string | null
...
export async function fetchYouTubeTrending(country: string = 'FR'): Promise<CachedData<YouTubeVideo>> {
  if (!YOUTUBE_API_KEY) {
    return {
      data: [],
      fetchedAt: new Date().toISOString(),
...
export async function fetchRealVideos(country: string): Promise<CachedData<RealVideo>> {
  ...
  try {
    ...
    const videos: RealVideo[] = (data.items || []).map((item: {
      id: string
      snippet: { title: string; channelTitle: string; thumbnails: { high?: { url: string }; medium?: { url: string } }; publishedAt: string }
      ...
      return {
        ...
        viralScore,
        growthRate,
        badge: (index < 3 ? 'Viral' : index < 8 ? 'Early' : growthRate > 100 ? 'Breaking' : 'Trend') as RealVideo['badge'],
        country: country.toUpperCase(),
...
export async function fetchAllVideos(): Promise<CachedData<RealVideo>> {
  const results = await Promise.all(VIDEO_COUNTRIES.map(c => fetchRealVideos(c)))
...
export async function fetchTMDBTrending(mediaType: 'movie' | 'tv' | 'all' = 'all'): Promise<CachedData<TMDBContent>> {
  try {
    const response = await fetch(
    ...
    const content: TMDBContent[] = (data.results || []).map((item: {
      id: number
      title?: string
      name?: string
      overview: string
      ...
      genres: (item.genre_ids || []).map((id: number) => TMDB_GENRE_MAP[id] || 'Other')
    }))
...
export async function fetchLastFMTracks(country?: string): Promise<CachedData<LastFMTrack>> {
  try {
    ...
    const tracks: LastFMTrack[] = (tracksData || []).map((track: {
      name: string
      ...
      imageUrl: track.image?.[3]?.['#text'] || track.image?.[2]?.['#text'] || null,
      url: track.url
    }))
...
export async function fetchLastFMArtists(country?: string): Promise<CachedData<LastFMArtist>> {
  try {
    const endpoint = country 
    ...
    const data = await response.json()
    const artistsData = country ? data.topartists?.artist : data.artists?.artist
    
    const artists: LastFMArtist[] = (artistsData || []).map((artist: {
      name: string
      playcount: string
      listeners: string
      image: Array<{ '#text': string }>
      ...
      imageUrl: artist.image?.[3]?.['#text'] || artist.image?.[2]?.['#text'] || null,
      url: artist.url
    }))
