...
export function buildPageMetadata(input: BuildPageMetadataInput): Metadata {
  ...
  const fullTitle = input.rawTitle
    ? input.title
    : `${input.title} · ${SITE_NAME}`;
  const desc = truncate(
    input.description ||
      `${SITE_NAME} · tendances, radar et intelligence culturelle.`,
    160,
  );

  let ogImageUrl: string;
  if (input.ogImage) {
    ogImageUrl = input.ogImage.startsWith("http")
      ? input.ogImage
...
export function buildRootMetadata(): Metadata {
  ...
  return {
    ...
    openGraph: {
      ...
      url: base,
      images: [
        { url: ogImageUrl, width: 1200, height: 630, alt: defaultTitle },
      ],
    },
    twitter: {
      card: "summary_large_image",
