...
export function normalizeMovieItem(
  ...
  return {
    ...
    source: "TMDB",
    timestamp: String(
      item.releaseDate || item.release_date || new Date().toISOString(),
    ),
    region,
    score:
      Number(
...
export function createFallbackPacket<T>(
  ...
  return {
    ...
    fetchedAt: now,
    expiresAt: new Date(
      Date.now() + (config?.maxAgeMs || 3600000),
    ).toISOString(),
    ...
    error:
      error ||
      FALLBACK_MESSAGES[sourceId as keyof typeof FALLBACK_MESSAGES] ||
...
export function createDataPacket<T>(
  ...
  const expiresAt = new Date(
    now.getTime() + (config?.refreshIntervalMs || 900000),
  );
