...
function multiLevelFrom(
  ...
  const propagationProxy = clamp(
    growth * 0.55 +
      (input.views && input.views > 0 ? Math.log10(input.views + 1) * 12 : 0),
    0,
    100,
  );
...
function batchSimulations(
  avgViral: number,
  avgConfidence: number,
): AlgoCoreIntelligenceReport["scenarioSimulations"] {
...
function autoOptimizationNotes(
  ...
  if (avgConfidence < 0.52) {
    suggestions.push(
      "Enrichir les signaux (volumes, dates, sources) avant de durcir les seuils.",
    );
  }
  if (lowReliabilityCount > 0) {
    suggestions.push(
      "Traiter en priorité les entrées à fiabilité basse : compléter ou exclure.",
    );
  }
  if (weakShare > 0.35) {
    suggestions.push(
      "Part importante de signaux faibles : conserver une lecture prudente et re-vérifier.",
    );
  }
  if (suggestions.length === 0) {
    suggestions.push(
      "Maintenir le rythme actuel : qualité des entrées suffisante pour des classements stables.",
    );
  }
...
function vigilanceNotes(
  ...
  const lowReliabilityCount = items.filter(
    (i) => i.reliability.score < 55,
  ).length;
  const notes: string[] = [];
  if (patterns.duplicateTitles > 0) {
    notes.push(
      "Titres dupliqués détectés : risque de double comptage ou de scrap répété.",
    );
  }
  if (lowReliabilityCount > 0) {
    notes.push(
      `${lowReliabilityCount} élément(s) avec fiabilité basse · vérifier la provenance.`,
    );
  }
...
function viralitySliceFrom(items: AlgoCoreItemReport[]): ViralitySlice {
  ...
  for (const it of items) {
    ...
    if (
      it.intelligence.viralScore.phase === "rising" ||
      it.intelligence.viralScore.phase === "peak"
    ) {
      rising += 1;
    }
...
function userLayerNotes(
  ctx: AlgoCoreUserContext | undefined,
  items: AlgoCoreItemReport[],
): AlgoCoreIntelligenceReport["userLayer"] {
  if (
    !ctx?.interestByCategory ||
    Object.keys(ctx.interestByCategory).length === 0
  ) {
    return {
      applied: false,
      notes: [
        "Pas de profil d’intérêts fourni : priorisation purement signal-based.",
      ],
    };
  }
  ...
  return {
    ...
    notes: [
      ...
      ctx.regionHint
        ? `Contexte régional pris en compte : ${ctx.regionHint}.`
        : "Locale générique : affiner regionHint si besoin.",
    ],
  };
}
...
export function runAlgoCoreIntelligence(
  ...
  if (rawItems.length === 0) {
    return {
      ...
      autoOptimization: {
        suggestions: [
          "Ajouter des entrées pour activer l’analyse multi-niveaux.",
        ],
  ...
  const avgViral =
    items.reduce((s, i) => s + i.intelligence.viralScore.score, 0) /
    items.length;
  const avgConfidence =
  ...
  const autoOptimization = autoOptimizationNotes(
    avgConfidence,
    vig.lowReliabilityCount,
    weakShare,
  );
