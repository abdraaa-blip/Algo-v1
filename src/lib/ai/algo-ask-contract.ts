...
export const algoAskResponseSchema = z.object({
  answer: z
    ...
    .min(1)
    .max(12_000)
    .describe('Réponse principale ALGO AI en français (markdown léger possible).'),
  options: z
    .array(algoAskOptionSchema)
    .max(4)
    .optional()
    .describe('2–4 options comparées si la question appelle un choix ; sinon omettre le champ.'),
...
export function toPublicStructured(output: AlgoAskModelOutput): AlgoAskStructured | undefined {
  ...
  if (!structured.options && !structured.recommendedChoice && !structured.nextStep) {
    return undefined
  }
  return structured
}
