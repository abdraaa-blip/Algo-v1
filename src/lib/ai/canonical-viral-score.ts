import { BASE_WEIGHTS, computeAdaptiveWeights, type AdaptiveSignals } from '@/lib/ai/adaptive-weighting'
...
function keywordOverlapScore(topic: string, keywords: string[], signals: string[]): number {
...
function hookScore(topic: string, hasThumbnail: boolean, hasVideo: boolean, keywords: string[]): number {
  let score = 45
...
function formatScore(platform: CanonicalScoreInput['platform'], hasVideo: boolean, hasThumbnail: boolean): number {
  let score = 50
  if ((platform === 'youtube' || platform === 'tiktok') && hasVideo) score += 25
...
function emotionScore(topic: string): number {
  ...
  const emotionalTerms = [
    'incroyable', 'choquant', 'viral', 'secret', 'urgent', 'surprise',
    'amazing', 'shocking', 'unbelievable', 'breaking', 'secret',
  ]
  let score = 40
...
export function computeCanonicalViralScore(input: CanonicalScoreInput): CanonicalScoreOutput {
  const { weights, notes, rollbackApplied, version } = computeAdaptiveWeights(input.adaptiveSignals)
  ...
  const hook = hookScore(input.topic, input.hasThumbnail, input.hasVideo, input.keywords)
  const trend = keywordOverlapScore(input.topic, input.keywords, input.trendSignals)
  const format = formatScore(input.platform, input.hasVideo, input.hasThumbnail)
