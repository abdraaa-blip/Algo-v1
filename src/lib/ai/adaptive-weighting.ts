...
export function computeAdaptiveWeights(signals?: Partial<AdaptiveSignals>): {
  ...
  if (
    !signals ||
    typeof signals.engagementRate !== "number" ||
    typeof signals.frictionRate !== "number"
  ) {
    return {
  ...
  if (frictionRate > 0.2 || (frictionRate > 0.12 && engagementRate < 0.1)) {
    return {
      weights: BASE_WEIGHTS,
      notes: [
        "automatic rollback: degraded adaptive signals, reverted to baseline",
      ],
      rollbackApplied: true,
      version: getWeightVersion(BASE_WEIGHTS),
    };
  ...
  if (engagementRate < 0.15) {
    adjusted.trend = clamp(
      adjusted.trend + MAX_WEIGHT_SHIFT,
      MIN_WEIGHT,
      MAX_WEIGHT,
    );
    adjusted.hook = clamp(
      adjusted.hook + MAX_WEIGHT_SHIFT / 2,
      MIN_WEIGHT,
      MAX_WEIGHT,
    );
    notes.push("engagement low: boosted trend/hook weights");
  } else if (engagementRate > 0.35) {
    adjusted.format = clamp(
      adjusted.format + MAX_WEIGHT_SHIFT / 2,
      MIN_WEIGHT,
      MAX_WEIGHT,
    );
  ...
  if (frictionRate > 0.08) {
    adjusted.timing = clamp(
      adjusted.timing + MAX_WEIGHT_SHIFT,
      MIN_WEIGHT,
      MAX_WEIGHT,
    );
    adjusted.emotion = clamp(
      adjusted.emotion + MAX_WEIGHT_SHIFT / 2,
      MIN_WEIGHT,
      MAX_WEIGHT,
    );
  ...
  return {
    ...
    notes: notes.length
      ? notes
      : ["adaptive signals stable: baseline-biased weights"],
