 ...
 */
function calculateInterestScores(interactions: UserInteraction[]): Record<string, number> {
 ...
 */
function calculateTypePreferences(interactions: UserInteraction[]): Record<string, number> {
  const counts: Record<string, number> = {}
  
  for (const interaction of interactions) {
    if (interaction.action !== 'skip') {
      counts[interaction.contentType] = (counts[interaction.contentType] || 0) + 1
    }
  }
...
export function scoreContentRelevance(
  ...
  const categoryScore = profile.categories[content.category.toLowerCase()] || 50
 ...
 */
export function getTopInterests(profile: UserInterestProfile | null, limit = 5): string[] {
  if (!profile) return []
 ...
 */
export function isPeakActivityTime(profile: UserInterestProfile | null): boolean {
 ...
 */
export function getPersonalizedContent<T extends { category: string; viralScore: number }>(
  contents: T[],
  profile: UserInterestProfile | null,
  contentType: string
