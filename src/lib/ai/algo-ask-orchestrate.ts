...
export function shouldServerEnrichTrends(
  ...
  const list =
    clientTrends?.filter((t) => typeof t === "string" && t.trim()) ?? [];
...
export async function orchestrateAskAlgo(input: {
  ...
  if (
    shouldServerEnrichTrends(input.clientContext?.currentTrends, {
      serverEnrich: input.serverEnrich,
    })
  ) {
    try {
      ...
      console.warn(
        "[algo-ask-orchestrate] enrichissement tendances indisponible:",
        e,
      );
    }
  }
  ...
  const userCountryLabel =
    country || input.clientContext?.userCountry?.trim() || "global";
  ...
  try {
    ...
    console.warn(
      "[algo-ask-orchestrate] askAlgo exception · repli synthèse",
      err,
    );
