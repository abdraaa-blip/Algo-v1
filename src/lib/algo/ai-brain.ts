...
function generateCreatorTip(
  ...
  const platformTips: Record<string, string> = {
    youtube:
      "Créer une vidéo de 8–12 minutes avec un hook dans les 3 premières secondes",
    reddit:
      "Poster avec un titre provocateur ou une question ouverte pour maximiser les commentaires",
    hackernews:
      "Presenter l'angle technique ou startup de maniere concise et factuelle",
    github:
      "Ajouter un README clair avec demo GIF et badges pour maximiser les stars",
  ...
  const formatTips: Record<string, string> = {
    video:
      "Format vertical 9:16 pour TikTok/Reels, hook visuel dans la premiere seconde",
    article:
      "Thread Twitter avec les points cles + lien vers l'article complet",
  ...
  const platformTip =
    platformTips[source] || "Adapter le format a ta plateforme principale";
  const formatTip =
    formatTips[format] || "Choisir le format qui engage le plus ton audience";
...
function generateCulturalContext(item: ContentItem, category: string): string {
  ...
  for (const [theme, keywords] of Object.entries(themes)) {
    if (
      keywords.some(
        (k) =>
          title.includes(k) || tags.some((t) => t.toLowerCase().includes(k)),
      )
    ) {
...
function getThemeContext(theme: string): string {
  const contexts: Record<string, string> = {
    gaming:
      "La communauté gaming réagit fortement aux nouveautés et controverses. Ce contenu touche une audience jeune et très engagée sur les réseaux.",
    ...
    politics:
      "Les sujets politiques génèrent des réactions intenses et polarisées. Le timing électoral ou une actualité chaude explique la viralité.",
    entertainment:
      "Le divertissement domine les conversations sociales. Ce contenu profite du buzz autour d'une sortie ou d'une actualité people.",
    sports:
      "Les événements sportifs créent des pics d'engagement massifs. Ce contenu surfe sur une compétition ou une polémique en cours.",
    music:
      "La musique virale crée des trends TikTok et des challenges. Ce contenu pourrait devenir un son viral dans les prochains jours.",
    science:
      "Les découvertes scientifiques fascinent quand elles sont accessibles. Ce contenu vulgarise un sujet complexe de manière engageante.",
  };
  return (
    contexts[theme] ||
    "Ce contenu touche un sujet d'actualité qui résonne avec l'audience cible."
  );
}
