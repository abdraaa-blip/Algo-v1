...
export const ALGORITHM_WEIGHTS = {
  ...
  engagementRate: 0.2, // (likes + comments + shares) / views
  crossPlatformSpread: 0.2, // Same content on multiple platforms
  searchInterest: 0.15, // Google Trends correlation
  commentSentiment: 0.1, // Positive/negative ratio
  recencyBonus: 0.1, // Freshness multiplier
} as const;
...
export function calculateViralScore(
  ...
  const crossPlatformSpread =
    item.source === "youtube" && views > 100000
      ? 60
      : item.source === "reddit" && (m.upvotes || 0) > 10000
        ? 50
        : item.source === "hackernews" && (m.upvotes || 0) > 500
          ? 45
          : item.source === "github" && likes > 1000
            ? 55
            : 30;
  ...
  // Determine tier
  const tier =
    total >= SCORE_THRESHOLDS.explosive
      ? "explosive"
      : total >= SCORE_THRESHOLDS.viral
        ? "fire"
        : total >= SCORE_THRESHOLDS.trending
          ? "hot"
          : total >= SCORE_THRESHOLDS.earlySignal
            ? "warm"
            : "cold";
...
function generatePrediction(
  ...
  const trajectory =
    momentum > 5 ? "rising" : momentum < -3 ? "fading" : "peaking";
    ...
    signals: [
      {
        ...
        weight: "25%",
        description:
          "Views per hour divided by follower count. Measures how fast content spreads relative to audience size.",
      },
      {
        name: "Engagement Rate",
        weight: "20%",
        description:
          "Likes + comments×2 + shares×3 divided by views. Comments and shares weighted higher as stronger signals.",
      },
      {
        name: "Cross-Platform Spread",
        weight: "20%",
        description:
          "Detection of same content appearing across multiple platforms. Key predictor of mainstream virality.",
      },
      {
        name: "Search Interest",
        weight: "15%",
        description:
          "Google Trends correlation for related keywords. Shows if content is driving searches.",
      },
      {
        name: "Comment Sentiment",
        weight: "10%",
        description:
          "AI analysis of comment sentiment. Strong reactions (positive or negative) indicate viral potential.",
      },
      {
        name: "Recency Bonus",
        weight: "10%",
        description:
          "Freshness multiplier. Content under 6 hours gets maximum boost, then decays at 2% per hour.",
      },
    ],
  };
}
