...
function trackExperimentAssignment(experimentId: string, variant: string) {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[ALGO Experiment]', { experimentId, variant, userId: getUserId() })
  }
}

// Track conversion for an experiment
export function trackConversion(experimentId: string, metric: string, value?: number) {
...
export function getActiveExperiments(): Array<{ experiment: Experiment; variant: string | null }> {
  return EXPERIMENTS
    .filter(e => e.active)
    .map(experiment => ({
      experiment,
