...
export interface Achievement {
  ...
  description: string
  icon: string
  category: 'discovery' | 'engagement' | 'social' | 'streak' | 'mastery' | 'special'
...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  },
  {
    ...
    rarity: 'epic',
    xpReward: 800,
    condition: { type: 'count', metric: 'comment_likes_received', threshold: 100 }
  },
...
export const RARITY_COLORS = {
  common: { bg: 'bg-zinc-700', text: 'text-zinc-300', border: 'border-zinc-600' },
  rare: { bg: 'bg-blue-900/50', text: 'text-blue-400', border: 'border-blue-500' },
  epic: { bg: 'bg-purple-900/50', text: 'text-purple-400', border: 'border-purple-500' },
  legendary: { bg: 'bg-amber-900/50', text: 'text-amber-400', border: 'border-amber-500' },
  mythic: { bg: 'bg-rose-900/50', text: 'text-rose-400', border: 'border-rose-500' }
}
...
export class AchievementManager {
  ...
  getProgress(achievementId: string): { current: number; max: number; percentage: number } {
  ...
  getLevel(): { level: number; name: string; currentXP: number; nextLevelXP: number; progress: number } {
    ...
    for (let i = 0; i < levels.length; i++) {
      if (totalXP >= levels[i].xp) {
        ...
        nextLevel = levels[i + 1] || { level: 11, name: 'Transcendent', xp: 50000 }
      }
    }
