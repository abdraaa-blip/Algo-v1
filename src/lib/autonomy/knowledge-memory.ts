...
export function addKnowledgeMemory(entry: Omit<KnowledgeMemoryEntry, 'id' | 'createdAt'>): KnowledgeMemoryEntry {
...
export async function persistKnowledgeMemory(entry: KnowledgeMemoryEntry): Promise<void> {
...
export function getKnowledgeMemory(limit = 100, domain?: KnowledgeMemoryEntry['domain']) {
  const filtered = domain ? memoryStore.filter((m) => m.domain === domain) : memoryStore
...
export async function getDurableKnowledgeMemory(
  ...
  const tableQuery = supabase
    ...
    .select('id, created_at, region, domain, summary, signals, confidence, outcome, tags')
  ...
  if (Array.isArray(analyticsRows) && analyticsRows.length > 0) {
    const mapped = analyticsRows
      ...
      .filter(Boolean)
      .map((p) => ({
        id: String(p?.id || `legacy_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`),
