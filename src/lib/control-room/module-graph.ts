...
export type ControlRoomModuleId = 'probe' | 'health' | 'pipeline' | 'radar' | 'trends'

export type ControlRoomFlowEdge = { from: ControlRoomModuleId; to: ControlRoomModuleId }
...
export function activeFlowEdges(
  ...
  return CONTROL_ROOM_FLOW_EDGES.filter((e) => set.has(e.from) && set.has(e.to))
}
