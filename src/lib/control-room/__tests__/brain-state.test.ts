...
import { clampControlRoomIntensity, DEFAULT_CONTROL_ROOM_BRAIN_STATE } from '@/lib/control-room/brain-state'
...
describe('DEFAULT_CONTROL_ROOM_BRAIN_STATE', () => {
  it('expose flux et modules pour le HUD', () => {
    expect(DEFAULT_CONTROL_ROOM_BRAIN_STATE.flowSpeed).toBeGreaterThanOrEqual(0)
    expect(DEFAULT_CONTROL_ROOM_BRAIN_STATE.activeModuleIds.length).toBeGreaterThan(0)
  })
})
