...
describe('mapHealthProbeResultToBrainState', () => {
  ...
  it('alert quand HTTP KO', () => {
    const s = mapHealthProbeResultToBrainState({ resOk: false, json: { ok: true, platformKeysConfigured: true } })
    expect(s.mode).toBe('alert')
    expect(s.activeModuleIds).toEqual(['probe', 'health'])
  })

  it('alert quand json absent ou ok false', () => {
    expect(mapHealthProbeResultToBrainState({ resOk: true, json: null }).mode).toBe('alert')
    expect(mapHealthProbeResultToBrainState({ resOk: true, json: { ok: false } }).mode).toBe('alert')
  })
})
