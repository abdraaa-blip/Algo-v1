...
import { activeFlowEdges, CONTROL_ROOM_FLOW_EDGES, defaultActiveModulesForMode } from '@/lib/control-room/module-graph'
...
describe('activeFlowEdges', () => {
  it('filtre les arêtes dont les deux extrémités sont actives', () => {
    ...
    expect(edges.some((e) => e.from === 'pipeline' && e.to === 'radar')).toBe(false)
  })
...
describe('defaultActiveModulesForMode', () => {
  it('couvre les modes utilisés par la Control Room', () => {
    ...
    expect(defaultActiveModulesForMode({ mode: 'analyzing' })).toEqual(['probe', 'health'])
    expect(defaultActiveModulesForMode({ mode: 'alert' })).toEqual(['probe', 'health'])
  })
})
