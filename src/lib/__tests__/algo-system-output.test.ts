...
describe("algo system · enveloppe & QC", () => {
  ...
  it("evaluateAlgoResponseQuality agrège les trois indicateurs", () => {
    const q = evaluateAlgoResponseQuality(
      "Une réponse assez longue pour dépasser le seuil de clarté et d’utilité minimale.",
      {
  ...
  it("qualityTransparencyHints ne parle que des flags faux", () => {
    const hints = qualityTransparencyHints({
      isClear: false,
      isUseful: true,
      isCoherent: true,
    });
  ...
  it("attachAskOutputQuality combine enveloppe + quality", () => {
    const { standard, quality } = attachAskOutputQuality(
      "Q?",
      "Réponse assez longue pour passer le seuil utilité · avec une structure.",
      {
        options: [{ title: "A" }],
      },
    );
  ...
  it("processAlgoAiRequest injecte route + confiance", async () => {
    ...
    const out = await processAlgoAiRequest({
      question: "Analyse viralité TikTok",
    });
