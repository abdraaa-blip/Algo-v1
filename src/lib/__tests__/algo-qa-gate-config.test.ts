...
describe('config/algo-qa-gate', () => {
  it('référence le fichier design system', () => {
    expect(ALGO_QA_SOURCES.designAndUiRules).toBe('config/algo-system-rules.ts')
    expect(ALGO_QA_SOURCES.deployGate).toBe('config/algo-deploy-gate.ts')
    expect(ALGO_QA_SOURCES.coherenceRitual).toBe('docs/ALGO_COHERENCE_RITUAL.md')
    expect(ALGO_QA_SOURCES.coherenceCursorRule).toContain('algo-coherence-review')
    expect(ALGO_QA_SOURCES.offlineEvolution).toBe('docs/ALGO_OFFLINE_EVOLUTION.md')
    expect(ALGO_QA_SOURCES.productionBoundaries).toBe('docs/ALGO_PRODUCTION_BOUNDARIES.md')
    expect(ALGO_QA_SOURCES.controlRoom).toBe('docs/ALGO_CONTROL_ROOM.md')
    expect(ALGO_QA_SOURCES.designEvolution).toBe('docs/ALGO_DESIGN_EVOLUTION.md')
    expect(ALGO_QA_SOURCES.uxCharter).toBe('docs/ALGO_UX_CHARTER.md')
    expect(ALGO_QA_SOURCES.uxCognitiveAudit).toBe('docs/ALGO_UX_COGNITIVE_AUDIT.md')
    expect(ALGO_QA_SOURCES.releaseReadiness).toBe('docs/ALGO_RELEASE_READINESS.md')
    expect(ALGO_QA_SOURCES.gtmNotes).toBe('docs/ALGO_GTM_NOTES.md')
    expect(ALGO_QA_SOURCES.docsIndex).toBe('docs/README.md')
    expect(ALGO_QA_SOURCES.productMasterOverview).toBe('docs/product/master-overview.md')
    expect(ALGO_QA_SOURCES.operationsPlaybook).toBe('docs/ALGO_OPERATIONS_PLAYBOOK.md')
  })

  it('expose une checklist couvrante', () => {
    expect(ALGO_QA_CHECKLIST.technique.length).toBeGreaterThanOrEqual(2)
    expect(ALGO_QA_CHECKLIST.uiUx.some((s) => s.includes('algo-system-rules'))).toBe(true)
  ...
  it('définit la commande verify:release', () => {
    ...
    expect(ALGO_QA_RELEASE_COMMANDS.optionalDeeper).toContain('npm run typecheck')
  })
  ...
  it('playbook ops : section blueprints et ancrage src/app (anti-pack générique)', () => {
    const playbookPath = path.join(process.cwd(), ALGO_QA_SOURCES.operationsPlaybook)
