...
describe("parseSnapshotQuery", () => {
  ...
  it("parses since ISO", () => {
    const q = parseSnapshotQuery(
      new URLSearchParams("since=2026-01-15T12:00:00.000Z"),
    );
    expect(q.ok).toBe(true);
    if (!q.ok) return;
  ...
  it("filters types", () => {
    const q = parseSnapshotQuery(
      new URLSearchParams("types=trend_signal,model_weight_version"),
    );
    expect(q.ok).toBe(true);
    if (!q.ok) return;
  ...
  it("rejects unknown type", () => {
    const q = parseSnapshotQuery(
      new URLSearchParams("types=trend_signal,unknown"),
    );
    expect(q.ok).toBe(false);
  });
