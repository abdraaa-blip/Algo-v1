...
describe("config/algo-deploy-gate", () => {
  it("référence QA et design + workflows", () => {
    expect(ALGO_DEPLOY_SOURCES.releaseReadiness).toBe(
  ...
  it("commande minimale alignée verify:release", () => {
    expect(ALGO_DEPLOY_COMMANDS.minimumBeforeDeploy).toContain(
      "verify:release",
    );
  });
  ...
  it("checklist couvre build, sécurité, smoke", () => {
    expect(
      ALGO_DEPLOY_CHECKLIST.buildAndCompilation.length,
    ).toBeGreaterThanOrEqual(2);
    expect(
      ALGO_DEPLOY_CHECKLIST.security.some((s) => s.includes("NEXT_PUBLIC")),
    ).toBe(true);
    expect(
      ALGO_DEPLOY_CHECKLIST.functionalSmoke.some((s) =>
        s.includes("/api/ai/ask"),
      ),
    ).toBe(true);
  });

  it("politique de blocage explicite", () => {
    expect(ALGO_DEPLOY_BLOCK_POLICY.immediateActions[0]).toMatch(
      /Bloquer|bloquer/i,
    );
