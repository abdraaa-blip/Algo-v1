...
import { runAlgoCoreIntelligence, ALGO_CORE_PHILOSOPHY_FR } from '@/lib/algo-core-intelligence'
...
describe('runAlgoCoreIntelligence', () => {
  ...
  it('ranks a single item and runs three scenario simulations', () => {
    ...
    expect(r.scenarioSimulations.viralBurst.simulatedScore).toBeGreaterThanOrEqual(0)
  ...
  it('detects duplicate titles and repeated tokens across batch', () => {
    const r = runAlgoCoreIntelligence([
      ...
      sampleItem({ id: 'b', title: 'Breaking news climate summit', source: 'other' }),
      sampleItem({ id: 'c', title: 'Climate summit updates today' }),
    ])
    expect(r.patterns.duplicateTitles).toBeGreaterThanOrEqual(1)
    expect(r.patterns.repeatedTokens.some((t) => t.token === 'climate' || t.token === 'summit')).toBe(true)
  })
  ...
  it('applies user interest boost on matching category', () => {
    ...
    expect(boosted.items[0].priority.userBoost).toBeGreaterThan(base.items[0].priority.userBoost)
