...
import {
  ...
  buildViralControlCockpitPayload,
} from "@/lib/intelligence/viral-control-cockpit";
...
describe("viral-control-cockpit", () => {
  ...
  it("deriveVelocityAcceleration returns nulls for empty or single point", () => {
    expect(deriveVelocityAcceleration([])).toEqual({
      velocityPerHour: null,
      accelerationPerHour2: null,
    });
    expect(
      deriveVelocityAcceleration([
        {
  ...
  it("deriveVelocityAcceleration computes acceleration with four points", () => {
    const pts = [
      {
        at: new Date("2026-01-01T04:00:00Z").toISOString(),
        viralityScore: 48,
        confidence: 0.5,
        anomalyCount: 0,
      },
      {
        at: new Date("2026-01-01T06:00:00Z").toISOString(),
        viralityScore: 50,
        confidence: 0.5,
        anomalyCount: 0,
      },
      {
        at: new Date("2026-01-01T08:00:00Z").toISOString(),
        viralityScore: 52,
        confidence: 0.5,
        anomalyCount: 0,
      },
    ...
    const { velocityPerHour, accelerationPerHour2 } =
      deriveVelocityAcceleration(pts);
  ...
  it("buildViralControlCockpitPayload returns shaped cockpit", () => {
    const p = buildViralControlCockpitPayload({
      ...
      recommendations: [
        { type: "timing", title: "Tester un hook court", confidence: 0.7 },
      ],
    });
