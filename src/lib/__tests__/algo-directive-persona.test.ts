...
describe("Directive 2 → synthèse ALGO", () => {
  ...
  it("la couche directive reste sobre et ancrée produit", () => {
    ...
    expect(ALGO_DIRECTIVE_OPERATING_LAYER).toMatch(
      /double focale|création \+ risque/i,
    );
  ...
  it("la couche Core Intelligence pousse utilité et décision sans promesses magiques", () => {
    ...
    expect(ALGO_AI_CORE_INTELLIGENCE_LAYER).toMatch(
      /promesses magiques|doctrine/i,
    );
  });

  it("la directive maître ancre cohérence système et utilité", () => {
    expect(ALGO_MASTER_SYSTEM_DIRECTIVE_LAYER).toMatch(
      /Directive maître|système global/i,
    );
    expect(ALGO_MASTER_SYSTEM_DIRECTIVE_LAYER).toMatch(
      /comprendre|décider|agir/i,
    );
