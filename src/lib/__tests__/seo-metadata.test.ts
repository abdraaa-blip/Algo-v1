...
import {
  absoluteUrl,
  getSiteBaseUrl,
  SITE_TRANSPARENCY_AI_CALIBRATION_HREF,
} from "@/lib/seo/site";
...
describe("seo site", () => {
  ...
  it("exposes stable calibrage IA anchor href", () => {
    expect(SITE_TRANSPARENCY_AI_CALIBRATION_HREF).toBe(
      "/transparency#algo-ai-directive",
    );
  });
});
...
describe("buildPageMetadata", () => {
  it("includes openGraph and twitter images", () => {
    const m = buildPageMetadata({
      title: "Test Page",
      description:
        "A description long enough for SEO checks and truncation paths.",
      path: "/test",
      keywords: ["a", "b"],
    });
    ...
    const ogCount = Array.isArray(ogImages)
      ? ogImages.length
      : ogImages
        ? 1
        : 0;
    const twCount = Array.isArray(twImages)
      ? twImages.length
      : twImages
        ? 1
        : 0;
...
describe("keywords-from-title", () => {
  it("extracts keywords", () => {
    const k = extractTitleKeywords(
      "Le président annonce une réforme majeure du numérique",
    );
