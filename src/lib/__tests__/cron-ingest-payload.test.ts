...
describe("cron ingest payload", () => {
  it("normalise Spotify dégradé : tableau data avec title sur chaque item", () => {
    ...
    expect(
      rows.every((r) => typeof (r as { title?: string }).title === "string"),
    ).toBe(true);
    expect(businessLevelErrorFromJson(degraded)).toBe(
  ...
  it("businessLevelErrorFromJson absent si success true", () => {
    expect(
      businessLevelErrorFromJson({ success: true, data: [] }),
    ).toBeUndefined();
  });

  it("businessLevelErrorFromJson sans message explicite", () => {
    expect(businessLevelErrorFromJson({ success: false })).toBe(
      "success:false",
    );
  });
