import { recordYoutubeObservation, getYoutubeObservationSeries } from '@/lib/social/youtube-observation-series'
...
type Cached = { expiresAtMs: number; data: Omit<YoutubeVideoPublicMetrics, 'fetchedAt' | 'observationSeries'> }
 ...
 */
export async function fetchYoutubePublicMetrics(videoId: string): Promise<YoutubeVideoPublicMetrics | null> {
  ...
  if (cached && cached.expiresAtMs > now) {
    ...
    googleCache.set(videoId, { expiresAtMs: now + GOOGLE_CACHE_TTL_MS, data: base })
  }
