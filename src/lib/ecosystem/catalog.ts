import { ALGO_ECOSYSTEM_API_VERSION, ALGO_ECOSYSTEM_SCHEMA_DATE } from '@/lib/ecosystem/constants'
...
export const ECOSYSTEM_ENDPOINT_CATALOG: CatalogEndpoint[] = [
  {
    ...
    auth: 'none',
    summary: 'Catalogue machine-readable des capacités ALGO + liens documentation.',
    category: 'meta',
  },
  {
    id: 'ecosystem.health',
    ...
    path: '/api/v1/health',
    auth: 'none',
    summary: 'Santé légère pour orchestrateurs (clés plateforme configurées, version).',
    category: 'meta',
  },
  {
    id: 'ecosystem.trends',
    ...
    summary: 'Tendances normalisées (JSON ou CSV). Param: country=FR optionnel.',
    category: 'read',
  },
  {
    ...
    summary: 'Ingestion d’événements depuis une app tierce (même contrat que /api/analytics/events).',
    category: 'write',
  },
  {
  ...
  },
  {
    ...
    auth: 'ip_rate_limit',
    rateLimitPerMinute: 120,
    summary: 'Carte fiabilité des sources (fallbacks, limites, baseline) — transparence data.',
    category: 'meta',
  },
  {
    id: 'ai.ask',
    ...
    summary:
      'ALGO AI — Q&R via centralAsk ; answer + structured? (options, recommendedChoice, nextStep), contextMeta, transparencyLines.',
    category: 'ai',
  },
  {
...
export function buildManifestPayload(baseUrl: string) {
  return {
    ...
    documentation: {
      ...
      description: 'Guide connexion apps + setup Supabase/Vercel (migration, clés, cron).',
    },
    interchange: {
      primary: 'REST JSON',
