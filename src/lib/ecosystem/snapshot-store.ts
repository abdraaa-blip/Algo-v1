...
export function parseSnapshotQuery(
...
): { ok: true; value: SnapshotQuery } | { ok: false; error: string } {
  const sinceRaw = searchParams.get("since");
  let since: Date;
  if (sinceRaw) {
    const t = Date.parse(sinceRaw);
    if (Number.isNaN(t))
      return { ok: false, error: "Invalid since: use ISO 8601 datetime" };
    since = new Date(t);
  } else {
  ...
  if (typesRaw?.trim()) {
    ...
    const invalid = parts.filter(
      (p) => !SNAPSHOT_ENTITY_TYPES.includes(p as SnapshotEntityType),
    );
    if (invalid.length) {
      return {
        ok: false,
  ...
  const limitPerType = parseDefaultedListLimit(
...
export async function fetchSnapshotBundle(
  ...
  if (!supabase)
    return {
      ok: false,
      error:
        "Clé secrète Supabase manquante (SERVICE_ROLE ou SUPABASE_SECRET_KEY)",
    };
