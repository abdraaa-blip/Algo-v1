...
export async function runTrendSnapshotJob(
  ...
  if (!admin) {
    return regions.map((r) => ({
      ...
      ok: false,
      error:
        "Clé secrète Supabase non configurée (SUPABASE_SERVICE_ROLE_KEY ou SUPABASE_SECRET_KEY)",
    }));
  }
...
export async function runModelWeightSnapshotJob(): Promise<{
  ...
  if (!createSupabaseAdminClient()) {
    return {
      ok: false,
      error:
        "Clé secrète Supabase non configurée (SERVICE_ROLE ou SUPABASE_SECRET_KEY)",
    };
  }
