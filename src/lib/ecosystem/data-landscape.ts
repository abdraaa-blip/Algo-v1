...
export type StorageClass = 'relational' | 'memory_process' | 'client_browser' | 'external_api' | 'derived'
...
export const DATA_LANDSCAPE: DataDomainRow[] = [
  {
    ...
    examples: ['live-trends', 'realtime/trends', 'v1/trends', 'trend_signal (Supabase)'],
    storage: 'external_api',
    notes: 'Agrégation Google Trends + caches TTL ; persistance optionnelle en base via migration ecosystem + ALGO_SNAPSHOT_PERSIST.',
  },
  {
  ...
  },
  {
    ...
    storage: 'external_api',
    notes: 'Fournisseurs externes; normaliser via DTOs communs côté intégration.',
  },
  {
    domain: 'Événements analytics & feedback',
    examples: ['/api/analytics/events', '/api/v1/ingest/events', 'intelligence/feedback'],
    storage: 'memory_process',
    notes: 'File mémoire process + option Supabase `analytics_events` si configuré.',
  },
  {
    domain: 'Intelligence / ops / apprentissage',
    examples: ['/api/intelligence/*', 'radar-history', 'decision-log', 'learning-history'],
    storage: 'memory_process',
    notes:
  ...
  },
  {
    ...
    notes: 'Préférences légères côté client; ne pas traiter comme source de vérité serveur.',
  },
  {
