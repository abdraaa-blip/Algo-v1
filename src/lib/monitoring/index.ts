...
export function trackPerformanceAnomaly(
  ...
  if (value > threshold) {
    ...
    addToBuffer(event)
    logger.warn(`[Monitoring] Performance anomaly: ${metric}`, { value, threshold })
  }
}
...
export function generateDailyReport(): {
  ...
  const todayEvents = eventBuffer.filter(e => e.timestamp.startsWith(today || ''))
  ...
  return {
    ...
    rlsViolations,
    apiErrors,
    performanceAnomalies: performanceAnomalies.filter(a => a.timestamp.startsWith(today || '')).length,
    topErrors,
  }
}
...
export function getMonitoringHealth(): {
  ...
  const recentErrors = recentEvents.filter(e => e.severity === 'error' || e.severity === 'critical').length
  const recentAnomalies = performanceAnomalies.filter(a => a.timestamp > fiveMinutesAgo).length
