...
export const DATA_RELIABILITY_MAP: readonly DataReliabilityEntry[] = [
  {
    ...
    fallbacksFr: [
      "Réponse cache / source secondaire dans real-data-service",
      "Payload vide + message côté client",
    ],
  ...
  },
  {
    ...
    fallbacksFr: [
      "Données cache ou partielles",
      "Liste vide avec état partial",
    ],
  ...
  },
  {
    id: "ai_context",
    primaryRoute:
      "POST /api/ai/ask (processAlgoAiRequest · enrichissement tendances)",
    fallbacksFr: [
  ...
  },
  {
    ...
    primaryRoute:
      "GET /api/intelligence/radar-history · table intelligence_radar_point",
    fallbacksFr: [
...
export function getDataReliabilityEntry(
  id: string,
): DataReliabilityEntry | undefined {
  return DATA_RELIABILITY_MAP.find((e) => e.id === id);
}

/** Exposition simple pour une future UI « couverture / confiance ». */
export function listDataReliabilitySummaries(): Array<{
  id: string;
  baselineReliability: number;
  limitsFr: string;
}> {
