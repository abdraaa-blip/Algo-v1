...
export async function fetchWithCache<T>(
  ...
  const fetchPromise = withResilience<T>(`fetch:${key}`, fetcher, {
    retries: 3,
    retryDelay: 1000,
  });
...
async function revalidateInBackground<T>(
  ...
  try {
    const data = await withResilience<T>(`revalidate:${key}`, fetcher, {
      retries: 2,
    });
...
export function mutateCache<T>(
  ...
  const newData =
    typeof data === "function"
      ? (data as (c: T | undefined) => T)(current)
      : data;
...
export function prefetch<T>(
  ...
  if (
    cached &&
    !isStale(cached, config.staleTime ?? DEFAULT_CACHE_CONFIG.staleTime)
  ) {
    return;
  }
...
>(baseUrl: string, defaultConfig: Partial<CacheConfig> = {}) {
  return {
