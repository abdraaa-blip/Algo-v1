...
function transitionState(circuit: CircuitStats, newState: CircuitState): void {
  if (circuit.state !== newState) {
    if (process.env.NODE_ENV === 'development') {
      console.log(`[CircuitBreaker] State transition: ${circuit.state} -> ${newState}`)
    }
...
export async function withResilience<T>(
  ...
  const { fallback, circuit, bulkhead, retries = 3, retryDelay = 1000 } = options
...
export function getAllCircuitStats(): Record<string, CircuitStats> {
  ...
  circuits.forEach((v, k) => { stats[k] = v })
  return stats
}
