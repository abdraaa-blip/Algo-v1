...
function switchToBackup(service: ServiceConfig) {
  ...
  const currentIndex = [service.primary, ...service.backups].indexOf(
    currentEndpoint || "",
  );
  ...
  if (process.env.NODE_ENV === "development") {
    console.log(
      `[ALGO Self-Healing] Switched ${service.name} to backup: ${newEndpoint}`,
    );
  }
}
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (activeEndpoints.get(service.name) !== service.primary) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `[ALGO Self-Healing] Restored ${service.name} to primary`,
            );
          }
        }
      } else {
...
export function createErrorRecovery(componentName: string, maxRetries = 3) {
  ...
  return {
    onError: (error: Error) => {
      retryCount++;
      console.error(
        `[ALGO Recovery] ${componentName} error (${retryCount}/${maxRetries}):`,
        error,
      );
