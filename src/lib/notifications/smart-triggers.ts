...
export interface NotificationTrigger {
  id: string
  type: 'viral' | 'streak' | 'recommendation' | 'breaking' | 'comeback' | 'achievement'
  title: string
  body: string
 ...
 */
export function saveNotificationPrefs(prefs: Partial<UserNotificationPrefs>): void {
...
function incrementSentToday(): void {
  ...
  localStorage.setItem(SENT_TODAY_KEY, JSON.stringify({ date: today, count: current + 1 }))
}

/**
 * Check if we can send a notification
 */
export function canSendNotification(type: NotificationTrigger['type']): boolean {
  const prefs = getNotificationPrefs()
 ...
 */
export function createComebackNotification(daysMissed: number): NotificationTrigger {
 ...
 */
export async function sendNotification(trigger: NotificationTrigger): Promise<boolean> {
