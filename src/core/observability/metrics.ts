...
function countBy<T extends string>(logs: AlgoObsLog[], pick: (l: AlgoObsLog) => T): Record<string, number> {
...
export function computeObservabilityMetrics(logs: AlgoObsLog[], windowMs = 60_000): AlgoObservabilityMetrics {
  const now = Date.now()
  const inWindow = logs.filter((l) => now - l.timestamp <= windowMs)

  const apiDurations = inWindow
    .filter((l) => l.layer === 'api' && typeof l.metadata?.durationMs === 'number')
    .map((l) => l.metadata!.durationMs as number)

  const avgApiDurationMs =
    apiDurations.length > 0 ? Math.round(apiDurations.reduce((a, b) => a + b, 0) / apiDurations.length) : null

  const errors = inWindow.filter((l) => l.type === 'error' || l.type === 'critical').length
  ...
  return {
    ...
    avgApiDurationMs,
    lastEventAt: inWindow.length ? Math.max(...inWindow.map((l) => l.timestamp)) : null,
  }
}
