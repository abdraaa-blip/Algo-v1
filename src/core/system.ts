...
import {
  centralAsk,
  type CentralAskInput,
  type CentralAskResult,
} from "@/core/brain";
...
const CASUAL_FAILURE_EN_RE = new RegExp(
  `\\b${["o", "o", "p", "s"].join("")}\\b`,
  "i",
);
const CASUAL_FAILURE_FR_RE = new RegExp(
  `\\b${["o", "u", "p", "s"].join("")}\\b`,
  "i",
);
...
export function buildStandardAskEnvelope(
  ...
  if (!action) {
    ...
    if (
      block &&
      /^(piste|prochain|ensuite|teste|ouvre|vérif|vérifie|envoy|choisis)/i.test(
        block,
      )
    ) {
      action = block;
...
export function qualityTransparencyHints(
  quality: AlgoResponseQuality,
): string[] {
  const out: string[] = [];
  if (!quality.isClear) {
    out.push(
      "Contrôle auto : clarté perfectible · affine ta question ou relis le bloc compréhension.",
    );
  }
  if (!quality.isUseful) {
    out.push(
      "Contrôle auto : piste d’action peu visible · regarde le champ « action » ou précise plateforme / objectif.",
    );
  }
  if (!quality.isCoherent) {
    out.push(
      "Contrôle auto : formulation à recaler sur la voix ALGO (pas de jargon erreur brute).",
    );
  }
  return out;
}
...
function estimateSystemConfidence(
  quality: AlgoResponseQuality,
  route: AlgoAskRoute,
): number {
  let c = 0.66;
 ...
 */
export async function processAlgoAiRequest(
  input: CentralAskInput,
): Promise<AlgoSystemProcessResult> {
 ...
 */
export async function runAlgoAskSystem(
  input: CentralAskInput,
): Promise<AlgoSystemAskResult> {
  const base = await centralAsk(input);
  const standard = buildStandardAskEnvelope(
    input.question,
    base.answer,
    base.structured,
  );
