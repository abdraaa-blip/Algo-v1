...
async function reverseGeocode(
  ...
  try {
    ...
    return country
      ? { code: country.code, name: getCountryName(country, "fr") }
      : null;
  } catch {
    return null;
  }
...
export function useGeolocation(): UseGeolocationReturn {
  ...
  const requestLocation = useCallback(async () => {
    ...
    if (!("geolocation" in navigator)) {
      ...
      if (ipResult) {
        setResult({
          countryCode: ipResult.code,
          countryName: ipResult.name,
          source: "ip",
        });
        setStatus("granted");
      }
  ...
  const suggestedScope: AppScope | null =
    result.countryCode && result.countryName
      ? { type: "country", code: result.countryCode, name: result.countryName }
      : null;

  const suggestedLocale = result.countryCode
    ? getLocaleForCountry(result.countryCode)
    : null;
