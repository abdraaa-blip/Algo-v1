...
export function useNetworkStatus(): NetworkStatus {
  ...
  const [status, setStatus] = useState<NetworkStatus>(() => getCurrentStatus() ?? {
    isOnline: true,
 ...
 */
export function getImageQuality(status: NetworkStatus): 'low' | 'medium' | 'high' {
  if (!status.isOnline || status.saveData) return 'low'
  if (status.isSlowConnection || (status.downlink && status.downlink < 1.5)) return 'low'
