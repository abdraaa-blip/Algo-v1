...
import type {
  ...
  ViralScore,
  TrendPrediction,
} from "@/lib/algorithm/viral-engine";
...
export function useRealTimeTrends(options: UseRealTimeTrendsOptions = {}) {
  ...
  const risingFast = trends.filter(
    (t) => t.score.tier === "S" || t.score.tier === "A",
  );
  const postNowOpportunities = trends.filter(
...
export function useRealTimeTrend(
  keyword: string,
  options: UseRealTimeTrendsOptions = {},
) {
