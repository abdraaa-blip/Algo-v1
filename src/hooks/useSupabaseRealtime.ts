      ...
      .on(
        ...
        (payload: RealtimePostgresChangesPayload<RealtimeTrendUpdate>) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setTrends(prev => 
              prev.map(t => t.id === (payload.new as RealtimeTrendUpdate).id 
        ...
        (payload: RealtimePostgresChangesPayload<RealtimeContentUpdate>) => {
          ...
          if (payload.eventType === 'INSERT') {
            setContent(prev => {
              ...
              if (options?.platform && newContent.platform !== options.platform) {
                return prev
              }
              if (options?.minViralScore && newContent.viral_score < options.minViralScore) {
                return prev
              }
            ...
            setContent(prev => 
              prev.map(c => c.id === (payload.new as RealtimeContentUpdate).id 
    ...
    channelRef.current = supabase
      ...
      .on(
        ...
        (payload: RealtimePostgresChangesPayload<RealtimeAlertUpdate>) => {
          ...
          if (options?.severities?.length && !options.severities.includes(newAlert.severity)) {
            return
          }

          setAlerts(prev => [newAlert, ...prev].slice(0, options?.limit || 50))
          if (!newAlert.read) {
