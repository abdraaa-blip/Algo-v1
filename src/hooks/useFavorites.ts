...
export function useFavorites(): UseFavoritesReturn {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        if (supabase) {
          ...
          if (user && mounted) {
            ...
            if (data && mounted) {
              ...
              setContents(getFavoriteContents().filter(c => contentIds.includes(c.id)))
              setIsLoaded(true)
              return
            }
          }
        ...
        console.warn('[v0] Supabase not available for favorites, using localStorage')
      }
  ...
  const toggle = useCallback(async (contentId: string) => {
    ...
    setContents(getFavoriteContents())
    track(isCurrentlyFav ? 'content_unsaved' : 'content_saved', { contentId })
  }, [ids, userId])
