...
export function useDataPipeline<T = NormalizedDataItem[]>(
  ...
  const effectiveRefreshInterval =
    refreshIntervalMs || config?.refreshIntervalMs || 15 * 60 * 1000;
    ...
    async (isBackground = false) => {
      ...
      const fetchPromise = (async () => {
        try {
          ...
          // Update meta
          setMeta({
            totalItems: Array.isArray(normalizedData)
          ...
          console.error(
            `[DataPipeline] Error fetching ${sourceId}:`,
            error.message,
          );
...
export function useBatchData(options: UseBatchDataOptions): BatchDataResult {
  ...
  useEffect(() => {
    ...
    const minInterval = Math.min(
      ...sources.map(
        (s) =>
          DataPipeline.sources[s.sourceId]?.refreshIntervalMs || 15 * 60 * 1000,
      ),
    );
