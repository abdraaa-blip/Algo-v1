...
export function ScopeSelector({ scope, onScopeChange }: ScopeSelectorProps) {
  ...
  const getCurrentDisplay = () => {
    ...
    if (scope.type === "region") {
      ...
      return {
        flag: region?.flag || "🌍",
        name: scope.name || region?.name.fr || "Region",
      };
    }
    if (scope.type === "country") {
      ...
      return {
        flag: country?.flag || "🏳️",
        name: scope.name || country?.name.fr || "Pays",
      };
    }
    return { flag: "🌐", name: "Global" };
  };
  ...
  const isActive = (type: string, code?: string) => {
    ...
    if (type === "region")
      return scope.type === "region" && scope.code === code;
    if (type === "country")
      return scope.type === "country" && scope.code === code;
    return false;
  };
            ...
            {availableRegions.map((region) => {
              ...
              return (
                <button
                  key={region.code}
                  onClick={() =>
                    handleSelect({ type: "region", code: region.code, name })
                  }
                  className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition-colors hover:bg-white/5 ${
            ...
            {availableCountries.map((country) => {
              ...
              return (
                <button
                  key={country.code}
                  onClick={() =>
                    handleSelect({ type: "country", code: country.code, name })
                  }
                  className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition-colors hover:bg-white/5 ${
                    active ? "text-violet-400" : "text-white/70"
                  }`}
