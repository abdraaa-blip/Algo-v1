...
export function AlgoCoreIntelligencePanel({ region }: { region: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/intelligence/core?region=${encodeURIComponent(region)}`,
        { cache: "no-store" },
      );
      ...
      setError(
        mapUserFacingApiError(
          e instanceof Error ? e.message : ALGO_UI_ERROR.message,
        ),
      );
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            <h2 className="font-bold text-sm text-white">
              ALGO Core Intelligence
            </h2>
            <p className="text-[11px] text-white/50 mt-0.5 max-w-xl">
              Analyse multi-facteurs, simulations (viral / stagnation / rejet),
              motifs, priorités et fiabilité · à partir des tendances et actus
              live pour la région sélectionnée. Indicateurs seulement.
            </p>
            ...
            {report && (
              <p className="text-[10px] text-violet-300/80 mt-1 italic">
                &ldquo;{report.philosophy.tagline}&rdquo;
              </p>
            )}
          </div>
        </div>
      ...
      {error && (
        <div className="mb-3 p-2.5 rounded-lg bg-rose-500/10 border border-rose-500/25 text-rose-200 text-xs">
          {error}
        </div>
      )}
      ...
      {report && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="space-y-3">
            ...
            <div className="p-3 rounded-xl algo-surface">
              ...
              <ul className="space-y-1.5 text-[11px] text-white/70">
                <li>
                  <span className="text-emerald-300/90">Hausse</span> ·{" "}
                  {report.scenarioSimulations.viralBurst.simulatedScore} pts (
                  {report.scenarioSimulations.viralBurst.recommendation})
                </li>
                <li>
                  <span className="text-amber-300/90">Stagnation</span> ·{" "}
                  {report.scenarioSimulations.stagnation.simulatedScore} pts (
                  {report.scenarioSimulations.stagnation.recommendation})
                </li>
                <li>
                  <span className="text-rose-300/90">Rejet</span> ·{" "}
                  {report.scenarioSimulations.rejection.simulatedScore} pts (
            ...
            <div className="p-3 rounded-xl algo-surface">
              ...
              <p className="text-[11px] text-white/55 mb-1">
                ...
                {report.patterns.repeatedTokens.slice(0, 5).map((t) => (
                  <span
                    key={t.token}
                    className="inline-block mr-1.5 text-white/75"
                  >
                    {t.token}×{t.count}
                  </span>
                ))}
          ...
          <div className="space-y-3">
            <div className="p-3 rounded-xl algo-surface">
              ...
              <ul className="space-y-2">
                {topItems.map((it) => (
                  <li
                    key={it.id}
                    className="text-xs border-b border-[var(--color-border)] pb-2 last:border-0 last:pb-0"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <span className="text-white/85 line-clamp-2 font-medium">
                        {it.title}
                      </span>
                      <span
                        className={cn(
                          "shrink-0 text-[9px] uppercase font-bold px-1.5 py-0.5 rounded",
                          it.priority.bucket === "focus" &&
                            "bg-emerald-500/20 text-emerald-300",
                          it.priority.bucket === "watch" &&
                            "bg-amber-500/20 text-amber-300",
                          it.priority.bucket === "noise" &&
                    ...
                    <p className="text-[10px] text-white/45 mt-1">
                      {it.intelligence.viralScore.label} · {it.type} · rang{" "}
                      {it.priority.rankScore.toFixed(1)} · fiab.{" "}
            ...
            <div className="p-3 rounded-xl algo-surface">
              <p className="text-[10px] uppercase tracking-wider text-white/40 mb-2">
                Auto-ajustement (suggestions)
              </p>
              <p className="text-[11px] text-violet-200/70 mb-2">
                {report.autoOptimization.confidenceHint}
              </p>
            ...
            <div className="p-3 rounded-xl algo-surface">
              ...
              <p className="text-[11px] text-white/65">
                Précoces : {report.virality.earlySignalsCount} · phases fortes :{" "}
                {report.virality.risingOrPeakCount}
              </p>
              <p className="text-[10px] text-white/45 mt-2">
                {report.virality.topActionLabels.slice(0, 4).join(" · ") || "–"}
              </p>
              <p className="text-[10px] text-white/35 mt-2">
                {report.userLayer.notes.join(" ")}
              </p>
            </div>
          </div>
        </div>
