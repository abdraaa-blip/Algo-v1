  ...
  useEffect(() => {
    ...
    const initServices = async () => {
      try {
        ...
        if (orchestrator.status === 'fulfilled' && orchestrator.value.AlgoOrchestrator?.start) {
          orchestrator.value.AlgoOrchestrator.start()
        }
        if (guard.status === 'fulfilled' && guard.value.AlgoCoherenceGuard?.start) {
          guard.value.AlgoCoherenceGuard.start()
        }
        if (engine.status === 'fulfilled' && engine.value.AlgoScoreEngine?.start) {
