...
function stableMetricFromKeyword(keyword: string, min: number, range: number): number {
  let hash = 0
...
export function TrendingTicker({ className }: TrendingTickerProps) {
  ...
  const trends = useMemo<TrendingItem[]>(() => {
    ...
    return rawTrends.slice(0, 20).map((t: { title?: string; name?: string; keyword?: string; score?: number }) => ({
      keyword: t.title || t.name || t.keyword || 'Trending',
      volume: stableMetricFromKeyword(t.title || t.name || t.keyword || 'Trending', 10_000, 500_000),
      change: stableMetricFromKeyword(t.title || t.name || t.keyword || 'Trending', 10, 200)
    }))
  }, [rawTrends])
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-20 flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-amber-500/20 rounded-full">
        ...
        <span className="text-[8px] sm:text-[10px] font-bold text-amber-300 tracking-wider">TENDANCES</span>
      </div>
