      ...
      try {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          regionIdxRef.current =
            (regionIdxRef.current + 1) % NEWS_REGIONS.length;
        }
      } catch (e) {
      ...
      <section className="max-w-7xl mx-auto px-3 sm:px-4 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="aspect-video bg-[var(--color-card)] rounded-xl animate-pulse"
            />
          ))}
        </div>
      </section>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="relative">
            <Radio
              size={18}
              className={isLive ? "text-amber-400" : "text-zinc-400"}
            />
            <span
              className={cn(
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[10px] text-white/60">
                {typeof currentFeatured.source === "string"
                  ? currentFeatured.source
                  : currentFeatured.source.name}
              </span>
        ...
        <div className="flex flex-col gap-3">
          {sideNews.map((item, i) => (
            ...
            >
              ...
              <ExternalLink
                size={14}
                className="text-[var(--color-text-muted)] group-hover:text-violet-400 flex-shrink-0"
              />
            </a>
          ))}
