...
function estimateSaturation(views: number, growthRate: number): { level: 'low' | 'medium' | 'high' | 'saturated'; label: string; color: string } {
  const saturationScore = (views / 1000000) * (100 / Math.max(growthRate, 10))
  if (saturationScore < 0.5) return { level: 'low', label: 'Opportunite', color: '#00FFB2' }
  if (saturationScore < 1.5) return { level: 'medium', label: 'Bon timing', color: '#00D1FF' }
  if (saturationScore < 3) return { level: 'high', label: 'Agis vite', color: '#FFC107' }
...
function getContentHref(content: { id: string; platform?: string }): string {
  ...
  if (id.startsWith('youtube-') || id.startsWith('tmdb-') || id.startsWith('lastfm-') || id.startsWith('news-')) {
    return `/content/${id}`
  }
...
export function ContentCard({ content, onClick }: ContentCardProps) {
  ...
  const blabel = isExploding 
    ...
    : content.growthTrend === 'down' 
      ? 'En declin' 
      : btype === 'Viral' ? 'Viral'
      : btype === 'Early' ? 'Signal precoce'
      : btype === 'Breaking' ? 'Breaking'
      : 'Tendance'
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{ 
          paddingBottom: '56.25%',
          background: 'linear-gradient(135deg, rgba(123,97,255,0.12), rgba(0,209,255,0.06))',
      ...
      <div className="p-3 flex flex-col gap-2">
            ...
            {(() => {
              ...
              return (
                <div 
                  ...
                  style={{ 
                    background: isUrgent ? 'rgba(255,77,109,0.15)' : 'rgba(255,255,255,0.05)',
        ...
        >
          {saturation.level === 'low' && <TrendingUp size={10} style={{ color: saturation.color }} />}
          {saturation.level === 'medium' && <Zap size={10} style={{ color: saturation.color }} />}
          {saturation.level === 'high' && <Clock size={10} style={{ color: saturation.color }} />}
          {saturation.level === 'saturated' && <AlertTriangle size={10} style={{ color: saturation.color }} />}
        ...
        >
          {content.views && (
            <span>{formatViews(content.views)} vues</span>
          )}
          {content.category && (
        ...
        <button
          onClick={(e) => {
            ...
            router.push(`/creator-mode?trend=${encodeURIComponent(content.title)}`)
          }}
          className="w-full mt-1 py-2 rounded-lg text-[11px] font-bold transition-all hover:scale-[1.02] active:scale-[0.98]"
          style={{
            background: 'linear-gradient(135deg, rgba(123,97,255,0.2), rgba(0,209,255,0.1))',
