...
export function PastPredictions({ className = '', compact = false }: PastPredictionsProps) {
  ...
  useEffect(() => {
    ...
    const fetchSignals = async () => {
      setLoading(true)
      try {
        const res = await fetch('/api/viral-content?country=US&limit=8', { cache: 'no-store' })
        ...
        const mapped: Prediction[] = content
          ...
          .map((item: Record<string, unknown>, idx: number) => {
            const publishedAt = String(item.publishedAt || new Date().toISOString())
            const sourceName = String(item.source || item.platform || 'unknown')
            const viewsRaw = Number((item.metrics as { views?: number } | undefined)?.views || 0)
            ...
            return {
              ...
              actualViews: formatViews(Number.isFinite(viewsRaw) ? viewsRaw : 0),
  ...
  const successRate = Math.round((predictions.filter((p) => p.success).length / predictions.length) * 100)
      ...
      <div className={`${className}`}>
        ...
        >
          ...
          <span className="text-emerald-300">
            <span className="font-bold">{successRate}%</span> de signaux detectes ont confirmee une traction
          </span>
        </div>
      </div>
    )
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-white/90">Predictions recentes (reelles)</h3>
            <p className="text-[10px] text-white/40">Basees sur les flux detectes et horodates</p>
          </div>
        </div>
        <div 
        ...
        {predictions.map((_, i) => (
          <button
            ...
            className={`w-1.5 h-1.5 rounded-full transition-all ${
              i === currentIndex ? 'w-4 bg-violet-400' : 'bg-white/20 hover:bg-white/30'
            }`}
          />
        ))}
      </div>
