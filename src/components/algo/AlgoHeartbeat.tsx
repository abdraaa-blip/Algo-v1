  ...
  if (!mounted) {
    return (
      <div className="flex items-center">
        <span className="text-xl font-black text-white tracking-tight">ALGO</span>
      </div>
    )
  }
    ...
    <div className="relative flex items-center overflow-hidden z-10">
      ...
      <div 
        ...
        style={{
          ...
          filter: 'blur(8px)',
          animation: phase === 'pulse' ? 'pulse-aura 1.5s ease-in-out infinite' : 'none'
        }}
      />
      ...
      <span 
        ...
        style={{
          ...
          textShadow: phase === 'pulse' 
            ? '0 0 20px rgba(123,97,255,0.8), 0 0 40px rgba(123,97,255,0.4)' 
            : 'none',
          animation: phase === 'pulse' ? 'heartbeat-text 1.5s ease-in-out infinite' : 'none'
        }}
      >
        ALGO
      </span>
      ...
      <style jsx>{`
        @keyframes pulse-aura {
          ...
          42% { transform: scale(1.2); opacity: 0.6; }
        ...
        @keyframes particle-fly {
          ...
          100% { opacity: 0; transform: translate(var(--x, 30px), var(--y, -20px)) scale(0); }
        }
      `}</style>
    </div>
  )
