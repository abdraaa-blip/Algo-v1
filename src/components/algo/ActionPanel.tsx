...
import { Zap, Clock, Target, Sparkles, ChevronRight, AlertTriangle } from 'lucide-react'
...
const EMOTION_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  ...
  tristesse: { bg: 'rgba(100,149,237,0.15)', text: '#6495ed', label: 'Tristesse' },
  indignation: { bg: 'rgba(255,99,71,0.15)', text: '#ff6347', label: 'Indignation' },
}

const CONFIDENCE_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  faible: { bg: 'rgba(255,193,7,0.12)', text: 'rgba(255,193,7,0.8)', label: 'Confiance faible' },
  moyen: { bg: 'rgba(0,209,178,0.12)', text: 'rgba(0,209,178,0.8)', label: 'Confiance moyenne' },
  eleve: { bg: 'rgba(123,97,255,0.15)', text: '#7b61ff', label: 'Confiance elevee' },
}
...
export function ActionPanel({
  ...
  const isUrgent = viralScore >= 80 || timeRemaining?.includes('h') && parseInt(timeRemaining) <= 6
  const isCritical = viralScore >= 90 || timeRemaining?.includes('h') && parseInt(timeRemaining) <= 2
      ...
      <div className="p-4 border-b border-white/[0.06]">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-white/90">Ce que tu dois faire</h3>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <span className="text-2xl font-black text-violet-400">{viralScore}</span>
      ...
      <div className="p-4">
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-xs font-bold text-white/80 mb-2">Action immediate:</p>
              <ul className="space-y-1.5 text-[11px] text-white/60">
                ...
                {hookSuggestion && (
                  <li className="flex items-start gap-2">
                    <ChevronRight size={12} className="text-violet-400 mt-0.5 flex-shrink-0" />
                    <span>Hook: <em className="text-violet-300">&quot;{hookSuggestion}&quot;</em></span>
                  </li>
                )}
                {postBefore && (
                  <li className="flex items-start gap-2">
                    <ChevronRight size={12} className="text-amber-400 mt-0.5 flex-shrink-0" />
                    <span className="text-amber-300/80">Poster avant {postBefore}</span>
                  </li>
                )}
              </ul>
            </div>
        ...
        {isExpanded && (
          <div className="mt-4 pt-4 border-t border-white/[0.06] space-y-3">
            <div>
              <p className="text-[10px] text-white/40 uppercase tracking-wider mb-1">Pourquoi ca marche</p>
              <p className="text-[11px] text-white/60 leading-relaxed">
                Ce contenu declenche une emotion forte ({emotionStyle.label.toLowerCase()}) qui favorise le partage. 
                Le format {format} est optimal pour ce type de sujet sur {idealPlatform[0]}.
              </p>
            </div>
            <div>
              <p className="text-[10px] text-white/40 uppercase tracking-wider mb-1">Potentiel monetisation</p>
              <p className="text-[11px] text-white/60">
                {viralScore >= 80 ? 'Oui - Fort potentiel de revenus publicitaires' : 'Moyen - Bon pour la visibilite'}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-white/40 uppercase tracking-wider mb-1">Duree estimee du buzz</p>
              <p className="text-[11px] text-white/60">
                {viralScore >= 85 ? '48-72h' : viralScore >= 70 ? '24-48h' : '12-24h'}
              </p>
            </div>
          </div>
        )}
