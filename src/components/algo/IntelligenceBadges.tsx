...
export function ViralScoreRingEnhanced({
  score,
  phase,
  size = 48,
  showPhase = false,
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        {showPhase && (
          <span
            className={cn("text-[8px] uppercase tracking-wider", config.color)}
          >
            {phase}
          </span>
        )}
...
const EMOTION_CONFIG: Record<
  EmotionType,
  { color: string; bg: string; icon: typeof Heart; label: string }
> = {
  curiosity: {
    color: "text-cyan-400",
    bg: "bg-cyan-500/20",
  ...
  return (
    ...
    >
      <ViralScoreBadge
        ...
        showLabel={!compact}
      />
      <EmotionTag emotion={emotion} size={compact ? "sm" : "md"} />
