...
import { X, ExternalLink, Clock, Target, Sparkles, ChevronRight } from 'lucide-react'
...
export function NewsDetailModal({ article, onClose, onGenerateIdea }: NewsDetailModalProps) {
  const [imageError, setImageError] = useState(false)
  const [renderTimestamp] = useState(() => Date.now())
  const randomCreatorPercent = 5 + (article.id.length % 15)
  
  // Calculate scores based on recency
  const hoursAgo = (renderTimestamp - new Date(article.publishedAt).getTime()) / 3600000
  const viralScore = Math.max(10, Math.min(100, Math.round(90 - hoursAgo * 5)))
  const timeRemaining = viralScore >= 85 ? '6h' : viralScore >= 70 ? '12h' : viralScore >= 50 ? '24h' : '48h'
  const isUrgent = viralScore >= 75
  
  const sourceName = typeof article.source === 'string' ? article.source : article.source?.name || 'News'
  const hasImage = article.urlToImage && !imageError && !article.urlToImage.includes('null')
  
  // Determine emotion based on keywords (simplified)
  const title = article.title.toLowerCase()
  const emotion = title.includes('scandal') || title.includes('choc') || title.includes('colere') ? 'colere'
    : title.includes('victoire') || title.includes('succes') ? 'joie'
    : title.includes('mort') || title.includes('deces') ? 'tristesse'
    : 'surprise'
    ...
    >
      ...
      >
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Title */}
          <h2 className="text-xl font-bold text-white/90 leading-tight">{article.title}</h2>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <span className="text-3xl font-black text-violet-400">{viralScore}</span>
                <div>
                  <p className="text-[10px] text-white/40 uppercase">Score viral</p>
                  <p className="text-xs text-white/60">Confiance {viralScore >= 70 ? 'elevee' : 'moyenne'}</p>
                </div>
              </div>
              <div 
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold"
                style={{ 
                  background: isUrgent ? 'rgba(255,77,109,0.2)' : 'rgba(255,193,7,0.15)',
            ...
            <div 
              ...
              style={{ background: 'rgba(123,97,255,0.1)', border: '1px solid rgba(123,97,255,0.15)' }}
            >
              <div className="flex items-start gap-2">
                <Target size={14} className="text-violet-400 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-[11px] font-bold text-white/70 mb-1">Action immediate:</p>
