...
export function TrendIntelligenceCard({
  ...
  return (
    <article
      ...
      style={{
        background:
          "linear-gradient(145deg, rgba(20,20,30,0.98), rgba(10,10,15,0.99))",
    ...
    >
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 text-xs text-white/50">
              ...
              <span
                className={
                  trend.growthRate > 0 ? "text-emerald-400" : "text-red-400"
                }
              >
                {trend.growthRate > 0 ? "+" : ""}
                {trend.growthRate}%
          ...
          <div className="flex-1 flex items-center gap-2">
            <div className="flex-1 h-1.5 rounded-full bg-white/10 overflow-hidden">
              <div
                ...
                style={{
                  width: saturation.width,
                  background: saturation.color,
                }}
              />
            </div>
            <span
              className="text-[10px] font-bold"
              style={{ color: saturation.color }}
            >
              {saturation.label}
            </span>
          </div>
      ...
      <div className="px-4 pb-4">
        ...
        {activeView === "psycho" && (
          <PsychoView
            analysis={trend.analysis.psycho}
            semantics={trend.semantics}
          />
        )}
        {activeView === "socio" && (
        ...
        {activeView === "algorithm" && (
          <AlgorithmView
            analysis={trend.analysis.algorithm}
        ...
        {activeView === "content" && (
          <ContentView
            analysis={trend.analysis.content}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="p-2 rounded-lg bg-white/5 text-center">
          <div className="text-lg font-bold text-violet-400">
            {trend.analysis.algorithm.engagementPotential}%
          </div>
          <div className="text-[9px] text-white/40 uppercase">Engagement</div>
        </div>
        <div className="p-2 rounded-lg bg-white/5 text-center">
          <div className="text-lg font-bold text-cyan-400">
            {trend.analysis.algorithm.viralProbability}%
          </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="p-2 rounded-lg bg-white/5">
          ...
          <div className="text-xs text-white/80">
            {trend.heatmap.platforms[0]?.name || "N/A"}
          </div>
        </div>
        <div className="p-2 rounded-lg bg-white/5">
          <div className="text-[9px] text-white/40 uppercase mb-1">
          ...
          <div className="text-xs text-white/80">
            {trend.heatmap.audiences[0]?.name || "N/A"}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-3">
      <div className="p-3 rounded-xl bg-purple-500/10 border border-purple-500/20">
        ...
        <div className="flex flex-wrap gap-1.5">
          {analysis.emotionalDrivers.map((driver, i) => (
            <span
              key={i}
              className="px-2 py-1 rounded-lg bg-white/5 text-xs text-white/70"
            >
              {driver}
            </span>
          ))}
      ...
      <div className="p-3 rounded-xl bg-white/5">
        ...
        <div className="text-xs text-white/80">
          {semantics.storytellingStructure}
        </div>
      </div>
          ...
          <div className="absolute inset-0 flex">
            {Object.entries(saturationConfig).map(([key, config]) => (
              <div
                ...
                style={{
                  background: key === saturation ? config.color : "transparent",
                }}
              />
            ))}
          </div>
      ...
      <div className="p-3 rounded-xl bg-orange-500/10 border border-orange-500/20">
        ...
        <div className="flex flex-wrap gap-1.5">
          {analysis.bestFormats.map((format, i) => (
            <span
              key={i}
              className="px-2 py-1 rounded-lg bg-white/10 text-xs text-white/80 font-medium"
            >
              {format}
            </span>
          ))}
        ...
        <div className="mt-2 text-xs text-white/50">
          Duree ideale:{" "}
          <span className="text-white/80">{analysis.recommendedLength}</span>
        </div>
      </div>
      ...
      <div className="p-3 rounded-xl bg-white/5">
        ...
        <div className="flex flex-wrap gap-1.5">
          {analysis.avoidList.map((item, i) => (
            <span
              key={i}
              className="px-2 py-1 rounded-lg bg-red-500/10 text-xs text-red-400"
            >
              ✗ {item}
            </span>
          ))}
...
export function generateTrendIntelligence(basicTrend: {
  ...
  const primaryEmotion =
    emotionMap[basicTrend.emotion || "curiosity"] || "curiosity";
  ...
  if (saturation === "saturated" || saturation === "declining")
    badges.push("saturated");
  if (basicTrend.category === "Tech" || basicTrend.category === "Finance")
    badges.push("niche_growing");
  ...
  return {
    ...
    analysis: {
      psycho: {
        ...
        emotionalHook:
          basicTrend.explanation || "Tendance qui capte l'attention",
      },
      socio: {
      ...
      algorithm: {
        ...
        engagementPotential: Math.min(
          100,
          Math.floor(score * 0.8 + growthRate * 0.1),
        ),
        viralProbability: Math.min(
          100,
          Math.floor(growthRate * 0.4 + score * 0.3),
        ),
        peakPrediction:
          saturation === "early"
            ? "Dans 2-4 jours"
        ...
        urgency:
          saturation === "peak"
            ...
            : saturation === "rising"
              ...
              : "flexible",
        angle:
          basicTrend.contentIdeas?.[0] || `Mon analyse de "${basicTrend.name}"`,
      },
      {
        type: "reaction",
    ...
    semantics: {
      whyItWorks:
        basicTrend.explanation ||
        "Cette tendance capte l'attention par son actualité et son potentiel de partage.",
