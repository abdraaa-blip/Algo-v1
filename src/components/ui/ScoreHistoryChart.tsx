...
const COLORS = {
  violet: { stroke: '#7B61FF', fill: 'rgba(123, 97, 255, 0.1)', gradient: 'from-violet-500/20' },
  emerald: { stroke: '#00FFB2', fill: 'rgba(0, 255, 178, 0.1)', gradient: 'from-emerald-500/20' },
  amber: { stroke: '#FFB800', fill: 'rgba(255, 184, 0, 0.1)', gradient: 'from-amber-500/20' },
  red: { stroke: '#FF4D4D', fill: 'rgba(255, 77, 77, 0.1)', gradient: 'from-red-500/20' },
}
...
export function ScoreHistoryChart({
  ...
  const { path, area, stats, points } = useMemo(() => {
    ...
    return {
      path: pathD,
      area: areaD,
      stats: { min: minScore, max: maxScore, change, changePercent, trend, current: lastScore },
      points: pointsData
    }
  }, [data, height])
  ...
  if (!data || data.length < 2) {
    return (
      <div className={cn('flex items-center justify-center text-white/30 text-sm', className)} style={{ height }}>
        Not enough data
      </div>
    )
  }
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <div className={cn(
              ...
              {stats.trend !== 'stable' && (
                <span>{stats.trend === 'up' ? '+' : ''}{stats.changePercent}%</span>
              )}
            </div>
          </div>
