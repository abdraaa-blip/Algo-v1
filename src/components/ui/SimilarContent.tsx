...
export function SimilarContent({ items, title, type = 'film' }: SimilarContentProps) {
  if (!items || items.length === 0) return null

  const sectionTitle = title || (type === 'film' ? 'Contenus similaires en tendance' : 'Videos similaires')
    ...
    <section className="mt-6">
      ...
      <div className="flex gap-2.5 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
        {items.map((item) => (
          ...
          >
            <div className={`${type === 'film' ? 'aspect-[2/3]' : 'aspect-video'} overflow-hidden bg-white/[0.06] relative`}>
              ...
              {item.badge && (
                <div className="absolute top-1.5 left-1.5">
                  <Badge type={item.badge as 'Viral' | 'Trend' | 'Early'} label={item.badge} size="sm" />
                </div>
              )}
            </div>
            <div className="p-2">
              ...
              <div className="flex items-center gap-1.5 mt-1 text-[9px] text-white/35">
                ...
                {item.channelName && <span className="line-clamp-1">{item.channelName}</span>}
              </div>
            </div>
          </Link>
        ))}
