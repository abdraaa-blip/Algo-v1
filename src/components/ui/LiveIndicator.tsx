...
export function LiveIndicator({ 
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 backdrop-blur-sm border border-white/10 ${className}`}>
      <div className="flex items-center gap-1.5">
        <span className={`w-2 h-2 rounded-full ${config.color} ${source === 'live' ? 'animate-pulse' : ''}`} />
...
export function LiveBadge({ 
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-bold rounded border ${colors[source]} ${className}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${source === 'live' ? 'bg-amber-400' : source === 'cached' ? 'bg-zinc-400' : 'bg-orange-400'}`} />
      {labels[source]}
    </span>
  )
}
