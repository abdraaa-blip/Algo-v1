...
import { X, Flame, TrendingUp, Bell, Zap, Star } from "lucide-react";
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-white/60 mb-3">
                  Reçois une notification quand un buzz explose ou qu&apos;une
                  trend correspond a tes interets.
                </p>
                <button
    ...
    <div className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
      <div className="flex items-center gap-3">
        ...
        >
          <Icon
            size={16}
            className={enabled ? "text-violet-400" : "text-white/40"}
          />
        </div>
        <div>
...
export function useBuzzAlerts(
  ...
  useEffect(() => {
    ...
    const buzzWorthy = trends.filter(
      (t) =>
        t.score.tier === "S" &&
    ...
    if (buzzWorthy.length > 0) {
      ...
      buzzWorthy.forEach((trend) => {
        const notification = createBuzzAlert(
          trend.keyword,
          trend.score.overall,
          trend.avgVelocity,
        );
