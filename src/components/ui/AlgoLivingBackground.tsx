    ...
    const draw = (time: number) => {
      ...
      for (const line of linesRef.current) {
        ...
        for (let x = -6; x <= w + 6; x += 6) {
          const y =
            line.yBase +
            Math.sin(x * line.freq + time * 0.001 * line.speed + line.phase) *
              line.amp +
            Math.sin(x * (line.freq * 1.8) - time * 0.0008 * line.speed) *
              (line.amp * 0.32) +
      ...
      if (sweep.active) {
        ...
        const done =
          sweep.direction === 1
            ? sweep.progress > 1 + sweep.width
            : sweep.progress < -sweep.width;
        const sweepX = w * sweep.progress;

        const grad = ctx.createLinearGradient(
          sweepX - w * sweep.width,
          0,
          sweepX + w * sweep.width,
          0,
        );
