...
export function RelatedContent({
  ...
  const keywordsString =
    keywords.length > 0 ? keywords.join(",") : category || "";
    ...
    const fetchRelated = async () => {
      ...
      try {
        const response = await fetch(
          `/api/content/related?id=${encodeURIComponent(resolvedContentId)}&type=${encodeURIComponent(resolvedContentType)}&keywords=${encodeURIComponent(keywordsString)}&limit=${limit}`,
        );
        ...
        setItems(
          generateMockRelated(keywordsString.split(",").filter(Boolean), limit),
        );
      } finally {
        setIsLoading(false);
        setHasFetched(true);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {items.map((item) => (
          ...
          >
            ...
            <ExternalLink
              size={14}
              className="text-[var(--color-text-muted)] group-hover:text-[var(--color-text-tertiary)] shrink-0 transition-colors"
            />
          </Link>
        ))}
      </div>
