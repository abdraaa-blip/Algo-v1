...
import { 
  ImageOff, Play, Video, Music, Newspaper, Film,
  type LucideIcon
} from 'lucide-react'

// Platform colors for gradient fallbacks
const PLATFORM_COLORS: Record<string, { from: string; to: string; icon: LucideIcon }> = {
...
export function ImageWithFallback({
  ...
  const platformConfig = PLATFORM_COLORS[platform.toLowerCase()] || PLATFORM_COLORS.default
  ...
  const renderFallback = () => {
    ...
    return (
      ...
      >
        ...
        >
          <PlatformIcon size={fill ? 32 : Math.min(width || 24, 32)} className="text-white/60" />
        </div>
        {platform !== 'default' && (
  ...
  const knownDomains = ['youtube', 'unsplash', 'wikimedia', 'ui-avatars', 'lastfm', 'themoviedb', 'tmdb']
  const isExternalUnknown = src.startsWith('http') && !knownDomains.some(d => src.includes(d))
...
export function VideoThumbnail({
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-xl', `aspect-[${aspectRatio}]`, className)}>
      <ImageWithFallback
        src={src}
        alt={alt}
        fill
