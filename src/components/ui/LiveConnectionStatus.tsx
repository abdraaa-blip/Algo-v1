      ...
      <div className={cn('flex items-center gap-2', className)}>
        <div className="flex items-center gap-1.5">
          {isLoading ? (
            <>
              <span className="size-2 rounded-full bg-amber-500 animate-pulse" />
              <span className="text-[10px] text-amber-400 font-medium">Connexion...</span>
            </>
          ) : isLive ? (
            <>
              <span className="size-2 rounded-full bg-amber-500" />
              <span className="text-[10px] text-amber-400 font-medium">Connecte</span>
              {sources.length > 0 && (
        ...
        {onRefresh && (
          <button
            ...
            title="Actualiser"
          >
            <RefreshCw size={12} className={cn('text-white/40', isLoading && 'animate-spin')} />
          </button>
        )}
      ...
      >
        <div className="flex items-center gap-2">
          {isLoading ? (
            <>
              ...
              <span className="text-xs text-amber-400 font-medium">Connexion en cours...</span>
            </>
          ) : isLive ? (
            <>
              <span className="size-2.5 rounded-full bg-amber-500" />
              <span className="text-xs text-amber-400 font-medium">Sources actives</span>
            </>
          ) : (
            <>
              <WifiOff size={12} className="text-red-400" />
              <span className="text-xs text-red-400 font-medium">Connexion perdue</span>
            </>
          )}
          ...
          {sources.length > 0 && (
            <span className={cn(
              ...
              hasErrors ? 'bg-amber-500/20 text-amber-300' : 'bg-white/10 text-white/50'
            )}>
        ...
        <div className="flex items-center gap-2">
          ...
          {onRefresh && (
            ...
            >
              <RefreshCw size={14} className={cn('text-white/50', isLoading && 'animate-spin')} />
            </button>
          )}
        </div>
      </button>
        ...
        <div className="border-t border-white/5 p-3 space-y-2">
          {sources.map((source, i) => (
            <div key={i} className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {source.status === 'connected' && <Check size={12} className="text-emerald-400" />}
                {source.status === 'loading' && <RefreshCw size={12} className="text-amber-400 animate-spin" />}
                {source.status === 'error' && <AlertCircle size={12} className="text-red-400" />}
                {source.status === 'offline' && <WifiOff size={10} className="text-white/30" />}
                <span className={cn(
                  'text-xs font-medium',
                  source.status === 'connected' ? 'text-white/70' :
