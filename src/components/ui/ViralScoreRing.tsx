...
const sizeCfg: Record<
  Size,
  { ring: number; stroke: number; fontSize: string; fontWeight: number }
> = {
...
export function ViralScoreRing({
  ...
  const config =
    typeof size === "number" && !Number.isNaN(size)
      ...
      : sizeCfg[(size as Size) in sizeCfg ? (size as Size) : "md"] ||
        sizeCfg.md;
  ...
  const safeStroke =
    typeof stroke === "number" && !Number.isNaN(stroke) ? stroke : 3.5;
  ...
  const safeValue =
    typeof actualValue === "number" && !Number.isNaN(actualValue)
      ? actualValue
      : 0;
