...
import { 
  ...
  BadgeCheck, Music, Mic, Laugh, Camera, Gamepad2, Trophy, Film,
  Globe, Video, Play, ExternalLink, Sparkles, Flame, Zap, Eye,
  type LucideIcon
} from 'lucide-react'
...
interface RisingStar {
  ...
  trending_content: { title: string; type: string; url: string; views: number }[] | string[]
...
const momentumConfig: Record<string, { label: string; icon: LucideIcon; cls: string }> = {
  exploding: { label: 'EXPLOSION', icon: Flame, cls: 'bg-orange-500/15 text-orange-400 border-orange-500/30 animate-pulse' },
  rising: { label: 'En hausse', icon: TrendingUp, cls: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/25' },
  peaked: { label: 'Au sommet', icon: Zap, cls: 'bg-violet-500/15 text-violet-400 border-violet-500/25' },
  steady: { label: 'Stable', icon: Sparkles, cls: 'bg-cyan-500/15 text-cyan-400 border-cyan-500/25' },
  cooling: { label: 'En baisse', icon: TrendingDown, cls: 'bg-white/5 text-white/40 border-white/10' },
}
...
export function StarCard({ star, rank, variant = 'compact', className }: StarCardProps) {
  ...
  if (variant === 'featured') {
    return (
      ...
      >
        ...
        <div className={cn('p-5', star.cover_url && '-mt-10 relative', star.is_future_star && !star.cover_url && 'pt-8')}>
          {/* Rank badge */}
          {rank && rank <= 3 && (
            <div className={cn(
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="text-white font-bold text-lg truncate">{star.name}</h3>
                {star.verified && <BadgeCheck size={16} className="text-violet-400 shrink-0" />}
              </div>
          ...
          {/* Bio */}
          {star.bio && (
            <p className="text-white/60 text-xs mt-3 line-clamp-2">{star.bio}</p>
          )}
          ...
          {star.algo_prediction && (
            <div className="mt-3 p-2 rounded-lg bg-violet-500/10 border border-violet-500/20">
              ...
              <p className="text-white/70 text-xs mt-1">{star.algo_prediction}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3 mt-4">
            <div className="text-center p-2 rounded-xl bg-white/[0.03]">
              <p className="text-white font-bold text-sm">{formatNumber(star.total_followers)}</p>
              <p className="text-white/40 text-[10px]">Followers</p>
            </div>
            <div className="text-center p-2 rounded-xl bg-white/[0.03]">
              <p className={cn(
                'font-bold text-sm',
                star.follower_growth_24h > 0 ? 'text-emerald-400' : 'text-white/40',
              )}>
            ...
            <div className="text-center p-2 rounded-xl bg-white/[0.03]">
              <p className="text-white font-bold text-sm">{formatNumber(star.mention_count_24h)}</p>
          ...
          {star.best_video && (
            <div className="mt-4">
              <p className="text-[10px] text-white/40 uppercase tracking-wider mb-2">Meilleure video</p>
              <a 
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent">
                  <p className="text-white text-xs font-medium truncate">{star.best_video.title}</p>
          ...
          <div className="flex items-center gap-2 mt-4">
            {star.top_platforms.slice(0, 4).map((p, i) => {
              ...
              const colorClass = platformColors[p.platform] || 'hover:bg-white/10'
              return (
                <a
                  key={i}
                  href={p.url || '#'}
                ...
                >
                  ...
                  <span className="text-white/70 text-[10px] font-medium">{formatNumber(p.followers)}</span>
                  <span className={cn(
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="text-white font-semibold text-sm truncate">{star.name}</span>
          {star.verified && <BadgeCheck size={12} className="text-violet-400 shrink-0" />}
          {star.is_future_star && (
            <span className="px-1.5 py-0.5 bg-orange-500/20 text-orange-400 text-[8px] font-bold rounded">FUTUR</span>
          )}
        </div>
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-white/40 text-[10px]">{formatNumber(star.total_followers)}</span>
        </div>
      </div>
      ...
      <div className={cn(
        ...
        {star.follower_growth_24h > 0 ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
        {formatGrowth(star.follower_growth_24h)}
      </div>
    </div>
  )
