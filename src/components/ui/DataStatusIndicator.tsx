...
export function DataStatusIndicator({
  ...
  useEffect(() => {
    const update = () => {
      const newStatus = calculateDataStatus(
        fetchedDate,
        hasAutoRefresh,
        refreshIntervalMs,
      );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        {displayStatus === "error" && (
          <WifiOff size={14} className={colors.text} />
        )}
        {displayStatus === "loading" && (
          <RefreshCw size={14} className={cn(colors.text, "animate-spin")} />
        )}
        {(displayStatus === "delayed" || displayStatus === "static") && (
        ...
        <div className="flex items-center gap-1.5 text-xs text-[var(--color-text-tertiary)]">
          ...
          {fetchedDate && (
            <span className="text-[var(--color-text-muted)]">
              ({formatAbsoluteTime(fetchedDate, localeCode, timeZone)})
            </span>
          )}
          {showTimeZone && (
            <span className="text-[var(--color-text-muted)]">
