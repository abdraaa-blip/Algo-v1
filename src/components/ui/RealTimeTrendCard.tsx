...
function RealTimeTrendCardComponent({
  ...
  const momentum =
    scoreVelocity > 70 ? "rising" : scoreVelocity > 40 ? "steady" : "fading";
    ...
    switch (recommendedAction) {
      ...
      case "wait":
        return {
          icon: Clock,
          text: "WAIT",
          color: "text-[#00D1FF] bg-[#00D1FF]/20",
        };
      case "too_late":
        return {
          icon: AlertTriangle,
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/[0.02] rounded-xl p-2">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-sm font-bold text-white">
              {formatNumber(Math.round(trend.avgVelocity))}
            </span>
            {velocityDelta !== 0 && (
              <span
                className={cn(
        ...
        <div className="bg-white/[0.02] rounded-xl p-2">
          ...
          <span className="text-sm font-bold text-white">
            {formatNumber(trend.totalVolume)}
          </span>
        </div>
