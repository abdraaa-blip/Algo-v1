...
import { 
  X, Zap, TrendingUp, Clock, Target, 
  Copy, Sparkles, ChevronRight, Play, Users, Globe2,
  BarChart3, CheckCircle, Timer, Brain
} from 'lucide-react'
...
export function TrendDetailModal({ trend, onClose, onCopyHook }: TrendDetailModalProps) {
  ...
  const config = actionConfig[prediction.recommendedAction] || actionConfig.prepare
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-sm">
                <MomentumPill 
                  value={trend.velocity > 0 ? `+${trend.velocity}%` : `${trend.velocity}%`}
        ...
        <div className="p-5 space-y-6">
          ...
          <div className={cn(
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className={cn('text-xs font-bold tracking-wider', config.text)}>
                  {config.label}
                </p>
          ...
          {windows && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-white">Fenetres optimales</h3>
              </div>
              <div className="grid grid-cols-3 gap-2">
                {windows.map((window, i) => (
                  ...
                  >
                    ...
                    {i === 0 && (
                      <p className="text-[10px] text-violet-400/60 mt-1">IDEAL</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white">Decomposition du score</h3>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <ScoreBar label="Volume" value={scoreBreakdown.volume} max={100} color="violet" />
              <ScoreBar label="Velocite" value={scoreBreakdown.velocity} max={100} color="cyan" />
              <ScoreBar label="Engagement" value={scoreBreakdown.engagement} max={100} color="orange" />
              <ScoreBar label="Croissance" value={scoreBreakdown.growth} max={100} color="green" />
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white">Hooks suggeres par l&apos;IA</h3>
            </div>
              ...
              {suggestedHooks.map((hook, i) => (
                ...
                >
                  ...
                  {copiedHook === i ? (
                    ...
                    <Copy size={16} className="text-white/30 group-hover:text-white/60 shrink-0" />
                  )}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-4 gap-2 pt-4 border-t border-white/5">
            <StatBox icon={Users} label="Createurs" value={formatNumber(trend.volume)} />
            <StatBox icon={TrendingUp} label="Velocite" value={`${trend.velocity > 0 ? '+' : ''}${trend.velocity}%`} />
            <StatBox icon={Globe2} label="Region" value={trend.region || 'Global'} />
            <StatBox icon={BarChart3} label="Duree" value={prediction.duration || '2-4h'} />
          </div>
  ...
  return (
    <div className="p-2 rounded-xl bg-white/5">
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-[10px] font-bold text-white/70">{Math.round(value)}</span>
      </div>
