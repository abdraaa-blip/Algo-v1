...
import { DEFAULT_PLANET_PREFS, loadPlanetPrefs, PLANET_PREFS_EVENT } from '@/lib/ui/planet-prefs'

type PlanetVariant = 'home' | 'videos' | 'news' | 'intelligence' | 'interactive' | 'default'
...
function getVariant(pathname: string): PlanetVariant {
  ...
  if (pathname.includes('/videos') || pathname.includes('/video')) return 'videos'
  if (pathname.includes('/news')) return 'news'
  if (pathname.includes('/intelligence') || pathname.includes('/monitor')) return 'intelligence'
  if (pathname.includes('/viral-analyzer') || pathname.includes('/trends') || pathname.includes('/creator-mode')) {
    return 'interactive'
  }
  return 'default'
}
...
function getVariantTuning(variant: PlanetVariant) {
  switch (variant) {
    case 'home':
      return { baseOpacity: 0.16, planetScale: 0.16, tint: 'rgba(130,170,255,', baseDensity: 80 }
    case 'videos':
      return { baseOpacity: 0.12, planetScale: 0.14, tint: 'rgba(110,150,255,', baseDensity: 65 }
    case 'news':
      return { baseOpacity: 0.18, planetScale: 0.155, tint: 'rgba(130,200,255,', baseDensity: 90 }
    case 'intelligence':
      return { baseOpacity: 0.2, planetScale: 0.17, tint: 'rgba(140,220,255,', baseDensity: 105 }
    case 'interactive':
      return { baseOpacity: 0.19, planetScale: 0.16, tint: 'rgba(160,180,255,', baseDensity: 98 }
    default:
      return { baseOpacity: 0.13, planetScale: 0.145, tint: 'rgba(130,170,255,', baseDensity: 70 }
  }
}
...
function createParticle(w: number, h: number, cx: number, cy: number): DataParticle {
...
export function AlgoDataPlanet() {
  ...
  const signalRef = useRef<ActivitySignals>({ score: 60, activeSources: 4, alerts: 0 })
  ...
  useEffect(() => {
    ...
    const isDataDrivenPage =
      pathname.includes('/intelligence') || pathname.includes('/monitor') || pathname.includes('/viral-analyzer')

    const pullSignals = async () => {
      if (!isDataDrivenPage) {
        signalRef.current = { score: 58 + Math.random() * 12, activeSources: 4, alerts: 0 }
        return
      }
      try {
        ...
        if (cancelled) return
        const score = predJson.success && predJson.data ? predJson.data.predictedViralityScore : 60
        const activeSources = opsJson.success ? Math.max(3, Number(opsJson.resilience?.totalCircuits || 4)) : 4
        const alerts = opsJson.success && Array.isArray(opsJson.alerts) ? opsJson.alerts.length : 0
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const targetCount = Math.max(
        38,
        Math.min(140, Math.round(tuning.baseDensity * prefsRef.current.intensity))
      )
      particlesRef.current = Array.from({ length: targetCount }, () => createParticle(w, h, cx, cy))
      starsRef.current = []
    }
    resize()
    ...
    const draw = (time: number) => {
      ...
      const radius = Math.min(w, h) * tuning.planetScale * prefsRef.current.size

      // Data-linked activity target.
      const scoreFactor = Math.min(1, Math.max(0, signalRef.current.score / 100))
      ...
      const interactionBoost = scrollSpeedRef.current * 0.35 + (pointer.active ? 0.08 : 0)
      const targetActivity = Math.max(0.2, Math.min(1, scoreFactor * 0.5 + sourceFactor * 0.35 + interactionBoost - alertPenalty))
      ...
      halo.addColorStop(0, `${tuning.tint}${0.22 * tuning.baseOpacity + activityRef.current * 0.08})`)
      halo.addColorStop(1, `${tuning.tint}0)`)
      ctx.fillStyle = halo
      ctx.fillRect(cx - radius * 2.6, cy - radius * 2.6, radius * 5.2, radius * 5.2)

      // Transparent sphere.
      const planet = ctx.createRadialGradient(cx - radius * 0.35, cy - radius * 0.45, radius * 0.1, cx, cy, radius)
      planet.addColorStop(0, `${tuning.tint}${0.16 + activityRef.current * 0.06})`)
      planet.addColorStop(0.6, `${tuning.tint}${0.05 + activityRef.current * 0.04})`)
      ...
      for (let i = -2; i <= 2; i++) {
        ...
        ctx.beginPath()
        ctx.ellipse(cx, cy + yOff + wobble, radius * (0.62 - Math.abs(i) * 0.09), radius * 0.1, rot, 0, Math.PI * 2)
      ...
      const desiredCount = Math.max(
        35,
        Math.min(160, Math.round(tuning.baseDensity * (0.7 + activityRef.current * 0.85) * intMul))
      )
      while (particles.length < desiredCount) particles.push(createParticle(w, h, cx, cy))
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        if (dist < radius * 0.72 || p.x < -16 || p.x > w + 16 || p.y < -16 || p.y > h + 16) {
