...
export function CommandPalette() {
    ...
    async function fetchTrending() {
      try {
        ...
        if (data.trends) {
          setTrendingNow(
            data.trends.map(
              (t: { keyword: string; score: { overall: number } }) => ({
                keyword: t.keyword,
  ...
  const allCommands = useMemo<CommandItem[]>(
    () => [
      ...
      },
      {
        id: "nav-trends",
        title: "Tendances",
        subtitle: "Ce qui explose",
        icon: <TrendingUp size={18} />,
        action: () => navigate("/trends"),
        category: "navigation",
        keywords: ["trends", "viral"],
      },
      {
        id: "nav-stars",
        title: "Stars montantes",
        subtitle: "Artistes du moment",
        icon: <Sparkles size={18} />,
        action: () => navigate("/rising-stars"),
        category: "navigation",
        keywords: ["stars", "artistes"],
      },
      {
        id: "nav-videos",
        title: "Videos",
        subtitle: "Contenus video",
        icon: <Video size={18} />,
        action: () => navigate("/videos"),
        category: "navigation",
        keywords: ["videos", "tiktok"],
      },
      {
        id: "nav-movies",
        ...
        category: "navigation",
        keywords: ["films", "movies", "series", "netflix"],
      },
      {
        id: "nav-news",
        title: "Actualités",
        subtitle: "Les dernières infos",
        icon: <Newspaper size={18} />,
        action: () => navigate("/news"),
        category: "navigation",
        keywords: ["news", "actualités", "actualites", "données", "donnees"],
      },
      {
        id: "nav-creator",
        title: "Mode Createur",
        subtitle: "Outils pour creer",
        icon: <Mic size={18} />,
        action: () => navigate("/creator-mode"),
        category: "navigation",
        keywords: ["creator"],
      },
      {
      ...
      },
      {
        id: "nav-transparency",
        title: "Transparence",
        subtitle: "Données, scores & calibrage IA",
        icon: <Scale size={18} />,
        action: () => navigate(SITE_TRANSPARENCY_AI_CALIBRATION_HREF),
        category: "navigation",
        keywords: [
          "transparence",
          "données",
      ...
      {
        id: "nav-about",
        title: "A propos d'ALGO",
        subtitle: "Notre histoire",
        icon: <Info size={18} />,
        action: () => navigate("/about"),
        category: "navigation",
        keywords: ["about", "histoire", "story", "mission"],
      },
      {
      ...
      ...trendingNow.map((t, i) => ({
        ...
        action: () => {
          saveSearch(t.keyword);
          navigate(`/search?q=${encodeURIComponent(t.keyword)}`);
        },
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className={cn(
          "relative w-full max-w-xl mx-4 algo-modal-panel overflow-hidden",
        )}
      ...
      >
        <div className="flex items-center gap-3 px-4 py-4 border-b border-[var(--color-border)]">
          ...
          <input
            ...
            placeholder="Rechercher dans ALGO..."
            className="flex-1 bg-transparent text-[var(--color-text-primary)] text-lg placeholder:text-[var(--color-text-muted)] focus:outline-none"
          />
          <kbd className="hidden sm:flex items-center gap-1 px-2 py-1 rounded bg-[var(--color-card)] border border-[var(--color-border)] text-[11px] text-[var(--color-text-tertiary)]">
            ESC
          </kbd>
        </div>
        <div className="max-h-[50vh] overflow-y-auto py-2">
          {filteredCommands.length === 0 && (
            <div className="px-4 py-8 text-center">
              ...
              <button
                ...
                className="mt-3 inline-flex items-center gap-2 text-violet-400 hover:text-violet-300"
              >
                <Search size={16} />
          ...
          {groupedCommands.navigation.length > 0 && (
            <div className="mb-2">
              <div className="px-4 py-2 text-[11px] font-medium text-white/30 uppercase tracking-wider">
                Navigation
              </div>
          ...
          {groupedCommands.trending.length > 0 && (
            <div className="mb-2">
              <div className="px-4 py-2 text-[11px] font-medium text-white/30 uppercase tracking-wider flex items-center gap-2">
        ...
        <div className="flex items-center justify-between gap-4 px-4 py-3 border-t border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_80%,transparent)]">
          <div className="flex items-center gap-4 text-[11px] text-[var(--color-text-muted)]">
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 rounded bg-[var(--color-card)] border border-[var(--color-border)]">
                ↑
              </kbd>
              <kbd className="px-1.5 py-0.5 rounded bg-[var(--color-card)] border border-[var(--color-border)]">
                ↓
...
function CommandRow({
  ...
  return (
    <button
      className={cn(
        "w-full flex items-center gap-3 px-4 py-2.5 text-left transition-[background-color,color] duration-200",
        isSelected
          ? "bg-[var(--color-violet-muted)] text-[var(--color-text-primary)]"
    ...
    >
      ...
      {item.score && (
        <div className="shrink-0 px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-400 text-[11px] font-bold">
          {item.score}
        </div>
      )}
      {isSelected && (
        <ArrowRight size={16} className="shrink-0 text-[var(--color-violet)]" />
      )}
    </button>
  );
}
