...
const probCfg = {
  high: {
    color: tokens.colors.accent.greenSignal,
    bg: "rgba(0,255,178,0.08)",
    border: "rgba(0,255,178,0.18)",
  },
  medium: {
  ...
  low: {
    color: tokens.colors.accent.redAlert,
    ...
    >
      ...
      <div className="space-y-1.5">
        ...
        <span className="inline-block px-2 py-0.5 rounded-full bg-[rgba(0,209,255,0.10)] border border-[rgba(0,209,255,0.20)] text-sky-300 text-[10px] font-semibold">
          {labels.formatLabels?.[insight.bestFormat] ??
            insight.bestFormat.replace("_", " ")}
        </span>
      </div>
      ...
      {watchersCount != null && watchersCount > 0 && (
        <div className="flex items-center gap-1.5 text-[11px] text-white/25">
          ...
          {labels.watchers.replace(
            "{count}",
            watchersCount.toLocaleString("fr-FR"),
          )}
        </div>
      )}
    </div>
