...
import { Bell, BellOff, BellRing, Check, X, Zap, TrendingUp } from 'lucide-react'
...
export function NotificationBell({ className }: NotificationBellProps) {
  ...
  useEffect(() => {
    ...
    if (stored) {
      const data = JSON.parse(stored) as { subscribed?: boolean; settings?: typeof settings }
          ...
          <div className="absolute right-0 top-full mt-2 w-72 z-50 rounded-2xl bg-[var(--color-bg-elevated)] border border-[var(--color-border)] shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-[var(--color-border)]">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-bold text-[var(--color-text-primary)] text-sm">Notifications Push</h3>
                <button
                  onClick={() => setShowMenu(false)}
                ...
                <div className="flex items-center gap-2">
                  {permission === 'denied' ? (
                    ...
                    <>
                      ...
                      <span className="text-xs text-[var(--color-text-secondary)]">Désactivées</span>
                    </>
                  )}
                </div>
...
function NotifOption({ icon: Icon, label, description, enabled, onToggle, color }: NotifOptionProps) {
  const colorClasses = {
