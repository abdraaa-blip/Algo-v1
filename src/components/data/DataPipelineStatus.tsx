...
const STATUS_STYLES: Record<DataStatus, { bg: string; text: string; dot: string }> = {
  active: {
    bg: 'bg-amber-500/10',
    text: 'text-amber-400',
    dot: 'bg-amber-400',
...
export function DataPipelineStatus({
  ...
  const status: DataStatus = statusOverride || (
    fetchedAt 
      ...
      <div className={cn(
        ...
        {onRefresh && (
          <button
            ...
            title="Actualiser"
          >
            <RefreshCw size={14} className={cn(styles.text, isRefreshing && 'animate-spin')} />
          </button>
        )}
      </div>
    )
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            {status === 'active' ? (
              ...
              <RefreshCw size={16} className={cn(styles.text, 'animate-spin')} />
            ) : (
...
export function MultiSourceStatus({ sources, className }: MultiSourceStatusProps) {
  const activeCount = sources.filter(s => 
    s.status === 'active' || 
    (s.fetchedAt && DataPipeline.determineStatus(
      s.fetchedAt, 
      DataPipeline.sources[s.id] || { refreshIntervalMs: 900000, maxAgeMs: 3600000 } as DataSourceConfig
    ) === 'active')
  ).length
  ...
  return (
    <div className={cn('flex items-center gap-2 text-xs', className)}>
      <span className={cn(
        ...
        allActive ? 'bg-amber-400' : activeCount > 0 ? 'bg-yellow-400' : 'bg-red-400'
      )} />
      <span className={allActive ? 'text-amber-400' : activeCount > 0 ? 'text-yellow-400' : 'text-red-400'}>
