...
import { Trophy, Clock, Users, ChevronRight, Sparkles, Zap } from 'lucide-react'
import { cn } from '@/lib/utils'
import { useHaptic } from '@/hooks/useHaptic'
import { DAILY_CHALLENGES, calculateProgress, type Challenge } from '@/lib/viral/challenge-system'
...
export function DailyChallengeWidget({ className, compact = false }: DailyChallengeWidgetProps) {
      ...
      <div className="p-4 border-b border-[var(--color-border)]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">Defis du jour</h3>
          ...
          <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-orange-500/10 text-orange-400">
            ...
            <span className="text-xs font-medium">+{challenges.reduce((sum, c) => sum + c.xpReward, 0)} XP</span>
          </div>
        </div>
      </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={cn(
                      ...
                      challenge.difficulty === 'legendary' && 'bg-violet-500/20 text-violet-400'
                    )}>
                      {challenge.difficulty === 'easy' ? 'facile' : 
                       challenge.difficulty === 'medium' ? 'moyen' : 
                       challenge.difficulty === 'hard' ? 'difficile' : 'legendaire'}
                    </span>
                  </div>
