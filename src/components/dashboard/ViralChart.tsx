...
export function ViralChart({
  data,
  ariaLabel = "Score viral dans le temps",
}: ViralChartProps) {
  if (!data.length) {
    return (
      <div
        ...
        role="status"
      >
        Pas assez de points d’historique pour tracer la courbe. Ouvre
        /intelligence/viral-control pour plus de contexte.
      </div>
    );
  }
    ...
    <div className="h-56 w-full" aria-label={ariaLabel}>
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <XAxis
            dataKey="time"
            tick={{ fill: "rgba(240,240,248,0.45)", fontSize: 10 }}
            axisLine={false}
            tickLine={false}
          />
          <YAxis
