      ...
      {data.topTrend ? (
        <div className="space-y-1">
          <p className="text-[11px] text-[var(--color-text-muted)]">
            Tendance utilisée pour la recherche
          </p>
          <p className="text-sm font-medium text-[var(--color-text-primary)]">
            {data.topTrend.title}
          </p>
          <p className="text-[11px] text-[var(--color-text-tertiary)]">
            Volume indicatif : {data.topTrend.trafficVolume} ·{" "}
            {data.topTrend.source}
          </p>
        </div>
      ) : (
        <p className="text-[13px] text-[var(--color-text-tertiary)]">
          Pas de tendance tête de liste pour cette zone.
        </p>
      )}

      {!data.youtubeConfigured ? (
        <p className="text-[12px] text-[var(--color-text-tertiary)]">
          YouTube non configuré (YOUTUBE_API_KEY) : pas de vidéos liées ici — le
          score radar reste disponible au-dessus.
        </p>
      ) : data.youtubeSearchSkipped === "require_pro" ? (
        <p className="text-[12px] text-[var(--color-text-tertiary)]">
          Recherche YouTube réservée au plan Pro sur ce déploiement. Les
          tendances ci-dessus restent disponibles.
        </p>
      ) : top ? (
        <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)]/40 p-3 space-y-2">
          <p className="text-[11px] text-[var(--color-text-muted)]">
            Top vidéo (score fusion ALGO)
          </p>
          ...
          <div className="flex flex-wrap items-center gap-3 text-[12px] text-[var(--color-text-tertiary)]">
            <span className="font-mono tabular-nums text-[var(--color-violet)] font-semibold">
              {top.fusionScore}/100
            </span>
        ...
        <p className="text-[12px] text-[var(--color-text-tertiary)]">
          Aucune vidéo renvoyée pour cette tendance.
        </p>
      )}
