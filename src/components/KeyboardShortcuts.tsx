...
export function KeyboardShortcuts() {
  ...
  const shortcuts: Shortcut[] = useMemo(
    () => [
      { key: 'h', label: 'Accueil (NOW)', action: () => router.push('/'), modifier: 'meta' },
      { key: 't', label: 'Trends', action: () => router.push('/trends'), modifier: 'meta' },
      { key: 's', label: 'Rising Stars', action: () => router.push('/rising-stars'), modifier: 'meta' },
      { key: 'v', label: 'Videos', action: () => router.push('/videos'), modifier: 'meta' },
      { key: 'n', label: 'News', action: () => router.push('/news'), modifier: 'meta' },
      { key: 'k', label: 'Recherche', action: () => router.push('/search'), modifier: 'meta' },
      { key: 'p', label: 'Profil', action: () => router.push('/profile'), modifier: 'meta' },
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    for (const shortcut of shortcuts) {
      const modifierPressed = shortcut.modifier
        ? (shortcut.modifier === 'meta' && (event.metaKey || event.ctrlKey)) ||
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-[var(--color-text-primary)]">Raccourcis clavier</h2>
          </div>
          <button
            onClick={() => setShowHelp(false)}
        ...
        <p className="text-xs text-[var(--color-text-muted)] text-center">
          Appuyez sur <kbd className="px-1.5 py-0.5 rounded bg-[var(--color-card)] border border-[var(--color-border)] text-[var(--color-text-tertiary)]">?</kbd> pour afficher ce menu
        </p>
      </div>
    </div>
  )
