...
export function AlgoControlRoomModuleGraph({ state, reduceMotion }: Props) {
  ...
  const edgeKeys = (e: { from: ControlRoomModuleId; to: ControlRoomModuleId }) => `${e.from}->${e.to}`
  ...
  return (
    ...
    >
      <figcaption className="sr-only">
        Schéma indicatif des flux entre probe, endpoint santé, pipeline question-réponse et surfaces radar — décor, pas
        une capture runtime.
      </figcaption>
      <svg viewBox="0 0 352 104" className="w-full h-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-card)]/50">
        <defs>
          <linearGradient id="crGraphStroke" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="var(--color-blue-neon, #5ecfff)" stopOpacity="0.25" />
            <stop offset="50%" stopColor="var(--color-blue-neon, #5ecfff)" stopOpacity="0.85" />
            <stop offset="100%" stopColor="var(--color-violet, #7B61FF)" stopOpacity="0.35" />
          </linearGradient>
        </defs>

        <g opacity={0.22}>
          {CONTROL_ROOM_FLOW_EDGES.map((e) => (
            <path key={`dim-${edgeKeys(e)}`} d={lineBetween(e.from, e.to)} stroke="var(--color-border)" strokeWidth={1} fill="none" />
          ))}
        </g>

        <g>
          {litEdges.map((e) => (
            <path key={edgeKeys(e)} className={styles.edge} d={lineBetween(e.from, e.to)} />
          ))}
        </g>

        {(Object.keys(CONTROL_ROOM_MODULE_LAYOUT) as ControlRoomModuleId[]).map((id) => {
