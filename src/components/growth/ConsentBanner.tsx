    ...
    >
      <div className="max-w-3xl mx-auto rounded-2xl border border-[var(--color-border-strong)] bg-[color-mix(in_srgb,var(--color-bg-secondary)_96%,transparent)] backdrop-blur-md shadow-2xl shadow-black/40 p-4 sm:p-5 text-sm text-[var(--color-text-primary)]">
        ...
        <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed mb-4">
          ALGO utilise des cookies strictement nécessaires au fonctionnement du
          site. Avec votre accord, nous pouvons activer une mesure d’audience{" "}
          <span className="text-[var(--color-text-primary)]">anonymisée</span>{" "}
    ...
    <button
      ...
      onClick={() => {
        if (typeof window !== "undefined")
          window.dispatchEvent(new Event("algo:open-consent"));
      }}
      className={className}
    >
      Préférences cookies
