...
export default function AlgoAiPage() {
  ...
  const [status, setStatus] = useState<"loading" | "ready" | "partial">(
    "loading",
  );
  const [expertise, setExpertise] =
    useState<AlgoExpertiseLevel>("intermediate");

  useEffect(() => {
    try {
      const ex = window.localStorage.getItem(
        EXPERTISE_KEY,
      ) as AlgoExpertiseLevel | null;
      if (ex === "novice" || ex === "intermediate" || ex === "advanced")
        setExpertise(ex);
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    window.localStorage.setItem(
      HISTORY_KEY,
      JSON.stringify(messages.slice(-18)),
    );
  }, [messages]);
  ...
  useEffect(() => {
    ...
    const loadContext = async () => {
      try {
        ...
        const t = (await trendsRes.json()) as {
        ...
        setStatus(
          tMapped.length > 0 || vMapped.length > 0 ? "ready" : "partial",
        );
      } catch {
        if (cancelled) return;
        setStatus("partial");
  ...
  const ask = async () => {
    ...
    try {
      ...
      if (mode === "assistant") {
        const res = await fetch("/api/ai/ask", {
          ...
          body: JSON.stringify({
            question,
            context: {
              currentTrends: trends.map((t) => t.title),
        ...
        const baseLines = Array.isArray(json.transparencyLines)
          ? json.transparencyLines
          : [];
        const answerText =
        ...
        assistant = {
          ...
          transparencyLines: [
            ...(Array.isArray(json.transparencyLines)
              ? json.transparencyLines
              : []),
            expertiseTransparencyLineFr(expertise),
          ],
        ...
        assistant = {
          ...
          text: [
            ...
            ...(b?.topSignals || [])
              .slice(0, 3)
              .map((s) => `- ${s.title} (${s.viralScore}) · ${s.whyImportant}`),
            "Breakouts probables:",
      ...
      setMessages((m) => [
        ...m,
        assistant || fallbackAnswer(mode, question, country, expertise),
      ]);
    } catch {
      setMessages((m) => [
        ...m,
        fallbackAnswer(mode, question, country, expertise),
      ]);
    } finally {
      setSending(false);
    }
      ...
      <section className="max-w-6xl mx-auto px-4 pt-8 pb-4">
        ...
        <div className="mt-3 rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-950/35 to-violet-950/25 p-4 sm:p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-white/60 mt-1 max-w-2xl">
                Intelligence analytique du système : signaux, tranchage,
                prochain pas · calibrée sur la même ligne directrice que le
                reste d’ALGO (création, viralité, analyse, garde-fous). Les
                scores et briefings sont des indicateurs, pas des certitudes.
              </p>
            </div>
            <span
              className={cn(
          ...
          <p className="text-[11px] text-white/40 mt-3">
            Transparence : si les APIs ou le modèle manquent de contexte, elle
            le dit et baisse la confiance · pas de mémoire fantôme, pas de
            sources inventées. Comportement ancré sur une directive interne
            sobre (pas de mysticisme, pas de promesses irréalistes sur les
            algorithmes des réseaux).{" "}
            <Link
              href={SITE_TRANSPARENCY_AI_CALIBRATION_HREF}
              className="text-cyan-400/85 hover:text-cyan-300 underline underline-offset-2"
            >
        ...
        <aside className="algo-surface rounded-xl p-3 space-y-2 h-fit">
          ...
          <div className="pt-2 border-t border-[var(--color-border)] text-[11px] text-[var(--color-text-tertiary)] space-y-2">
            ...
            <p className="text-[10px] text-white/35 leading-snug">
              Tous les modes qui appellent le modèle (guide, analyse, idées,
              veille) calibrent jargon et densité sur ce réglage.
            </p>
          </div>
          ...
          <div className="pt-2 border-t border-[var(--color-border)] text-[11px] text-[var(--color-text-tertiary)] space-y-1">
            ...
            <Link
              href="/viral-analyzer"
              className="block transition-colors duration-200 hover:text-[var(--color-text-primary)]"
            >
              Analyser un lien
            </Link>
            <Link
            ...
            {messages.length === 0 ? (
              <div className="h-full grid place-items-center text-center text-white/40 text-sm px-4">
                <div>
                  <Sparkles
                    size={22}
                    className="mx-auto mb-2 text-cyan-300/70"
                  />
                  <p className="text-white/55 font-medium mb-1">
                    Une question nette → une réponse structurée.
                  </p>
                  <p className="text-[12px] text-white/40 max-w-md mx-auto">
                    Elle conclut d’abord, explique vite, puis te donne une
                    action. Choisis un mode ou laisse le guide t’orienter.
                  </p>
                </div>
              </div>
            ) : (
                ...
                >
                  ...
                  msg.standard ? (
                    <div className="mb-2 rounded-lg border border-cyan-500/15 bg-black/20 p-2.5 space-y-2 text-[12px]">
                      <p className="text-[10px] uppercase tracking-wider font-bold text-cyan-200/80">
                        Lecture
                      </p>
                      <p className="text-white/60 leading-relaxed">
                        {msg.standard.comprehension}
                      </p>
                      <p className="text-[10px] uppercase tracking-wider font-bold text-cyan-200/80 pt-1">
                        Réponse
                      </p>
                      <p className="text-sm text-white/80 whitespace-pre-wrap">
                        {msg.standard.reponse}
                      </p>
                      <p className="text-[10px] uppercase tracking-wider font-bold text-emerald-200/85 pt-1">
                        Action
                      </p>
                      <p className="text-white/75 leading-relaxed border-l-2 border-emerald-500/35 pl-2">
                        {msg.standard.action}
                      </p>
                    </div>
                  ) : (
                  ...
                  ((msg.structured.options?.length ?? 0) > 0 ||
                    ...
                    <div className="mt-3 rounded-lg border border-violet-500/20 bg-violet-950/25 p-3 space-y-2 text-[12px]">
                      ...
                      {msg.structured.options &&
                      msg.structured.options.length > 0 ? (
                        <ul className="space-y-2 text-white/75">
                          {msg.structured.options.map((opt, i) => (
                            <li
                              key={i}
                              className="border-l border-violet-400/30 pl-2"
                            >
                              ...
                              {opt.downside ? (
                                <p className="text-[11px] text-amber-200/75 mt-0.5">
                                  − {opt.downside}
                                </p>
                              ) : null}
                            </li>
                      ...
                      {msg.structured.recommendedChoice ? (
                        <p className="text-white/80 text-[12px]">
                          <span className="text-violet-200/90 font-semibold">
                            Piste privilégiée :{" "}
                          </span>
                  ...
                  {msg.role === "assistant" &&
                  msg.transparencyLines &&
                  msg.transparencyLines.length > 0 ? (
                    <details className="mt-3 rounded-lg border border-cyan-500/15 bg-black/25 p-2.5 group">
                      <summary className="cursor-pointer list-none flex items-center justify-between gap-2 text-[11px] font-semibold text-cyan-200/95 hover:text-cyan-100 [&::-webkit-details-marker]:hidden">
                        ...
                        <span className="text-[10px] text-white/35 font-normal group-open:hidden">
                          Afficher
                        </span>
                        <span className="text-[10px] text-white/35 font-normal hidden group-open:inline">
                          Masquer
                        </span>
                      </summary>
                      <ul className="mt-2 space-y-1.5 text-[11px] text-white/60">
                        {msg.transparencyLines.map((line, i) => (
                          <li
                            key={i}
                            className="leading-relaxed border-l border-cyan-500/20 pl-2"
                          >
                            {line}
                          </li>
                        ))}
