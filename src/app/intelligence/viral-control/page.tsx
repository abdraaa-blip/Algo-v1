...
import {
  VIRAL_CONTROL_REGION_CODES,
  isViralControlRegion,
} from "@/lib/intelligence/viral-control-regions";
...
function momentumLabel(
  m: ViralControlCockpitPayload["global"]["momentum"],
): string {
...
function ViralSeriesChart({
  series,
}: {
  series: ViralControlCockpitPayload["series"];
}) {
  const titleId = useId();
  const descId = useId();
  const path = useMemo(() => {
    ...
    const pad = 6;
    const xs = series.map(
      (_, i) => pad + (i / (series.length - 1)) * (w - pad * 2),
    );
  ...
  if (series.length < 2) {
    return (
      <p className="text-xs text-[var(--color-text-tertiary)] py-8 text-center">
        Pas assez d’historique radar pour tracer une courbe. Lance un
        rafraîchissement du radar ou attends le prochain snapshot cron.
      </p>
    );
  }
  ...
  return (
    ...
    >
      ...
      <desc id={descId}>
        Courbe des scores agrégés sur la période affichée (pas des vues d’un
        contenu unique).
      </desc>
      <path
        d={path}
...
function YoutubeViewsChart({
  series,
}: {
  series: ViralControlYoutubeBlock["observationSeries"];
}) {
  const titleId = useId();
  const descId = useId();
  const path = useMemo(() => {
  ...
  if (series.length < 2) {
    return (
      <p className="text-xs text-[var(--color-text-tertiary)] py-6 text-center">
        Pas assez de points encore : la courbe se remplit à chaque
        rafraîchissement (polling ~30 s) tant que la clé YouTube est active.
      </p>
    );
  }
  ...
  return (
    ...
    >
      ...
      <desc id={descId}>
        Courbe des vues publiques enregistrées à chaque appel API, pas
        l&apos;historique complet YouTube.
      </desc>
      <path
        d={path}
        fill="none"
...
export default function ViralControlCenterPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/intelligence/viral-control?${q.toString()}`,
        { cache: "no-store" },
      );
      ...
      if (
        !res.ok ||
        !json.success ||
        json.kind !== "algo.viral_control_cockpit"
      ) {
  ...
  return (
    <div className="min-h-0 w-full text-[var(--color-text-primary)] p-4 sm:p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight">
              Viral Control Center
            </h1>
            <p className="text-sm text-[var(--color-text-secondary)] mt-2 max-w-xl">
              Cockpit indicatif : chaque bloc relie un signal à une décision
              possible. Option YouTube : vues / likes / commentaires publics par
              URL si{" "}
              <span className="font-mono text-[11px]">YOUTUBE_API_KEY</span> est
              défini.
            </p>
            <p className="text-[11px] text-[var(--color-text-muted)] mt-1 max-w-xl">
              Région préremplie depuis ton scope (navbar) quand c’est un pays
        ...
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)]/60 p-4 space-y-2">
          <label
            htmlFor="vcc-youtube-url"
            className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-tertiary)]"
          >
          ...
          <>
            ...
            {data.serverBundleCacheTtlMs ? (
              <p className="text-[10px] text-[var(--color-text-muted)] font-mono">
                Cache bundle serveur ~
                {Math.round(data.serverBundleCacheTtlMs / 1000)} s (même région
                + même lien).
              </p>
            ) : null}
            ...
            {data.youtube ? (
              <div className="rounded-2xl border border-violet-400/20 bg-violet-950/10 p-4 space-y-3">
                ...
                {data.youtube.fetchedAt ? (
                  <p className="text-[10px] font-mono text-[var(--color-text-tertiary)]">
                    API{" "}
                    {new Date(data.youtube.fetchedAt).toLocaleString("fr-FR", {
                      timeStyle: "medium",
                    })}
                  </p>
                ) : null}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)]/80 p-4 space-y-3">
                ...
                <p className="text-4xl font-black tabular-nums text-emerald-300">
                  {data.global.viralScore}
                </p>
                <p className="text-xs text-[var(--color-text-secondary)]">
                  Confiance indicative{" "}
                  {Math.round(data.global.confidence * 100)} % ·{" "}
                  {momentumLabel(data.global.momentum)}
                </p>
                <p className="text-xs text-[var(--color-text-tertiary)] font-mono">
                  Vitesse {formatDelta(data.global.velocityPerHour, "pts/h")} ·
                  Accélération{" "}
                  {formatDelta(data.global.accelerationPerHour2, "pts/h²")}
                </p>
                {data.topCategory ? (
                  <p className="text-xs text-[var(--color-text-secondary)]">
                    Catégorie tête de liste :{" "}
              ...
              <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)]/80 p-4 space-y-3">
                ...
                <p className="text-[11px] text-[var(--color-text-muted)]">
                  {data.engagementProxy.noteFr}
                </p>
                <ScoreBar
                  label="Vues (index)"
                  value={data.engagementProxy.viewsIndex}
                />
                <ScoreBar
                  label="Likes (index)"
                  value={data.engagementProxy.likesIndex}
                />
                <ScoreBar
                  label="Commentaires (index)"
                  value={data.engagementProxy.commentsIndex}
                />
                <ScoreBar
                  label="Partages (index)"
                  value={data.engagementProxy.sharesIndex}
                />
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)]/80 p-4 space-y-2">
                ...
                <p className="text-sm font-semibold text-[var(--color-text-primary)]">
                  {data.trendVsGlobal.headlineFr}
                </p>
                <p className="text-xs text-[var(--color-text-secondary)]">
                  {data.trendVsGlobal.detailFr}
                </p>
              </div>
              <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)]/80 p-4 space-y-2">
                <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-tertiary)]">
                ...
                {data.anomalies.length === 0 ? (
                  <p className="text-xs text-[var(--color-text-secondary)]">
                    Aucune anomalie remontée sur ce cycle.
                  </p>
                ) : (
                  <ul className="space-y-2 text-xs text-[var(--color-text-secondary)]">
                    {data.anomalies.map((a, i) => (
                      <li
                        key={`${a.type}-${i}`}
                        className="border-l-2 border-amber-400/40 pl-2"
                      >
