...
export default function IntelligenceOpsPage() {
  ...
  const load = useCallback(async () => {
    const aRes = await fetch("/api/intelligence/ops-alerts", {
      cache: "no-store",
    });
      ...
      <div className="max-w-6xl mx-auto space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] text-sm">
              Unified alerts, runbook execution, and incident log
            </p>
          </div>
        ...
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-3">
          ...
          <div className="space-y-2">
            {alerts.length === 0 && (
              <p className="text-xs text-emerald-300">No active alert.</p>
            )}
            {alerts.map((alert) => (
              <div
                key={alert.id}
                className="text-xs rounded-lg border border-[var(--color-border)] p-2 bg-[var(--color-bg-elevated)]"
              >
                <p
                  className={cn(
                    "font-semibold",
          ...
          <div className="space-y-2">
            {incidents.length === 0 ? (
              <p className="text-xs text-[var(--color-text-secondary)]">
                No incident yet (or token missing).
              </p>
            ) : (
