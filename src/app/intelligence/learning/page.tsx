...
export default function IntelligenceLearningPage() {
    ...
    const load = async () => {
      ...
      try {
        const [hRes, mRes] = await Promise.all([
          fetch("/api/intelligence/learning-history?limit=120", {
            cache: "no-store",
          }),
        ...
        if (hJson.success && Array.isArray(hJson.alerts))
          setAlerts(hJson.alerts);
  ...
  const stats = useMemo(() => {
    ...
    const avgThreshold =
      history.length > 0
        ? history.reduce((acc, h) => acc + h.nextMinConfidence, 0) /
          history.length
        : null;
    const avgHelpful =
      history.length > 0
      ...
      <div className="max-w-6xl mx-auto space-y-4">
        <div className="flex items-center justify-between gap-2">
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] text-sm">
              Calibration history, feedback quality, and memory stream
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
          ...
          <StatCard
            title="avg threshold"
            value={
              stats.avgThreshold === null ? "--" : stats.avgThreshold.toFixed(2)
            }
          />
          <StatCard
            title="avg helpful ratio"
            value={
              stats.avgHelpful === null
                ? "--"
                : `${Math.round(stats.avgHelpful * 100)}%`
            }
          />
        </div>
          ...
          <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-3">
            ...
            <div className="space-y-2">
              {alerts.map((alert, idx) => (
                <div
                  key={`${alert.type}-${idx}`}
                  className="text-xs border border-[var(--color-border)] rounded-lg p-2 bg-[var(--color-bg-elevated)]"
                >
                  <p
                    className={cn(
                      "font-semibold",
              ...
              {history.slice(0, 20).map((item) => (
                ...
                >
                  <p>
                    ...
                    {item.applied ? "applied" : "no-change"}
                  </p>
                  <p className="text-[var(--color-text-secondary)] mt-1">
                    threshold {item.previousMinConfidence.toFixed(2)} {"->"}{" "}
                    {item.nextMinConfidence.toFixed(2)} · helpful{" "}
                    {(item.helpfulRatio * 100).toFixed(0)}%
                  </p>
                </div>
              ))}
              {history.length === 0 && !loading && (
