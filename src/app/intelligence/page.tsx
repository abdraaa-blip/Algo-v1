...
import { Activity, BrainCircuit, Database, Lightbulb, PackageSearch, RefreshCw, TriangleAlert } from 'lucide-react'
...
export default function IntelligencePage() {
  ...
  const [globalData, setGlobalData] = useState<GlobalResponse['data'] | null>(null)
  const [predictive, setPredictive] = useState<PredictiveResponse['data'] | null>(null)
  ...
  const [autonomyProposals, setAutonomyProposals] = useState<AutonomyProposal[]>([])
  const [simulated, setSimulated] = useState<{ baselineScore: number; simulatedScore: number; delta: number; recommendation: string } | null>(null)
  const [learnResult, setLearnResult] = useState<{ learningApplied: boolean; minConfidenceForAuto?: number } | null>(null)
  ...
  const [reliabilityData, setReliabilityData] = useState<DataReliabilityApiPayload | null>(null)
  ...
  const fetchRadar = useCallback(async () => {
    ...
    try {
      const [g, p, pr, healthRes] = await Promise.all([
        fetch(`/api/intelligence/global?region=${region}&locale=fr`, { cache: 'no-store' }),
        fetch(`/api/intelligence/predictive?region=${region}&locale=fr`, { cache: 'no-store' }),
        fetch(`/api/intelligence/products?region=${region}&locale=fr`, { cache: 'no-store' }),
        fetch(`/api/intelligence/health?region=${region}`, { cache: 'no-store' }),
      ])
      ...
      setCacheTelemetry({
        ...
        health: {
          ...
          ttlMs: typeof healthJson.cacheTtlMs === 'number' ? healthJson.cacheTtlMs : null,
        },
      })
      ...
      if (healthRes.ok && healthJson.success && Array.isArray(healthJson.data)) {
        setCrossHealth(healthJson.data)
      } else {
        setCrossHealth([
          { id: 'news', label: 'News', status: 'degraded', details: 'fallback mode' },
          { id: 'music', label: 'Music', status: 'degraded', details: 'fallback mode' },
          { id: 'videos', label: 'Videos', status: 'degraded', details: 'fallback mode' },
          { id: 'trends', label: 'Trends', status: 'degraded', details: 'fallback mode' },
          { id: 'creator', label: 'Creator', status: 'degraded', details: 'fallback mode' },
        ])
      }
      ...
      if (globalSnapshot && predictiveSnapshot) {
        ...
        const nextInterval = hasHighFriction || anomalyCount >= 2 ? 30_000 : anomalyCount === 1 ? 45_000 : 75_000
  ...
  const historyStats = useMemo(() => {
    if (history.length === 0) return { min: 0, max: 100, avg: null as number | null }
  ...
  const decisionFeed = useMemo<DecisionItem[]>(() => {
    if (autonomyProposals.length > 0) {
      return autonomyProposals.map((proposal) => ({
        id: proposal.id,
        level: proposal.policyDecision.requiresApproval ? 'watch' : proposal.policyDecision.allowed ? 'action_now' : 'ignore',
        title: proposal.title,
        reason: proposal.rationale,
        riskLevel: proposal.riskLevel,
        requiresApproval: proposal.requiresApproval,
        approvalStatus: proposal.policyDecision.requiresApproval ? 'pending' : 'not_required',
        executionStatus: proposal.policyDecision.allowed ? 'simulated' : 'blocked',
    ...
    const highFriction = globalData.anomalies.some((a) => a.type === 'friction' && a.severity === 'high')
    ...
    if (predictive.predictedViralityScore >= 72 && predictive.confidence >= 0.68) {
      items.push({
        id: 'content-push',
        level: 'action_now',
  ...
  const globalCoherence = useMemo(() => {
    ...
    const coverageScore = Math.round((nonZeroSources / sourceCounts.length) * 100)
    const anomalyPenalty = Math.min(25, globalData.anomalies.length * 6)
    const engagementBonus = Math.round(globalData.sources.firstPartySignals.engagementRate * 20)
    const predictiveAlignment = Math.round((predictive.predictedViralityScore + (predictive.confidence * 100)) / 2)
    const value = Math.max(
      0,
      Math.min(100, Math.round(coverageScore * 0.35 + predictiveAlignment * 0.45 + engagementBonus - anomalyPenalty))
    )
    const status = value >= 75 ? 'high' : value >= 50 ? 'medium' : 'low'
    return { value, status, coverageScore, predictiveAlignment, anomalyPenalty }
  }, [globalData, predictive])

  const systemStatus = useMemo(() => {
    const downCount = crossHealth.filter((item) => item.status === 'down').length
    const degradedCount = crossHealth.filter((item) => item.status === 'degraded').length
    const staleCount = [cacheTelemetry.global, cacheTelemetry.predictive, cacheTelemetry.health].filter((s) => s.stale).length

    if (downCount > 0 || staleCount >= 2) {
      return { label: 'degraded', color: 'bg-rose-500/20 text-rose-300 border-rose-500/30' }
    }
    if (degradedCount > 1 || staleCount === 1) {
      return { label: 'watch', color: 'bg-amber-500/20 text-amber-300 border-amber-500/30' }
    }
    return { label: 'operational', color: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' }
  }, [cacheTelemetry.global, cacheTelemetry.health, cacheTelemetry.predictive, crossHealth])
  ...
  useEffect(() => {
    ...
    setDecisionLog((prev) => {
      ...
      const pruned = prev.filter((item) => new Date(item.at).getTime() >= horizon)
  ...
  useEffect(() => {
    ...
    if (!latest) return
    if (lastSyncedAtRef.current === latest.at || syncInFlightRef.current) return
  ...
  const runSimulation = useCallback(async () => {
    ...
    const res = await fetch('/api/intelligence/simulate', {
      ...
      body: JSON.stringify({
        ...
        scenario: { engagementDelta: 0.03, frictionDelta: -0.02, anomalyBoost: 0 },
      }),
    })
    const json = await res.json() as {
      success: boolean
      data?: { simulation: { baselineScore: number; simulatedScore: number; delta: number; recommendation: string } }
    }
  ...
  const executeProposal = useCallback(async (proposal: AutonomyProposal) => {
    ...
    setDecisionLog((prev) => {
      ...
      return [...prev.slice(0, -1), { ...last, items: nextItems, executedAt: new Date().toISOString() }]
    })
  }, [])

  const updateAutonomyMode = useCallback(async (mode: AutonomyState['mode']) => {
    if (!opsToken) return
  ...
  const loadDataReliabilityMap = useCallback(async () => {
    ...
    try {
      const res = await fetch(ALGO_DATA_RELIABILITY_PANEL.apiPath, { cache: 'no-store' })
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-[var(--color-text-tertiary)] mt-2">
              <Link href="/ai" className="text-cyan-400/90 hover:text-cyan-300 underline-offset-2 hover:underline">
                ALGO AI
              </Link>
          ...
          <div className="flex items-center gap-2">
            <span className={cn('text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full border font-bold', systemStatus.color)}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (typeof window !== 'undefined') {
                  if (e.target.value) {
                    window.sessionStorage.setItem('intelligence-ops-token', e.target.value)
                  } else {
        ...
        >
          <summary className="cursor-pointer select-none list-none flex items-center gap-2 px-4 py-3 text-sm font-semibold text-[var(--color-text-primary)] hover:bg-[var(--color-card-hover)]/60 [&::-webkit-details-marker]:hidden">
            <Database size={16} className="text-cyan-400/90 shrink-0" aria-hidden />
          ...
          <div className="px-4 pb-4 pt-0 space-y-3 border-t border-[var(--color-border)]/60">
            <p className="text-[11px] text-[var(--color-text-secondary)] pt-3">{ALGO_DATA_RELIABILITY_PANEL.note}</p>
            {reliabilityLoading && (
              <p className="text-xs text-[var(--color-text-tertiary)]">{ALGO_DATA_RELIABILITY_PANEL.loading}</p>
            )}
            {reliabilityError && <p className="text-xs text-rose-300/90">{reliabilityError}</p>}
                ...
                {reliabilityData.sources.map((row) => (
                  ...
                  >
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <span className="font-semibold text-[var(--color-text-primary)]">{row.id}</span>
                    ...
                    <p className="text-[11px] text-[var(--color-text-secondary)] mt-2">{row.limitsFr}</p>
            ...
            {reliabilityData?.generatedAt && (
              <p className="text-[10px] text-[var(--color-text-muted)]">
                Généré : {new Date(reliabilityData.generatedAt).toLocaleString('fr-FR')}
              </p>
            )}
            <p className="text-[10px]">
              <Link href="/transparency" className="text-cyan-400/90 hover:text-cyan-300 underline-offset-2 hover:underline">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card icon={BrainCircuit} title="Autonomy Control">
            <div className="space-y-2">
              <p className="text-xs text-white/60">
                mode: <span className="font-semibold text-white/90">{autonomyState?.mode || 'loading'}</span>
              </p>
              <p className="text-xs text-white/60">
                kill-switch:{' '}
                <span className={cn('font-semibold', autonomyState?.killSwitch ? 'text-rose-300' : 'text-emerald-300')}>
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(e) => void updateAutonomyMode(e.target.value as AutonomyState['mode'])}
              ...
              {simulated && (
                <p className="text-xs text-white/60">
                  sim: {simulated.baselineScore} → {simulated.simulatedScore} ({simulated.delta >= 0 ? '+' : ''}
                  {simulated.delta}) · {simulated.recommendation}
                </p>
              )}
              {learnResult && (
                <p className="text-xs text-white/60">
                  learning: {learnResult.learningApplied ? 'applied' : 'no-change'} · minConfidence: {learnResult.minConfidenceForAuto?.toFixed(2) ?? 'n/a'}
                </p>
              )}
            </div>
          </Card>
          ...
          <Card icon={Activity} title="Predicted Virality">
            <p className="text-3xl font-black text-cyan-300">{predictive?.predictedViralityScore ?? '--'}</p>
            <p className="text-xs text-white/60 mt-1">
              confidence: {predictive ? `${Math.round(predictive.confidence * 100)}%` : '--'}
            </p>
          ...
          <Card icon={BrainCircuit} title="Behavioral Signals">
            <p className="text-sm text-white/80">
              engagement: {globalData ? `${(globalData.sources.firstPartySignals.engagementRate * 100).toFixed(2)}%` : '--'}
            </p>
            <p className="text-sm text-white/80">
              friction: {globalData ? `${(globalData.sources.firstPartySignals.frictionRate * 100).toFixed(2)}%` : '--'}
            </p>
            <p className="text-xs text-white/50 mt-2">
            ...
            {globalData?.sources.science && (
              <p className="text-xs text-white/50 mt-1">
                science signals: {globalData.sources.science.count} ({globalData.sources.science.source})
              </p>
            )}
            {globalData?.sources.economic && (
              <p className="text-xs text-white/50 mt-1">
                economic signals: {globalData.sources.economic.count} ({globalData.sources.economic.source})
              </p>
            )}
            {globalData?.sources.socialExternal && (
              <p className="text-xs text-white/50 mt-1">
                social ext: {globalData.sources.socialExternal.count} ({globalData.sources.socialExternal.source})
              </p>
            )}
            {globalData?.sources.commerce && (
              <p className="text-xs text-white/50 mt-1">
                commerce: {globalData.sources.commerce.count} ({globalData.sources.commerce.source})
              </p>
            )}
          </Card>

          <Card icon={TriangleAlert} title="Anomalies">
            <p className="text-sm text-white/80">{globalData?.anomalies.length ?? 0} detected</p>
            <ul className="mt-2 space-y-1 text-xs text-white/60">
              {(globalData?.anomalies || []).slice(0, 3).map((a) => (
                <li key={`${a.type}-${a.message}`}>- {a.message}</li>
              ))}
              {(globalData?.anomalies || []).length === 0 && <li>- no active anomaly</li>}
            </ul>
          </Card>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {(
              [
                ...
                { id: 'predictive', label: 'Predictive', ...cacheTelemetry.predictive },
                { id: 'health', label: 'Health', ...cacheTelemetry.health },
              ] as const
            ).map((item) => (
              <div key={item.id} className="p-2 rounded-lg bg-[var(--color-card)] border border-[var(--color-border)]">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={cn(
                      ...
                      !item.stale && item.cached && 'bg-amber-500/20 text-amber-300',
                      !item.stale && !item.cached && 'bg-emerald-500/20 text-emerald-300'
                    )}
                  >
                ...
                <p className="text-[11px] text-white/50 mt-1">
                  ttl: {item.ttlMs === null ? 'n/a' : `${Math.round(item.ttlMs / 1000)}s`}
                </p>
              </div>
            ))}
          </div>
        ...
        <Card icon={BrainCircuit} title="Global Coherence Score">
          <div className="flex items-center justify-between gap-3">
            <p className="text-3xl font-black text-violet-300">{globalCoherence?.value ?? '--'}</p>
            <span
              className={cn(
                'text-[10px] uppercase tracking-wider px-2 py-1 rounded-full font-bold',
                globalCoherence?.status === 'high' && 'bg-emerald-500/20 text-emerald-300',
                globalCoherence?.status === 'medium' && 'bg-amber-500/20 text-amber-300',
                globalCoherence?.status === 'low' && 'bg-rose-500/20 text-rose-300'
              )}
            >
          ...
          <div className="mt-2 text-xs text-white/60 space-y-1">
            ...
            <p>predictive alignment: {globalCoherence?.predictiveAlignment ?? '--'}</p>
              ...
              <div key={item.id} className="p-2 rounded-lg bg-[var(--color-card)] border border-[var(--color-border)]">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={cn(
                      'text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded-full font-bold',
                      item.status === 'up' && 'bg-emerald-500/20 text-emerald-300',
                      item.status === 'degraded' && 'bg-amber-500/20 text-amber-300',
        ...
        <Card icon={Activity} title="Radar history (7j) + adaptive refresh">
          <div className="flex items-center justify-between gap-3 mb-3">
            <p className="text-xs text-white/60">
              auto refresh: every {Math.round(autoRefreshMs / 1000)}s (adjusts with anomaly pressure) · serveur échantillonne
              ~1 point / 10 min / région
            </p>
          ...
          <div className="h-20 rounded-lg bg-[var(--color-card)] border border-[var(--color-border)] p-2 flex items-end gap-1">
            {history.length === 0 ? (
              <p className="text-xs text-white/40">Waiting for first samples...</p>
            ) : (
              history.map((point) => {
                const range = Math.max(1, historyStats.max - historyStats.min)
                const normalized = (point.viralityScore - historyStats.min) / range
                const heightPx = Math.max(6, Math.round(10 + normalized * 50))
                const isAlert = point.anomalyCount > 0
                return (
                  <div
                    key={point.at}
                    className={cn('w-2 rounded-sm', isAlert ? 'bg-amber-300/80' : 'bg-cyan-300/80')}
          ...
          <p className="text-xs text-white/50 mt-2">
            avg: {historyStats.avg === null ? '--' : historyStats.avg.toFixed(1)} · min: {historyStats.min} · max: {historyStats.max}
          </p>
        </Card>

        <Card icon={Lightbulb} title="Decision Feed">
          <div className="flex items-center justify-between gap-2 mb-2">
            <p className="text-xs text-white/50">journal: {decisionLog.length} entries (last 60 min)</p>
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-2">
            {decisionFeed.length === 0 ? (
              <p className="text-xs text-white/50">No decision yet. Waiting for complete signal set.</p>
            ) : (
              decisionFeed.map((item) => (
                <div key={item.id} className="p-2.5 rounded-lg bg-[var(--color-card)] border border-[var(--color-border)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card icon={Lightbulb} title="Top Opportunities">
            <div className="space-y-2">
              {(globalData?.opportunities || []).slice(0, 5).map((o) => (
                <div key={`${o.type}-${o.title}`} className="p-2 rounded-lg bg-[var(--color-card)] border border-[var(--color-border)]">
          ...
          <Card icon={PackageSearch} title={ALGO_PRODUCT_RADAR.cardTitle}>
            <p className="text-xs text-white/50 mb-3 leading-relaxed">{ALGO_PRODUCT_RADAR.cardIntro}</p>
            {products?.missionFr && (
              <p className="text-[11px] text-violet-300/90 mb-3 leading-relaxed">{products.missionFr}</p>
            )}
            <div className="space-y-2">
              {(products?.products || []).length === 0 && loading ? (
                <p className="text-xs text-white/35">{ALGO_UI_LOADING.coreIntelligence}</p>
              ) : (products?.products || []).length === 0 ? (
                <p className="text-xs text-white/45">{ALGO_PRODUCT_RADAR.empty}</p>
              ) : (
                (products?.products || []).slice(0, 6).map((p) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium capitalize">{p.keyword}</p>
                      <p className="text-xs text-violet-300/90">{p.potentialLabel ?? p.potential}</p>
                      <p className="text-[11px] text-white/45 mt-1 leading-relaxed">{p.rationale}</p>
                    </div>
                    <div className="text-sm font-bold text-emerald-300 shrink-0 tabular-nums">{p.score}</div>
                  </div>
                ))
              )}
            </div>
            <div className="mt-3 pt-3 border-t border-white/10 space-y-2">
              <p className="text-[11px] text-white/40 leading-relaxed">{ALGO_PRODUCT_RADAR.disclaimer}</p>
              <Link
                href="/trends"
        ...
        <Card icon={Activity} title="Category Momentum">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
            {(globalData?.categories || []).map((c) => (
              <div key={c.name} className="p-3 rounded-lg bg-[var(--color-card)] border border-[var(--color-border)]">
          ...
          <p className="text-xs text-white/40 mt-3">
            {globalData?.generatedAt ? `last update: ${new Date(globalData.generatedAt).toLocaleString()}` : loading ? 'loading...' : 'no data'}
          </p>
        </Card>
      </div>
    </div>
