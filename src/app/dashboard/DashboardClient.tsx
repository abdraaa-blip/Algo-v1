...
type ViralPayload = {
  success?: boolean
  global?: { viralScore: number; confidence: number; momentum?: 'up' | 'stable' | 'down' }
...
export function DashboardClient() {
  ...
  const loadViral = useCallback(async () => {
    try {
      const res = await fetch(`/api/intelligence/viral-control?region=${encodeURIComponent(region)}&days=7`, {
        cache: 'no-store',
      })
  ...
  const loadFusion = useCallback(async () => {
    try {
      const res = await fetch(`/api/intelligence/viral-fusion?region=${encodeURIComponent(region)}`, {
        cache: 'no-store',
      })
      const json = (await res.json()) as ViralFusionPayload & { success?: boolean; error?: string }
      if (!res.ok) {
        setFusionError('Fusion tendance / YouTube indisponible pour l’instant.')
        setFusion(null)
        return
      }
  ...
  const loadInsightForPayload = useCallback(
    async (
      ...
      const merged = [hint, extraTrendHint?.trim()].filter((x): x is string => Boolean(x && x.trim().length > 1))
      ...
      try {
        const r2 = await fetchAlgo(
          ...
          { context: { userCountry: country, currentTrends: trendsCtx }, country }
        )
        ...
        if (!signal?.aborted) {
          setInsight('Lecture IA indisponible pour l’instant. Réessaie ou ouvre /ai.')
        }
      }
    },
    []
    ...
    async function boot() {
      ...
      try {
        const [resViral, resFusion] = await Promise.all([
          fetch(`/api/intelligence/viral-control?region=${encodeURIComponent(region)}&days=7`, {
            cache: 'no-store',
            signal: ac.signal,
          }),
          fetch(`/api/intelligence/viral-fusion?region=${encodeURIComponent(region)}`, {
            cache: 'no-store',
            signal: ac.signal,
          }),
        ])
        ...
        const fusionJson = (await resFusion.json()) as ViralFusionPayload & { success?: boolean }
        if (!ac.signal.aborted) {
      ...
      const trendHint = !ac.signal.aborted ? fusionPayload?.topTrend?.title ?? null : null
  ...
  const refreshInsight = useCallback(async () => {
    setInsightLoading(true)
    await loadInsightForPayload(viral, region, undefined, fusion?.topTrend?.title ?? null)
    ...
    <main className="min-h-dvh text-[var(--color-text-primary)] px-4 py-8 max-w-3xl mx-auto space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-[13px] text-[var(--color-text-tertiary)] max-w-xl">
          Trois réponses : est-ce que ça décolle ? pourquoi ? quoi faire maintenant ? Radar rafraîchi ~10s · zone{' '}
          <span className="font-mono text-[var(--color-text-secondary)]">{region}</span>
          {radarSyncedAt ? (
            <>
              {' '}
              · dernière synchro locale{' '}
              <time dateTime={radarSyncedAt.toISOString()}>
                {radarSyncedAt.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
              </time>
            </>
          ) : null}
        </p>
        <p className="text-[11px] text-[var(--color-text-muted)]">{viral?.disclaimerFr}</p>
      </header>

      {viralError ? <p className="text-sm text-amber-200/90">{viralError}</p> : null}

      <ScoreCard score={score} confidence={confidence} momentum={momentum} />

      <FusionContextPanel data={fusion} error={fusionError} loading={fusionLoading} />

      <section className="algo-surface rounded-xl p-4 border border-[var(--color-border)] space-y-2">
        <h2 className="text-sm font-semibold text-[var(--color-text-secondary)]">Courbe score (historique radar)</h2>
        <ViralChart data={chartData} />
        {viral?.radarDeltaHint ? (
          <p className="text-[12px] text-[var(--color-text-tertiary)] leading-relaxed pt-1" role="status">
            {viral.radarDeltaHint.noteFr}
          </p>
        ) : null}
      </section>

      <InsightBox text={insight} loading={insightLoading} onRefreshInsight={refreshInsight} />

      <footer className="pt-2 text-[12px] text-[var(--color-text-muted)] flex flex-wrap gap-3">
        <Link href="/intelligence/viral-control" className="text-[var(--color-violet)] hover:underline">
          Cockpit complet Viral Control →
        </Link>
        <Link href="/trends" className="text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)]">
          Tendances live
        </Link>
      </footer>
    </main>
