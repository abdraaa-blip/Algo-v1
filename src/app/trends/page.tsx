...
} from "@/components/algo/TrendIntelligenceCard";
...
const FALLBACK_TRENDS: Trend[] = [
  {
    ...
    whyItWorks:
      "Sujet nouveau + forte curiosité + impact direct sur les créateurs",
    contentIdeas: [
      "Reaction aux nouvelles annonces",
      "Test des outils IA video",
      "Avant/Apres avec IA",
    ],
    ...
    hookExamples: [
      "Ce que l'IA va changer dès demain",
      "On ne te dit pas tout sur…",
      "J'ai testé et voilà ce qui se passe",
    ],
    origin: "Twitter",
  ...
  },
  {
    ...
    hookExamples: [
      "Voila pourquoi la France va gagner",
      "Ce joueur va tout changer",
      "Personne ne s'attend a ca",
    ],
  },
  {
    id: "3",
    ...
    hookExamples: [
      ...
      "Le cameo que personne n'attendait",
      "Ca change TOUT",
    ],
  },
  {
  ...
  },
  {
    ...
    hookExamples: [
      ...
      "Je pensais pas que ca marcherait",
      "Version ratee vs reussie",
    ],
          ...
          const mapped = data.data.map(
            (t: Record<string, unknown>, i: number) => {
              ...
              const baseScore = Math.min(
                99,
                Math.max(50, 60 + Math.log10(trafficNum + 1) * 15),
              );
              ...
              const growthTrend =
                growthRate > 50 ? "up" : growthRate > 0 ? "stable" : "down";

              // Guess category from title
              const titleLower = title.toLowerCase();
              const category = /tech|ia|ai|digital|startup|robot|cyber/.test(
                titleLower,
              )
                ? "Tech"
                : /sport|football|match|ligue|nba|tennis|rugby|can|coupe/.test(
                      titleLower,
                    )
                  ? "Sport"
                  : /film|cinema|serie|netflix|disney|marvel|deadpool/.test(
                        titleLower,
                      )
                    ? "Cinema"
                    : /musique|concert|artiste|album|spotify|afrobeats/.test(
                          titleLower,
                        )
                      ? "Musique"
                      : /politique|president|gouvernement|election|ministre/.test(
                            titleLower,
                          )
                        ? "Politique"
                        : /economie|bourse|marche|entreprise|finance|crypto|bitcoin|naira/.test(
                              titleLower,
                            )
                          ? "Finance"
                          : /meteo|weather|storm/.test(titleLower)
                            ? "Meteo"
                            : /royal|people|scandale|drama|influenceur/.test(
                                  titleLower,
                                )
                              ? "People"
                              : "Actualite";
              ...
              const explanationTemplates = {
                Sport: [
                  `Match important en cours`,
                  `Resultats et reactions`,
                  `Actualite sportive brulante`,
                ],
                Cinema: [
                  `Nouvelle sortie ou annonce`,
                  `Buzz sur les réseaux`,
                  `Trailer ou leak viral`,
                ],
                Tech: [
                  `Innovation qui fait parler`,
                  `Annonce majeure du secteur`,
                  `Sujet tech en explosion`,
                ],
                Musique: [
                  `Nouveau titre viral`,
                  `Artiste en tendance`,
                ...
                Finance: [
                  `Mouvement de marche`,
                  `Actualite economique chaude`,
                  `Signal financier fort`,
                ],
                People: [
                  `Buzz ou scandale`,
                  `Actualite people`,
                ...
                Politique: [
                  `Actualite politique majeure`,
                  `Debat en cours`,
                  `Annonce gouvernementale`,
                ],
                default: [
                  `Sujet en forte croissance`,
                  `Tendance détectée par ALGO`,
                  `Signal viral émergent`,
                ],
              };
              const explanations =
                explanationTemplates[
                  category as keyof typeof explanationTemplates
                ] || explanationTemplates.default;
              const explanation =
                description || explanations[i % explanations.length];
              ...
              const whyItWorks =
                whyTemplates[category as keyof typeof whyTemplates] ||
                whyTemplates.default;
              ...
              const contentIdeas =
                ideasTemplates[category as keyof typeof ideasTemplates] ||
                ideasTemplates.default;
              ...
              const emotion =
                emotions[category as keyof typeof emotions] || emotions.default;
              ...
              const format =
                formats[category as keyof typeof formats] || formats.default;
              ...
              const deadline =
                score >= 85
                  ? "Poste dans les 2h"
                  : score >= 75
                    ? "Poste avant " + (now.getHours() < 18 ? "18h" : "22h")
                    : "Poste dans les 24h";
              ...
              return {
                ...
                arrivesOn:
                  platform !== "TikTok"
                    ? `TikTok dans ${Math.floor(Math.random() * 6 + 2)}h`
                    : undefined,
              };
            },
          );
          ...
          <div className="rounded-xl p-4 bg-[var(--color-violet-muted)] border border-[color-mix(in_srgb,var(--color-violet)_28%,var(--color-border))]">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-bold text-violet-300">
                Tendances suivies ({followedTrends.length})
              </span>
            </div>
            ...
            {filteredTrends.map((trend, i) => (
              ...
              >
                ...
                <div className="hidden sm:flex flex-col items-end gap-1 flex-shrink-0">
                  <MomentumPill
                    value={Math.abs(trend.growthRate)}
                    trend={trend.growthTrend}
                  />

                  {/* Time Window Badge */}
                  {(() => {
                    const window =
                      trend.growthRate > 200
                        ? "3h"
                        : trend.growthRate > 100
                          ? "6h"
                          : trend.growthRate > 50
...
function trendToActionData(trend: Trend) {
  ...
  const confidenceScore =
    trend.confidence ||
    (trend.score >= 85 && trend.growthRate > 80
      ? ("haute" as const)
      : trend.score >= 70 || trend.growthRate > 50
        ? ("moyenne" as const)
        : ("faible" as const));
  ...
  const actionHook =
    trend.actionHook || `${trend.name} — voici ce que tu dois savoir`;
  ...
  const actionDeadline =
    trend.actionDeadline ||
    (status === "hot"
      ? "Poste dans les 2h"
      : status === "rising"
        ? `Poste avant ${hour < 18 ? "18h" : "22h"}`
        : "Poste dans les 24h");

  // Build examples from contentIdeas or generate contextual ones
  const examples = trend.hookExamples?.length
    ? trend.hookExamples
            ...
            <div className="algo-sheet-surface px-4 py-3 border-t border-[var(--color-border)]">
              ...
              <AlgoSignalShareCard
                ...
                subtitle={
                  ...
                  "Tendance radar ALGO · avant saturation du feed."
                }
              />
            </div>
          </>
          ...
          <div className="px-2 pb-2 space-y-3">
            ...
            <div className="algo-sheet-surface px-3 py-3 rounded-xl border border-[var(--color-border)]">
              ...
              <AlgoSignalShareCard
                ...
                subtitle={
                  intelligenceData.analysis.psycho.emotionalHook.slice(
                    0,
                    120,
                  ) ||
                  trend.explanation?.slice(0, 100) ||
