...
export function TrendsClientShell({
  initialTrends,
  locale,
  labels,
}: TrendsClientShellProps) {
  ...
  function handleFollow(trendId: string) {
    toggle(trendId);
    track(isFollowing(trendId) ? "trend_unfollowed" : "trend_followed", {
      trendId,
      tab: activeTab,
    });
  }
  ...
  const stats = useMemo(() => {
    ...
    const postNow = trends.filter(
      (t) => t.prediction.recommendedAction === "post_now",
    ).length;
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[var(--color-bg-secondary)] to-[var(--color-bg-primary)] border border-[var(--color-border)] p-5">
        ...
        <div className="relative z-10">
          ...
          <div className="mb-3">
            <DataQualityChip
              source={
                (meta as { source?: string } | null)?.source ||
                `realtime:${getCountryFromScope()}`
              }
      ...
      {trends.length > 0 && viewMode === "list" && (
        <div
          className="space-y-2"
          role="list"
          aria-label={`Trends · ${labels.tabLabels[activeTab]}`}
        >
          {trends.map((trend, i) => (
            <RealTimeTrendCard
              key={trend.keyword || `trend-${i}`}
      ...
      {selectedTrend && (
        <TrendDetailModal
          ...
          onCopyHook={(hook) =>
            track("hook_copied", { trend: selectedTrend.keyword, hook })
          }
        />
      )}
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{
          color: rank <= 3 ? "var(--color-violet)" : "rgba(240,240,248,0.18)",
        }}
