...
import { User, Bookmark, Radar, Settings, LogIn, UserPlus, LogOut, Loader2, Info } from 'lucide-react'
    ...
    <div className="max-w-sm mx-auto px-4 py-10 space-y-8">
      ...
      <div className="flex flex-col items-center gap-3 text-center">
        <div
          ...
          style={{
            background:  isAuthenticated ? 'rgba(0,209,255,0.15)' : 'rgba(123,97,255,0.15)',
            border:      isAuthenticated ? '2px solid rgba(0,209,255,0.28)' : '2px solid rgba(123,97,255,0.28)',
          }}
        >
          {isAuthenticated ? (
            ...
            <User size={32} strokeWidth={1.4} className="text-violet-400" aria-hidden />
          )}
        </div>
        <div>
          {isAuthenticated ? (
        ...
        <ul className="space-y-2">
          {links.map(({ href, icon: Icon, label }) => (
            <li key={href}>
              ...
              >
                <Icon size={16} strokeWidth={1.6} className="text-white/30 shrink-0" aria-hidden />
