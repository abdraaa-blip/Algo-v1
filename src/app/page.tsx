...
import { CORE_NEWS_REGIONS, CORE_TREND_REGIONS } from '@/lib/geo/global-presets'
import { getSyncedRegionByIndex } from '@/lib/geo/rotation-sync'
import { MorningBriefingCard, type BriefingSignal } from '@/components/algo/MorningBriefingCard'
...
const LiveNewsSection = dynamic(
  ...
  { loading: () => <div className="h-64 rounded-xl bg-[var(--color-card)] mx-3 sm:mx-4" /> }
)
...
export default function HomePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const syncedTrendRegion = getSyncedRegionByIndex('home-trends', TREND_REGIONS)
      const syncedNewsRegion = getSyncedRegionByIndex('home-news', NEWS_REGIONS)
      ...
      const viralContentRes = await fetch('/api/viral-content?country=US&limit=12')
      ...
      if (trendsRes.ok) {
        ...
        if (trendsData?.data?.length) {
          const mapped = trendsData.data
            ...
            growthTrend: (String(t.growthTrend || 'stable') === 'up' ? 'up' : String(t.growthTrend) === 'down' ? 'down' : 'stable') as 'up' | 'down' | 'stable',
          }))
          if (mapped.length > 0) {
            setDailyHook({
              viral: mapped[0],
              early: mapped.find((t: TrendItem) => t.badge === 'Early') || mapped[Math.min(1, mapped.length - 1)],
            })
          }
        }
      }
      ...
      if (newsRes.ok) {
        ...
        if (newsData) {
          ...
          if (newsData.data?.length) {
            setNews(newsData.data.slice(0, 5).map((n: Record<string, unknown>, i: number) => {
              const sourceObj = n.source as { id?: string | null; name?: string } | string | undefined
              const sourceName = typeof sourceObj === 'string'
                ? sourceObj
      ...
      if (youtubeRes.ok) {
        ...
        if (youtubeData?.length) {
          unifiedContent = youtubeData.slice(0, 6).map((v: Record<string, unknown>) => ({
            id: String(v.id),
        ...
        const extra = Array.isArray(viralContent?.content)
          ? viralContent.content
              ...
              .map((item: Record<string, unknown>) => ({
                ...
                growthRate: 0,
                growthTrend: 'stable' as const,
                views: Number((item.metrics as { views?: number } | undefined)?.views || 0),
                badge: (Number(item.viralScore) >= 80 ? 'Viral' : Number(item.viralScore) >= 65 ? 'Early' : 'Trend') as ContentItem['badge'],
  ...
  const briefingSignals = useMemo((): BriefingSignal[] => {
    ...
    if (content[0]) {
      ...
      list.push({
        ...
        title: c0.title,
        href: `/content/${c0.id}`,
        meta: c0.platform ? `${c0.platform}${c0.viralScore ? ` · ${Math.round(c0.viralScore)}` : ''}` : undefined,
        icon: 'video',
      })
    }
    return list
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-primary)]">
        ...
        <div className="relative max-w-7xl mx-auto px-3 sm:px-4 pt-6 sm:pt-8 pb-8 sm:pb-12">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-6 sm:mb-8 algo-s2">
            ...
            <LiveIndicator
              ...
              source={dataSource.content === 'live' && dataSource.news === 'live' ? 'live' : dataSource.content === 'error' ? 'error' : 'cached'}
              onRefresh={fetchData}
              isRefreshing={isRefreshing}
            />
            <DataQualityChip
              source={dataSource.content === 'live' ? 'live multi-region feeds' : dataSource.content}
              freshness={dataSource.fetchedAt ? 'active refresh' : 'warming up'}
              confidence={dataSource.content === 'live' && dataSource.news !== 'error' ? 'high' : 'medium'}
            />
          </div>
          ...
          <PulsingCard updatedAt={dataSource.fetchedAt} intensity="high" className="mb-6 sm:mb-8 algo-s4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider" style={{ color: '#FF4D6D' }}>
                      Explose maintenant
                    </p>
                  ...
                  <p className="text-xs sm:text-sm font-semibold line-clamp-2" style={{ color: 'rgba(240,240,248,0.8)' }}>
                    {dailyHook.viral.title}
                  </p>
                  <div className="flex items-center gap-1.5 sm:gap-2 mt-2 flex-wrap">
                    <Badge type={dailyHook.viral.badge} label={dailyHook.viral.badge} />
                    <MomentumPill value={dailyHook.viral.growthRate || 0} trend={dailyHook.viral.growthTrend || 'up'} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider" style={{ color: '#00FFB2' }}>
                      Signal precoce
                    </p>
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2 mt-2 flex-wrap">
                    ...
                    <MomentumPill value={dailyHook.early.growthRate || 0} trend={dailyHook.early.growthTrend || 'up'} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 algo-s5">
            <Link
              ...
              style={{
                background: 'linear-gradient(135deg, var(--color-violet) 0%, color-mix(in srgb, var(--color-violet) 78%, white) 100%)',
                boxShadow: '0 0 32px color-mix(in srgb, var(--color-violet) 28%, transparent)',
              }}
            >
          ...
          <div className="flex items-center gap-2 sm:gap-4 overflow-x-auto pb-1 sm:pb-2 scrollbar-hide">
            ...
            {news.map((item, i) => (
              ...
              >
                ...
                <span className="max-w-[180px] sm:max-w-[280px] truncate">{item.title}</span>
                <span className="text-[9px] sm:text-[10px] hidden sm:inline" style={{ color: 'rgba(240,240,248,0.3)' }}>
                  {item.source}
                </span>
              </a>
            ))}
      ...
      <section className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-10">
        ...
        <div className="mb-3">
          <DataQualityChip
            ...
            freshness={dataSource.fetchedAt ? `updated ${new Date(dataSource.fetchedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}` : 'pending'}
            confidence={content.length >= 6 ? 'high' : content.length > 0 ? 'medium' : 'low'}
          />
        </div>
        ...
        {loading ? (
          ...
          <div className="algo-surface rounded-xl p-6 text-sm text-[var(--color-text-tertiary)]">
            Aucun signal contenu reel disponible pour le moment. Rafraichissement automatique actif.
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {content.map((item, i) => (
              ...
              >
                <ContentCard 
                  content={{ ...item, updatedAt: item.updatedAt || dataSource.fetchedAt || undefined }} 
      ...
      <section className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-10 border-t border-[var(--color-border)]">
        <h3 className="text-sm font-medium text-[var(--color-text-muted)] mb-3 sm:mb-4">Explorer</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2 sm:gap-4">
          {[
            { href: '/movies', label: 'Films', icon: '🎬', color: '#FF4D6D' },
            { href: '/music', label: 'Musique', icon: '🎵', color: '#00FFB2' },
            { href: '/news', label: 'Actualites', icon: '📰', color: '#FFD166' },
            { href: '/ai', label: 'ALGO AI', icon: '🤖', color: '#6ee7ff' },
            { href: '/viral-analyzer', label: 'Analyseur', icon: '📊', color: '#00D1FF' },
            { href: '/intelligence', label: 'Radar IA', icon: '🧠', color: '#9F7AEA' },
            { href: '/intelligence#algo-core', label: 'Core IA', icon: '⚡', color: '#c4b5fd' },
          ].map((link) => (
            <Link
              key={link.href}
              href={link.href}
            ...
            >
              ...
              <span className="text-xs sm:text-sm font-medium text-[var(--color-text-secondary)]">{link.label}</span>
            </Link>
          ))}
        </div>
      </section>
