  ...
  apis: {
    newsapi: {
      ...
      test: {
        success: boolean;
        count: number;
        source: "live" | "error" | "unconfigured";
        error?: string;
      } | null;
    };
    youtube: {
...
export default function StatusPage() {
  ...
  const [modelWeights, setModelWeights] = useState<ModelWeightsStatus | null>(
    null,
  );
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] text-sm mt-1">
              Connexions API live (NewsAPI, YouTube)
            </p>
          </div>
          <button
            onClick={fetchStatus}
        ...
        {status && (
          <div className="space-y-6">
            ...
            {/* Synthèse */}
            {(() => {
              const newsLive = Boolean(
                status.apis.newsapi.test?.success &&
                status.apis.newsapi.test?.source === "live",
              );
              const ytLive = Boolean(
                status.apis.youtube.test?.success &&
                status.apis.youtube.test?.source === "live",
              );
              const newsUn =
                status.apis.newsapi.test?.source === "unconfigured";
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm text-[var(--color-text-secondary)] mt-1">
                    ...
                    {tone === "partial" &&
                      "Au moins une source répond ; l’autre est absente ou en pause."}
                    {tone === "fault" &&
                      "Les clés sont présentes mais un appel a échoué : regarde le détail sous chaque carte."}
                  </p>
                </div>
              );
            })()}
              ...
              >
                ...
                {modelWeights.runtimeSignals && (
                  <div className="mt-3 text-xs text-[var(--color-text-secondary)]">
                    Signals · engagement:{" "}
                    {(modelWeights.runtimeSignals.engagementRate * 100).toFixed(
                      2,
                    )}
                    % · friction:{" "}
                    {(modelWeights.runtimeSignals.frictionRate * 100).toFixed(
                      2,
                    )}
                    %
                  </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            {isLive && (
              <CheckCircle size={18} className="text-green-400" aria-hidden />
            )}
            {isUnconfigured && (
              <AlertCircle size={18} className="text-amber-400" aria-hidden />
            )}
            {isFault && (
              <XCircle size={18} className="text-red-400" aria-hidden />
            )}
          </div>
          ...
          {isUnconfigured && (
            <p className="mt-3 text-xs text-[var(--color-text-secondary)]">
              ...
              · voir <span className="font-mono">.env.example</span>
            </p>
          )}
