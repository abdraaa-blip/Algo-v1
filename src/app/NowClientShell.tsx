...
const DataFlowVisualizer = dynamic(
  ...
  {
    ssr: false,
    loading: () => <div className="h-16 rounded-xl bg-white/5 mb-6" />,
  },
);
...
const LiveVideosSection = dynamic(
  () =>
    import("@/components/ui/LiveVideosSection").then(
      (m) => m.LiveVideosSection,
    ),
...
export function NowClientShell({
  ...
  const [selectedTrend, setSelectedTrend] = useState<RealTimeTrend | null>(
    null,
  );
  ...
  return (
    <>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 pb-12 space-y-8">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-1">
                  <p className="text-sm text-white/40 flex items-center gap-2">
                    ...
                    <span>
                      {meta
                        ? `${meta.totalSignals} signaux`
                        : ALGO_UI_LOADING.root}{" "}
                      &bull;{" "}
                      {isStale
                        ? "Actualisation..."
        ...
        <section
          aria-label={i18n.newsTitle}
          className="min-h-[120px] cls-stable"
        >
          {breaking.length > 0 ? (
            <>
              <SectionHeader
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                        ...
                        {n.tags.slice(0, 2).map((tag) => (
                          <span
                            key={tag}
                            className="text-[9px] text-white/28 px-1.5 py-0.5 rounded-full bg-white/4 border border-white/6"
                          >
                            {tag}
                          </span>
                        ))}
        ...
        <section
          aria-label="Signaux precoces"
          className="min-h-[280px] cls-stable"
        >
          {early.length > 1 ? (
            <>
              <SectionHeader
        ...
        <LiveVideosSection
          ...
          limit={6}
          country={
            scope.type !== "global" ? getCountryCodeFromScope(scope) : undefined
          }
        />

        {/* ── Post Now Opportunities - Reserve space to prevent CLS ─────── */}
        <section
          aria-label="Opportunites immediates"
          className="min-h-[150px] cls-stable"
        >
          {postNowOpportunities.length > 0 ? (
            <>
              <SectionHeader
        ...
        <section className="mt-8 pt-6 border-t border-white/5">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs text-white/30">
            <div className="flex items-center gap-2">
              ...
              <span>Données mises à jour périodiquement via APIs externes</span>
            </div>
