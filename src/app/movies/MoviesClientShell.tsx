...
export function MoviesClientShell({ locale, labels }: MoviesClientShellProps) {
  ...
  const [sortBy, setSortBy] = useState<
    "viralScore" | "rating" | "socialMentions"
  >("viralScore");
  ...
  const { data, isLoading } = useSWR<{
    ...
    source?: string;
    fetchedAt?: string;
  }>(`/api/movies?type=${activeType}`, fetcher, {
    ...
    <div className="min-h-0 w-full px-4 py-6 space-y-8 max-w-7xl mx-auto">
      ...
      <header className="space-y-2">
        ...
        <DataQualityChip
          ...
          confidence={
            movies.length >= 12 ? "high" : movies.length >= 5 ? "medium" : "low"
          }
        />
      </header>
            ...
            <div className="flex-1 space-y-4">
                  ...
                  <div className="flex items-center gap-3 mt-1 text-sm text-[var(--color-text-secondary)]">
                    ...
                    {featuredMovie.runtime && (
                      <>
                        <span>•</span>
                        <span>
                          {Math.floor(featuredMovie.runtime / 60)}h{" "}
                          {featuredMovie.runtime % 60}min
                        </span>
                      </>
                    )}
                    {featuredMovie.seasons && (
                      <>
                        <span>•</span>
                        <span>
                          {featuredMovie.seasons} saison
                          {featuredMovie.seasons > 1 ? "s" : ""}
                        </span>
                      </>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-6 text-sm">
                ...
                <div className="flex items-center gap-1.5 text-[var(--color-text-secondary)]">
                  <Users size={14} />
                  <span>
                    {(featuredMovie.socialMentions / 1000000).toFixed(1)}M
                    mentions
                  </span>
                </div>
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {featuredMovie.cast.slice(0, 4).map((actor) => (
                  <div
                    key={actor.name}
                    className="flex items-center gap-2 shrink-0 px-2 py-1 rounded-full bg-[var(--color-card)] border border-[var(--color-border)]"
                  >
                    <AvatarWithFallback
                      src={actor.photo}
                      name={actor.name}
...
function MovieDetailModal({
  ...
  } = useSWR<{ success: boolean; data: MovieDetailsData }>(
    `/api/movies/${movie.id}`,
    fetcher,
    ...
    >
      ...
      >
              ...
              {!hasTrailer && !detailsLoading && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-2 px-6 py-4 rounded-2xl bg-black/60 backdrop-blur-md border border-[var(--color-border)]">
                  ...
                  <span className="text-sm text-[var(--color-text-secondary)] text-center">
                    Bande-annonce indisponible pour ce film
                  </span>
                </div>
              )}
            </>
        ...
        <div className="p-4 sm:p-6 -mt-16 sm:-mt-20 relative space-y-4 sm:space-y-6">
            ...
            <div className="flex-1 space-y-2 sm:space-y-3 text-center sm:text-left">
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 sm:gap-3 text-xs sm:text-sm">
                ...
                {(details?.runtime || movie.runtime) && (
                  <span className="text-[var(--color-text-secondary)]">
                    {details?.runtime || movie.runtime} min
                  </span>
                )}
                {(details?.seasons || movie.seasons) && (
                  <span className="text-[var(--color-text-secondary)]">
                    {details?.seasons || movie.seasons} saisons
                  </span>
                )}
              ...
              <div className="flex flex-wrap justify-center sm:justify-start gap-1.5 sm:gap-2">
                {movie.genre.map((g) => (
                  <span
                    key={g}
                    className="px-2 py-0.5 sm:py-1 rounded-full bg-[var(--color-card)] border border-[var(--color-border)] text-[10px] sm:text-xs text-[var(--color-text-secondary)]"
                  >
                    {g}
                  </span>
                ))}
              ...
              <div className="flex items-center justify-center sm:justify-start gap-3">
                ...
                <div className="text-left">
                  ...
                  <div className="text-[10px] sm:text-xs text-[var(--color-text-tertiary)]">
                    {(movie.socialMentions / 1000000).toFixed(1)}M mentions
                  </div>
                </div>
              </div>
          ...
          {/* Description */}
          <div>
            <h3 className="text-sm font-medium text-[var(--color-text-tertiary)] uppercase tracking-wider mb-2">
              Synopsis
            </h3>
          ...
          {contentIdeas.length > 0 && (
            <div className="p-4 rounded-xl bg-gradient-to-br from-violet-500/10 to-fuchsia-500/10 border border-violet-500/20">
              ...
              <p className="mt-2 text-[10px] text-[var(--color-text-tertiary)] text-center">
                Clique sur une idée pour la copier
              </p>
            </div>
          )}
          ...
          {!detailsLoading && !detailsError && contentIdeas.length === 0 && (
            <div className="p-4 rounded-xl bg-gradient-to-br from-violet-500/5 to-fuchsia-500/5 border border-violet-500/10">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-bold text-violet-300/50">
                  Idées de contenu
                </h3>
              </div>
              <p className="text-sm text-[var(--color-text-tertiary)]">
                Les suggestions de contenu seront disponibles prochainement pour
                ce film.
              </p>
            </div>
          )}
          ...
          {detailsError && (
            <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-2">
              ...
              <span className="text-sm text-amber-300">
                Certains details sont temporairement indisponibles
              </span>
            </div>
          )}
          ...
          <ActionPanel
            ...
            emotion={
              movie.buzzKeywords?.includes("scandal")
                ? "colere"
                : movie.buzzKeywords?.includes("award")
                  ? "joie"
                  : "surprise"
            }
            format="reaction"
            ...
            hookSuggestion={
              contentIdeas[0] || `Mon avis honnete sur ${movie.title}...`
            }
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                {cast.map((actor) => (
                  <div key={actor.name} className="shrink-0 text-center">
                    ...
                    />
                    <div className="mt-2 text-xs font-medium text-[var(--color-text-primary)] truncate max-w-[80px]">
                      {actor.name}
                    </div>
                    <div className="text-[10px] text-[var(--color-text-tertiary)] truncate max-w-[80px]">
                      {actor.character}
                    </div>
                  </div>
                ))}
