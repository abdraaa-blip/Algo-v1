...
export const metadata: Metadata = buildPageMetadata({
  title: "Films & séries",
  description:
    "Films et séries qui font le buzz : scores, affluence et tendances culturelles.",
  path: "/movies",
    ...
    <div className="min-h-0 w-full p-4 space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {Array.from({ length: 5 }).map((_, i) => (
          <div
            key={i}
            className="h-8 w-20 bg-[var(--color-card)] rounded-full animate-pulse flex-shrink-0"
          />
        ))}
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {Array.from({ length: 10 }).map((_, i) => (
          <div
            key={i}
            className="aspect-[2/3] bg-[var(--color-card)] rounded-xl animate-pulse"
          />
        ))}
      </div>
    </div>
