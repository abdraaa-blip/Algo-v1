  ...
  try {
    ...
    for (const country of countries) {
      ...
      const found = data.data.find(
        (article: NewsArticle) => article.id === newsId,
      );
...
function guessCategory(title: string): string {
  ...
  if (/tech|ia|ai|digital|startup|innovation|robot|cyber/.test(titleLower))
    return "Tech";
  if (/sport|football|match|ligue|nba|tennis|rugby/.test(titleLower))
    return "Sport";
  if (/film|cinema|serie|netflix|disney|marvel/.test(titleLower))
    return "Entertainment";
  if (/politique|president|gouvernement|election|ministre/.test(titleLower))
    return "Politique";
  if (/economie|bourse|marche|entreprise|finance/.test(titleLower))
    return "Economie";
  if (/musique|concert|artiste|album|spotify/.test(titleLower))
    return "Musique";
...
export default async function NewsPage({ params }: Props) {
  ...
  const insight: Insight = {
    postNowProbability:
      importanceScore > 85 ? "high" : importanceScore > 70 ? "medium" : "low",
    timing,
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 pb-24">
        ...
        <div className="flex gap-4 mb-6">
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-rose-500/10 border border-rose-500/20">
            ...
            <div>
              <p className="text-[10px] text-rose-400/70 font-semibold">
                Importance
              </p>
        ...
        <InsightPanel
          ...
          labels={{
            ...
            postNow: {
              high: "Réagis maintenant !",
              medium: "Opportunité modérée",
              low: "Signal faible",
            },
            timing: {
              now: "Breaking — agis vite",
              too_late: "Sujet déjà traité",
              too_early: "Trop tôt",
            },
            ...
            postWindow: {
              ...
              saturated: "Sujet saturé",
              fading: "Intérêt en baisse",
            },
          }}
        />
          ...
          <ul className="space-y-2 text-[13px] text-[var(--color-text-secondary)]">
            ...
            {tags.length > 0 && (
              <li className="flex items-start gap-2">
                <span className="text-violet-400 mt-0.5">•</span>
                <span>
                  Utilise les hashtags: #{tags.slice(0, 3).join(" #")}
                </span>
              </li>
            )}
            <li className="flex items-start gap-2">
              ...
              <span>
                Poste d&apos;abord sur{" "}
                {insight.bestPlatform.slice(0, 2).join(" et ")}
              </span>
            </li>
          </ul>
        </section>
