...
export default function StarsPage() {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '50vh', color: '#7B61FF' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 16 }}>
        {data.map((star) => (
          <article
            key={star.id}
            className="algo-card"
            style={{ borderRadius: 12, overflow: 'hidden', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', textAlign: 'center' }}
          >
            <div style={{ position: 'relative', width: '100%', height: 180 }}>
              <ImageWithFallback
                src={star.profile_path ? `https://image.tmdb.org/t/p/w185${star.profile_path}` : null}
                alt={star.name}
                fill
            ...
            <div style={{ padding: 12 }}>
              <h3 style={{ fontSize: 13, fontWeight: 600, color: '#f0f0f8', margin: 0, lineHeight: 1.3, marginBottom: 4 }}>
                {star.name}
              </h3>
              <p style={{ fontSize: 10, color: 'rgba(240,240,248,0.38)', margin: 0 }}>
                {star.known_for_department}
              </p>
              {star.known_for?.[0] && (
                <p style={{ fontSize: 10, color: '#7B61FF', margin: '4px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
