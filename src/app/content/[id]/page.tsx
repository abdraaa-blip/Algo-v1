  ...
  try {
    ...
    return {
      ...
      explanation:
        ...
        `Video tendance sur YouTube avec ${views.toLocaleString("fr-FR")} vues.`,
      creatorTips:
        "Analyse ce format et adapte-le a ton audience dans les 24h.",
      insight: {
...
async function fetchTMDBContent(
  tmdbId: string,
  type: "movie" | "tv" = "movie",
): Promise<(Content & TMDBExtras) | null> {
  ...
  try {
    ...
    const [detailsRes, videosRes, creditsRes, similarRes] = await Promise.all([
      fetch(
        `https://api.themoviedb.org/3/${type}/${tmdbId}?api_key=${apiKey}&language=fr-FR`,
        { next: { revalidate: 21600 } },
      ),
      fetch(
        `https://api.themoviedb.org/3/${type}/${tmdbId}/videos?api_key=${apiKey}&language=fr-FR`,
        { next: { revalidate: 21600 } },
      ),
      fetch(
        `https://api.themoviedb.org/3/${type}/${tmdbId}/credits?api_key=${apiKey}&language=fr-FR`,
        { next: { revalidate: 21600 } },
      ),
      fetch(
        `https://api.themoviedb.org/3/${type}/${tmdbId}/similar?api_key=${apiKey}&language=fr-FR&page=1`,
        { next: { revalidate: 21600 } },
      ),
    ]);
    ...
    const trailer =
      allVideos.find(
        (v: { type: string; site: string; iso_639_1: string }) =>
          v.type === "Trailer" && v.site === "YouTube" && v.iso_639_1 === "fr",
      ) ||
      allVideos.find(
...
async function fetchRedditContent(postId: string): Promise<Content | null> {
  try {
    ...
    const viralScore = Math.min(
      99,
      Math.max(50, 60 + Math.log10(score + 1) * 10),
    );
    ...
    return {
      ...
      explanation:
        ...
        `Discussion Reddit avec ${score.toLocaleString("fr-FR")} upvotes.`,
      creatorTips:
        "Partage ton opinion sur ce sujet Reddit et invite au debat.",
      insight: {
...
async function fetchGitHubContent(repoName: string): Promise<Content | null> {
  try {
    ...
    const viralScore = Math.min(
      99,
      Math.max(50, 60 + Math.log10(stars + 1) * 8),
    );
    ...
    return {
      ...
      explanation:
        repo.description ||
        `Projet GitHub avec ${stars.toLocaleString("fr-FR")} etoiles.`,
...
async function fetchHackerNewsContent(
  ...
  try {
    ...
    const viralScore = Math.min(
      99,
      Math.max(50, 60 + Math.log10(score + 1) * 12),
    );
...
function generateFallbackContent(id: string): Content | null {
  const platformMap: Record<
    string,
    { platform: Platform; category: Category; url: string }
  > = {
    youtube: {
      platform: "YouTube",
      category: "Video",
    ...
    gh: {
      platform: "GitHub",
      category: "Tech",
      url: `https://github.com/${id.replace("gh_", "").replace("_", "/")}`,
    },
    reddit: {
      platform: "Reddit",
      category: "Discussion",
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const extra = [content.platform, content.category].filter(
    Boolean,
  ) as string[];
...
export default async function ContentPage({ params }: Props) {
  ...
  const badgeLabel =
    badgeLabels[badgeType as keyof typeof badgeLabels] ?? content.badge;
  ...
  const scoreHistoryData = [
    {
      timestamp: new Date(scoreHistoryAnchorMs - 5 * 86400000),
      score: Math.max(0, content.viralScore - 14),
    },
    {
      timestamp: new Date(scoreHistoryAnchorMs - 3 * 86400000),
      score: Math.max(0, content.viralScore - 9),
    },
    {
      timestamp: new Date(scoreHistoryAnchorMs - 86400000),
      score: Math.max(0, content.viralScore - 4),
    },
    ...
    <>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-[var(--color-card)]">
          ...
          {extended.trailerKey ? (
            <TrailerModal
              trailerKey={extended.trailerKey}
              title={content.title}
            />
          ) : null}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 flex-wrap text-[10px] text-white/30">
            ...
            <span>·</span>
            <time dateTime={content.detectedAt}>
              Detecte {formatRelativeTime(content.detectedAt)}
            </time>
          </div>
          ...
          <div className="mt-4 rounded-xl border border-violet-500/20 bg-violet-950/20 p-3">
            ...
            <AlgoSignalShareCard
              ...
              subtitle={
                content.explanation?.slice(0, 120) ||
                "Meta-radar ALGO · partage ton avantage."
              }
            />
          </div>
        </div>
        ...
        <InsightPanel
          ...
          watchersCount={content.watchersCount}
        ...
        {extended.cast && extended.cast.length > 0 ? (
          <CastSection cast={extended.cast} />
        ) : null}
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              {extended.voteCount ? (
                <div>
                  ...
                  <p className="text-white/70 font-semibold">
                    {extended.voteCount.toLocaleString("fr-FR")}
                  </p>
                </div>
              ) : null}
              {extended.releaseDate ? (
                <div>
                  ...
                  <p className="text-white/70 font-semibold">
                    {new Date(extended.releaseDate).toLocaleDateString("fr-FR")}
                  </p>
                </div>
              ) : null}
              {extended.runtime ? (
                ...
                <div className="col-span-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {extended.genres.map((g: string) => (
                      <span
                        key={g}
                        className="px-2 py-0.5 rounded-full bg-[var(--color-card-hover)] text-[10px] text-[var(--color-text-secondary)] font-medium border border-[var(--color-border)]"
                      >
                        {g}
                      </span>
                    ))}
            ...
            <div className="space-y-2.5">
              {content.sourceDistribution.map((s) => (
                <div key={s.platform} className="flex items-center gap-3">
                  <span className="text-xs text-white/45 w-20 shrink-0">
                    {s.platform}
                  </span>
                  ...
                  <span className="text-xs text-white/30 w-8 text-end tabular-nums">
                    {s.percentage}%
                  </span>
                </div>
              ))}
        ...
        {content.platform?.toLowerCase().includes("youtube") &&
          content.sourceUrl && (
            <section>
              <VideoPlayer
