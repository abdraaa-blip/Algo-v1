...
export function ObservabilityDashboard() {
  ...
  const globalState = useMemo(() => {
    ...
    if (a.some((x) => x.level === 'error')) return { label: 'Attention', tone: 'text-rose-300' as const }
    if (a.some((x) => x.level === 'warning')) return { label: 'Veille', tone: 'text-amber-300' as const }
    ...
    <main className="min-h-dvh text-[var(--color-text-primary)] px-4 py-8 max-w-5xl mx-auto space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-[13px] text-[var(--color-text-tertiary)]">
          Tampon process · rafraîchissement ~2,5s · dev ou <code className="text-[11px]">ALGO_OBSERVABILITY_DASHBOARD=1</code>
        </p>
      </header>

      {error ? (
        <p className="text-sm text-rose-300">Lecture snapshot impossible ({error}).</p>
      ) : null}

      <section className="algo-surface rounded-xl p-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 text-[12px]">
        <Metric label="État global" value={globalState.label} valueClass={globalState.tone} />
        <Metric
          label="Événements (1 min)"
          value={String((snapshot?.metrics1m as { totalInWindow?: number } | undefined)?.totalInWindow ?? '—')}
        />
        <Metric
          label="Erreurs / min"
          value={fmtNum((snapshot?.metrics1m as { errorsPerMinute?: number } | undefined)?.errorsPerMinute)}
        />
        <Metric
          label="Latence API moy. (1 min)"
          value={
            (snapshot?.metrics1m as { avgApiDurationMs?: number | null } | undefined)?.avgApiDurationMs != null
      ...
      <section className="algo-surface rounded-xl p-4 space-y-3">
        <h2 className="text-sm font-semibold text-[var(--color-text-secondary)]">Anomalies (heuristique)</h2>
        {(snapshot?.anomalies?.length ?? 0) === 0 ? (
          <p className="text-[12px] text-[var(--color-text-muted)]">Aucun signal sur la fenêtre récente.</p>
        ) : (
          <ul className="space-y-2 text-[12px]">
            {snapshot!.anomalies!.map((a) => (
              ...
              >
                <span className="font-mono text-[10px] opacity-70">{a.code}</span> · {a.detail}
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="algo-surface rounded-xl p-4 space-y-3">
        <div className="flex flex-wrap items-center gap-2 justify-between">
          <h2 className="text-sm font-semibold text-[var(--color-text-secondary)]">Flux de logs</h2>
          <div className="flex flex-wrap gap-1.5">
            {(['all', 'info', 'warning', 'error', 'critical'] as const).map((t) => (
              <button
                key={t}
                type="button"
        ...
        <p className="text-[10px] text-[var(--color-text-muted)]">
          Modules actifs (registre) : {(snapshot?.activeModules ?? []).join(' · ')} ·{' '}
        ...
        <ul className="max-h-[50vh] overflow-y-auto space-y-1.5 font-mono text-[11px]">
          {filtered.map((l) => (
            ...
            >
              <span className="text-[var(--color-text-muted)]">{new Date(l.timestamp).toISOString()}</span>{' '}
...
function Metric({
  ...
  return (
    <div>
      ...
      <p className={`text-lg font-semibold mt-0.5 ${valueClass ?? 'text-[var(--color-text-primary)]'}`}>{value}</p>
    </div>
  )
}
