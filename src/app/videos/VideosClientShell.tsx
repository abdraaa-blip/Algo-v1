  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-[var(--color-text-tertiary)]">
                Tendances YouTube · cache serveur ~
                {videosMeta?.refreshIntervalLabel ?? "15 min"}
                {videosMeta?.source === "fallback"
                  ? " · mode démo sans clé"
                  : ""}
              </p>
            </div>
          ...
          <div className="mt-3">
            <DataQualityChip
              ...
              freshness={
                fetchedAt
                  ? formatRelativeScopeTime(fetchedAt, scope)
                  : "pending"
              }
              confidence={
                videos.length >= 20
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
              ...
              <div className="aspect-video relative">
                ...
                <div className="absolute top-4 right-4">
                  <ViralScoreRing
                    value={Math.min(
                      99,
                      Math.round(50 + ((filtered[0].views || 0) / 1000000) * 5),
                    )}
                    size="md"
                  />
                </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {filtered.slice(1, 5).map((video, i) => (
                <VideoCard
                  key={`secondary-${video.id}-${i}`}
                  video={video}
                  animClass={STAGGER[(i + 1) % 6]}
                  scope={scope}
                />
              ))}
            </div>
      ...
      {!loading && !fetchError && filtered.length > 5 && (
        <section>
          <SectionHeader title={labels.allSectionTitle} className="mb-4" />
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
...
function VideoCard({
  ...
  const viralScore = Math.min(
    99,
    Math.round(40 + ((video.views || 0) / 500000) * 6),
  );
