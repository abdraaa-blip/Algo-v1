...
import { Globe, MapPin, Sparkles, Clapperboard, Eye, ChevronRight, Loader2, LocateFixed } from 'lucide-react'
  ...
  function finish() {
    try {
      ...
      if (country) {
        ...
        if (c) setScope({ type: 'country', code: c.code, name: getCountryName(c, lang) })
      }
    } catch { /* ignore */ }
    router.push('/')
  }
  ...
  return (
    <div className="min-h-[calc(100dvh-56px)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm space-y-8">
        ...
        {step === 1 && (
          <StepWrapper icon={Globe} title="Choisis ta langue" subtitle="Tu pourras la changer plus tard">
            <div className="space-y-2">
              {LANGS.map((l) => (
                <ChoiceButton key={l.code} selected={lang === l.code} onClick={() => setLang(l.code)}>
                  <span>{l.flag}</span>
                  <span>{l.label}</span>
                </ChoiceButton>
              ))}
        ...
        {step === 3 && (
          <StepWrapper icon={Sparkles} title="Qu&apos;est-ce qui t&apos;interesse ?" subtitle="Selectionne au moins une categorie">
        ...
        {step === 4 && (
          <StepWrapper icon={Eye} title="Comment tu utilises ALGO ?" subtitle="">
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <h1 className="text-white font-black text-xl tracking-tight mb-1">{title}</h1>
...
function ProfileCard({
  ...
  return (
    ...
    >
      <span className={cn('shrink-0 mt-0.5', selected ? 'text-violet-400' : 'text-white/28')}>
        {icon}
      </span>
      <div>
...
function CountryStep({
  lang, country, setCountry, title, subtitle,
}: {
  lang: Locale
  country: string | null
  ...
  useEffect(() => {
    if (result.countryCode && country === null) {
      const found = availableCountries.find(c => c.code === result.countryCode)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-1.5 max-h-52 overflow-y-auto pe-1">
        <ChoiceButton selected={country === null} onClick={() => setCountry(null)}>
          <Globe size={15} />
          <span>Global</span>
        </ChoiceButton>

        {availableCountries.map((c) => (
          <ChoiceButton key={c.code} selected={country === c.code} onClick={() => setCountry(c.code)}>
            <span>{c.flag}</span>
            <span>{getCountryName(c, lang)}</span>
            {result.countryCode === c.code && (
              <span className="ms-auto text-[10px] text-[#00D1FF] font-medium">Detecte</span>
            )}
          </ChoiceButton>
        ))}
      </div>
