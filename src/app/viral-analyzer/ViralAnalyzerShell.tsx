...
import { 
  Upload, Link2, FileText, Target, Sparkles, 
  ArrowRight, RefreshCw, Share2, Zap, TrendingUp,
  Play, Camera, AtSign, Video, AlertCircle,
...
export function ViralAnalyzerShell({ locale, labels }: ViralAnalyzerShellProps) {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const [predictiveRes] = await Promise.all([
        fetch('/api/intelligence/predictive?region=FR&locale=fr', { cache: 'no-store' }),
      ])
      const predictiveJson = predictiveRes.ok ? await predictiveRes.json() : null
      const predictiveScore = Number(predictiveJson?.data?.predictedViralityScore || 0)
      const topCategory = String(predictiveJson?.data?.drivers?.topCategory || 'general')
      ...
      const mapped: AnalysisResult = {
        ...
        scoreInsights: [
          { label: 'Hook', value: Number(data?.hookScore || 0), explanation: getExplanation('Hook', Number(data?.hookScore || 0)) },
          { label: 'Trend', value: Number(data?.trendScore || 0), explanation: getExplanation('Trend', Number(data?.trendScore || 0)) },
          { label: 'Format', value: Number(data?.formatScore || 0), explanation: getExplanation('Format', Number(data?.formatScore || 0)) },
          { label: 'Emotion', value: Number(data?.emotionScore || 0), explanation: getExplanation('Emotion', Number(data?.emotionScore || 0)) },
          { label: 'Timing', value: Number(data?.timingScore || 0), explanation: getExplanation('Timing', Number(data?.timingScore || 0)) },
        ],
      ...
      setAnalysisMeta({
        ...
        freshness: new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
  ...
  const canAnalyze =
    mode === 'url' ? url.length > 5 : mode === 'upload' ? Boolean(uploadedFile) : description.length > 20
    ...
    <div className="min-h-screen bg-gradient-to-b from-[var(--color-bg-secondary)] to-[var(--color-bg-primary)]">
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-8">
        ...
        <header className="text-center space-y-3">
          ...
          <p className="text-[var(--color-text-secondary)] text-sm max-w-md mx-auto">{labels.subtitle}</p>
              ...
              <div className="rounded-2xl bg-[var(--color-card)] border border-[var(--color-border)] p-6 space-y-4">
                ...
                {mode === 'upload' && (
                  ...
                  >
                    ...
                    <p className="text-[var(--color-text-secondary)] text-sm">{labels.uploadTitle}</p>
                    <p className="text-[var(--color-text-muted)] text-xs mt-1">{labels.uploadSubtitle}</p>
                    <input
                      ref={fileInputRef}
                      type="file"
                      accept="video/*,image/*"
            ...
            >
              ...
              <div className="rounded-2xl bg-gradient-to-br from-violet-500/10 to-fuchsia-500/10 border border-violet-500/20 p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">{labels.results}</h2>
                    <p className="text-[var(--color-text-secondary)] text-sm">Potentiel viral analyse</p>
                  </div>
                ...
                <div className="grid grid-cols-5 gap-2">
                  {Object.entries(result.breakdown).map(([key, value]) => (
                    <div key={key} className="text-center p-3 rounded-xl bg-[var(--color-card)] border border-[var(--color-border)]">
                      <div className="text-lg font-bold text-[var(--color-text-primary)]">{value}</div>
                      <div className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider">{key}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="flex items-center gap-2 p-3 rounded-xl bg-[var(--color-card)] border border-[var(--color-border)]">
                    ...
                    <div>
                      <div className="text-xs text-[var(--color-text-tertiary)]">Portee estimee</div>
                      <div className="text-sm font-semibold text-[var(--color-text-primary)]">{result.estimatedReach}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 p-3 rounded-xl bg-[var(--color-card)] border border-[var(--color-border)]">
                    <Clock size={16} className="text-sky-400" />
                    <div>
                      <div className="text-xs text-[var(--color-text-tertiary)]">Meilleur moment</div>
                      <div className="text-sm font-semibold text-[var(--color-text-primary)]">{result.optimalPostTime}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 p-3 rounded-xl bg-[var(--color-card)] border border-[var(--color-border)]">
                    <Zap size={16} className="text-amber-400" />
                    <div>
                      <div className="text-xs text-[var(--color-text-tertiary)]">Plateforme</div>
                      <div className="text-sm font-semibold text-[var(--color-text-primary)] capitalize">{result.bestPlatform}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-[var(--color-card)] border border-[var(--color-border)] p-6 space-y-4">
                ...
                {result.modelTelemetry && (
                  <div className="pt-2 border-t border-[var(--color-border)] text-xs text-[var(--color-text-secondary)]">
                    ...
                    {result.modelTelemetry.rollbackApplied ? ' · rollback applique' : ' · mode adaptatif actif'}
                  </div>
                )}
              </div>

              <div className="rounded-2xl bg-[var(--color-card)] border border-[var(--color-border)] p-6 space-y-3">
                <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">Explications detaillees du score</h3>
                <div className="space-y-2">
                  {result.scoreInsights.map((item) => (
                    <div key={item.label} className="p-2 rounded-lg bg-[var(--color-bg-elevated)] border border-[var(--color-border)]">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-sm text-[var(--color-text-secondary)]">{item.label}</span>
                        <span className="text-xs font-bold text-violet-300">{item.value}</span>
                      </div>
                      <p className="text-xs text-[var(--color-text-secondary)] mt-1">{item.explanation}</p>
                    </div>
                  ))}
                </div>
              </div>
