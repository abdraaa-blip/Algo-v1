...
function runScoreAnimation(target: number, setAnimatingScore: (n: number) => void) {
  let current = 0
...
export default function ViralAnalyzerPage() {
  ...
  const [descriptionPlatform, setDescriptionPlatform] = useState<ApiPlatform>('youtube')
  ...
  const analyzeContent = useCallback(async () => {
    if ((inputType === 'url' && !url.trim()) || (inputType === 'text' && !description.trim())) return
    ...
    <main className="min-h-screen pb-20">
      <section className="relative overflow-hidden border-b border-[var(--color-border)]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 pt-8 pb-6">
          <Link href="/creator-mode" className="inline-flex items-center gap-2 text-xs mb-4 transition-colors hover:text-white" style={{ color: 'rgba(240,240,248,0.4)' }}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
            Mode Createur
          </Link>
          <h1 className="text-2xl md:text-3xl font-black mb-2" style={{ color: '#f0f0f8' }}>
            Viral Analyzer
          </h1>
          ...
          <div
            ...
            role="note"
          >
            <strong className="font-semibold text-emerald-200">Transparence :</strong> les scores viennent du moteur{' '}
            <code className="text-emerald-200/80">POST /api/viral-analyzer</code> (poids ALGO + signaux tendance en
            cache). Ce sont des <strong>estimations</strong>, pas une prévision de performance réelle.
          </div>
        </div>
      </section>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 space-y-3">
          {inputType === 'url' ? (
            <>
              ...
              />
              {url.trim() ? (
                <p className="text-[11px]" style={{ color: 'rgba(240,240,248,0.45)' }}>
                  Plateforme détectée :{' '}
                  <span className="font-semibold text-[#a78bfa]">{formatPlatformLabel(inferredUrlPlatform)}</span>
                </p>
              ) : null}
            </>
          ) : (
            <>
              ...
              />
              <div>
                <label className="block text-[11px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'rgba(240,240,248,0.45)' }}>
                  Plateforme cible
                </label>
                <select
                  value={descriptionPlatform}
                  onChange={e => setDescriptionPlatform(e.target.value as ApiPlatform)}
                  className="algo-input-field focus:ring-0"
                >
        ...
        <button
          ...
          style={{
            background: 'linear-gradient(135deg, var(--color-violet), var(--color-blue-neon))',
            boxShadow: '0 0 28px color-mix(in srgb, var(--color-violet) 26%, transparent)',
          }}
        >
        ...
        {analysisError ? (
          <p className="mt-4 text-sm rounded-xl px-4 py-3 border border-red-400/30 bg-red-500/10 text-red-200/90" role="alert">
            {analysisError}
          </p>
        ) : null}

        {analyzing && <div className="mt-8"><AlgoLoader message={ALGO_UI_LOADING.viralPage} /></div>}

        {result && !analyzing && (
          <div className="mt-8 space-y-6">
            <div className="p-8 rounded-2xl text-center" style={{ background: 'linear-gradient(135deg, rgba(123,97,255,0.15), rgba(0,209,255,0.1))', border: '1px solid rgba(123,97,255,0.2)' }}>
              <p className="text-xs font-bold uppercase tracking-wider mb-4" style={{ color: 'rgba(240,240,248,0.5)' }}>Potentiel Viral</p>
              <div className="flex justify-center mb-4"><ViralScoreRing score={animatingScore} size={120} /></div>
              <p className="text-lg font-bold" style={{ color: '#f0f0f8' }}>
                {result.viralScore >= 80 ? 'Excellent potentiel!' : result.viralScore >= 60 ? 'Bon potentiel' : 'A ameliorer'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: 'Tendances', score: result.trendAlignment, color: '#00FFB2' },
                { label: 'Format', score: result.formatScore, color: '#00D1FF' },
                { label: 'Timing', score: result.timingScore, color: '#FFD166' },
              ].map(item => (
                <div key={item.label} className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs" style={{ color: 'rgba(240,240,248,0.5)' }}>{item.label}</span>
            ...
            <div className="p-5 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: 'rgba(240,240,248,0.5)' }}>Recommandations</p>
              <div className="flex flex-wrap gap-2 mb-4">
                {result.bestPlatform.map(p => (
                  <span key={p} className="px-3 py-1 rounded-full text-xs font-bold" style={{ background: 'rgba(123,97,255,0.15)', color: '#a78bfa' }}>{p}</span>
                ))}
                <span className="px-3 py-1 rounded-full text-xs font-bold" style={{ background: 'rgba(0,255,178,0.15)', color: '#00FFB2' }}>{result.bestTime}</span>
              </div>
              <ul className="space-y-2">
                {result.tips.map((tip, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm" style={{ color: 'rgba(240,240,248,0.6)' }}>
                    <span className="text-[var(--color-violet)]">{'\u2022'}</span>{tip}
                  </li>
                ))}
              </ul>
            </div>
            ...
            >
              <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: 'rgba(240,240,248,0.45)' }}>
                Partager l’analyse
              </p>
              <AlgoSignalShareCard
                headline={
                ...
                badgeLabel={
                  result.viralScore >= 80 ? 'Fort potentiel' : result.viralScore >= 60 ? 'Bon potentiel' : 'À booster'
                }
