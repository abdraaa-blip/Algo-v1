...
function transformToContent(
  ...
  const views = Number(
    item.viewCount || item.views || item.score || item.stars || 0,
  );
  ...
  const tipsPerCategory: Record<string, string[]> = {
    Tech: [
      ...
      "Montre des démos visuelles",
      "Compare avec les alternatives",
    ],
    Cinema: [
      "Réagis authentiquement",
      "Ajoute des théories",
      "Évite les spoilers majeurs",
    ],
    Musique: [
      "Utilise des extraits audio",
      "Partage ton ressenti",
      "Lie à l'actualité de l'artiste",
    ],
    Discussion: [
      "Donne ton opinion tranchée",
      "Invite au débat",
      "Reste respectueux",
    ],
  ...
  return {
    ...
    insight: {
      postNowProbability:
        viralScore >= 85 ? "high" : viralScore >= 75 ? "medium" : "low",
      timing,
      ...
      reproductionIdea:
        reproductionTemplates[index % reproductionTemplates.length],
...
export default function CreatorModePage() {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR("/api/live?limit=20", fetcher, {
    ...
    <main className="min-h-screen pb-20">
      <section className="relative overflow-hidden border-b border-[var(--color-border)]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 pt-8 pb-6">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-xs mb-4 text-[var(--color-text-muted)] transition-colors duration-200 hover:text-[var(--color-text-primary)]"
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <p className="text-sm text-[var(--color-text-secondary)]">
            Signaux agrégés depuis plusieurs flux : les scores sont des{" "}
            <strong className="font-semibold text-white/70">indicateurs</strong>{" "}
            pour prioriser et tester · pas une garantie d’audience. Export et
            partage pour documenter ta veille.
          </p>
          <p className="text-xs mt-2 text-[var(--color-text-tertiary)]">
            ...
            · analyse multi-facteurs, simulations et priorités sur les flux live
            (radar).
          </p>
        </div>
      </section>
            ...
            {liveData?.success ? (
              <>
                ...
                <span className="text-[10px] text-white/30">
                  {liveData?.sources
                    ? `${Object.keys(liveData.sources).filter((k) => liveData.sources[k]?.status === "success").length} sources (MAJ 5 min)`
                    : ""}
                </span>
              </>
            ) : liveError ? (
              ...
              <>
                ...
                <span className="text-xs text-yellow-500">
                  {ALGO_UI_LOADING.creatorTrending}
                </span>
              </>
            )}
          </div>
            ...
            <div>
                ...
                {content.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <ViralScoreRing
                        score={item.viralScore || item.score || 75}
                        size={40}
                      />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <Badge
                            type={item.badge || "Trend"}
                            label={item.badge || "Trend"}
                          />
                        </div>
                      </div>
                    </div>
              ...
              >
                <span className="text-sm font-bold">
                  Analyse ton propre contenu
                </span>
                <span className="block text-xs mt-1 text-[var(--color-text-tertiary)]">
                  Viral Analyzer
                </span>
              </Link>
            </div>
              ...
              {selectedContent ? (
                <div className="space-y-4">
                  ...
                  {selectedContent.insight && (
                    <InsightPanel
                      insight={adaptCreatorPageInsightToFull(
                        selectedContent.insight,
                      )}
                      watchersCount={Math.floor(
                        (selectedContent.views || 0) / 100,
                      )}
