...
export default function SettingsPage() {
  ...
  const [billingFlash, setBillingFlash] = useState<"success" | "cancel" | null>(
    null,
  );
  ...
  return (
    <div className="max-w-sm mx-auto px-4 py-6 space-y-8">
      ...
      <section aria-labelledby="lang-heading">
        <h2
          id="lang-heading"
          className="text-[10px] font-bold text-white/28 uppercase tracking-[0.12em] mb-3"
        >
          Langue
        </h2>
      ...
      >
        <h2
          id="billing-heading"
          className="text-[10px] font-bold text-white/28 uppercase tracking-[0.12em]"
        >
          {ALGO_BILLING.sectionTitle}
        </h2>
        {billingFlash === "success" ? (
          <p
            className="text-xs text-emerald-400/95 leading-relaxed"
            role="status"
          >
        ...
        <p className="text-xs text-white/55 leading-relaxed">
          {ALGO_BILLING.valuePitch}
        </p>
        <p className="text-[11px] font-medium text-violet-200/90">
          {billingPlan === "pro"
            ? ALGO_BILLING.planLabelPro
            : ALGO_BILLING.planLabelFree}
        </p>
        <p className="text-[11px] text-white/40 leading-relaxed">
          {ALGO_BILLING.freeLabel}
        </p>
        <p className="text-[11px] text-white/40 leading-relaxed">
          {ALGO_BILLING.proPitch}
        </p>
        {portalAvailable ? (
          <button
            type="button"
        ...
        {checkoutAvailable && billingPlan === "pro" ? (
          ...
          <p className="text-[11px] text-white/35 leading-relaxed">
            {ALGO_BILLING.checkoutUnavailable}
          </p>
        )}
        {checkoutMessage ? (
      ...
      <section aria-labelledby="zone-heading">
        ...
        <div className="space-y-1.5 max-h-72 overflow-y-auto pe-1">
          ...
          {/* Regions */}
          <div className="pt-2">
            <p className="text-[9px] font-bold text-white/20 uppercase tracking-wider mb-1.5 px-1">
              Regions
            </p>
            {availableRegions.map((r) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={isActive}
                  onClick={() =>
                    setScope({ type: "region", code: r.code, name })
                  }
                  className={cn(
            ...
            {availableCountries.map((c) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={isActive}
                  onClick={() =>
                    setScope({ type: "country", code: c.code, name })
                  }
                  className={cn(
                    "w-full flex items-center gap-3 px-4 py-2.5 rounded-xl border text-sm font-semibold text-start mb-1.5",
                    "transition-all duration-150",
