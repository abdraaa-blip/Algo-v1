...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-viral-analyzer:${identifier}`, {
    limit: 18,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    if (supabaseServiceRoleConfigured()) {
      const subjectKey = createHash("sha256")
        .update(
          `${input.platform}:${input.mode}:${input.url || ""}:${contentData.topic.slice(0, 240)}`,
        )
        .digest("hex")
        .slice(0, 48);
      void persistViralScoreSnapshot({
        ...
        confidence:
          result.modelTelemetry?.adaptiveSignals != null ? 0.82 : 0.55,
        payload: {
    ...
    if (response.ok) {
      ...
      if (video) {
        ...
        return {
          ...
          keywords:
            snippet.tags ||
            extractKeywordsFromText(snippet.title + " " + snippet.description),
...
async function getTrendData(locale: string): Promise<TrendData> {
  ...
  try {
    const ytResponse = await fetch(
      `${process.env.NEXT_PUBLIC_URL || ""}/api/live-videos?country=${country}`,
    );
    if (ytResponse.ok) {
      const ytData = await ytResponse.json();
      youtubeSearches =
        ytData.data?.slice(0, 10).map((v: { title: string }) => v.title) || [];
    }
  } catch {
    // Use cache fallback
  }
...
async function analyzeContent(
  ...
  return {
    ...
    recommendations,
    trendingTopics: [
      ...trends.googleTrends.slice(0, 5),
      ...trends.youtubeSearches.slice(0, 5),
    ],
...
async function getAdaptiveSignals(): Promise<{
  engagementRate: number;
  frictionRate: number;
} | null> {
  const baseUrl =
...
function recommendBestPlatform(
  content: ContentData,
...
function generateHashtags(content: ContentData, trends: TrendData): string[] {
  ...
  hashtags.push(
    ...content.keywords.slice(0, 3).map((k) => k.replace(/\s+/g, "")),
  );

  // Add trending topics
  hashtags.push(
    ...trends.googleTrends.slice(0, 2).map((t) => t.replace(/\s+/g, "")),
  );
...
function estimateReach(
  ...
  const base =
    baseReach[platform as keyof typeof baseReach] || baseReach.youtube;
...
function generateCompetitorInsights(
  ...
  if (trends.youtubeSearches.length > 0) {
    insights.push(
      `Les videos "${trends.youtubeSearches[0]}" performent bien en ce moment`,
    );
  }

  if (trends.googleTrends.length > 0) {
    insights.push(
      `Le sujet "${trends.googleTrends[0]}" est en tendance sur Google`,
    );
  }

  insights.push(
    "Les formats courts (< 60s) ont 2x plus de chances de devenir viraux",
  );
...
function detectYouTubeFormat(snippet: {
  title?: string;
  description?: string;
}): string {
  const text = (
