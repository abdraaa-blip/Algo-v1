...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-model-weights:${identifier}`, {
    limit: 90,
    windowMs: 60_000,
  });
  ...
  const hasSignals =
    Number.isFinite(engagementRate) && Number.isFinite(frictionRate);
  ...
  return NextResponse.json({
    ...
    rollbackRule:
      "frictionRate > 0.20 OR (frictionRate > 0.12 AND engagementRate < 0.10)",
    history: {
