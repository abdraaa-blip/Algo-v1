...
function generateViralContent(scope: AppScope): Content[] {
  ...
  const viralPatterns = {
    ...
    Category,
    { titles: string[]; platforms: Platform[]; formats: ContentFormat[] }
  >;
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const title =
      pattern.titles[Math.floor(Math.random() * pattern.titles.length)];
    const platform =
      pattern.platforms[Math.floor(Math.random() * pattern.platforms.length)];
    const format =
      pattern.formats[Math.floor(Math.random() * pattern.formats.length)];
    ...
    content.push({
      ...
      insight: {
        postNowProbability:
          viralScore > 85 ? "high" : viralScore > 70 ? "medium" : "low",
        timing: "now",
      ...
      sourceDistribution: [
        ...
        {
          platform: pattern.platforms[1] || "YouTube",
          percentage: 40,
          momentum: "medium",
        },
      ],
...
export async function GET(request: Request) {
  ...
  const rateLimit = checkRateLimit(`api-content:${identifier}`, {
    limit: 90,
    windowMs: 60_000,
  });
