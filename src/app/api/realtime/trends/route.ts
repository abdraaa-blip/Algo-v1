...
const cache = {
  news: new Map<
    string,
    { data: NewsArticle[]; fetchedAt: string; expiresAt: string }
  >(),
  videos: new Map<
    string,
    { data: VideoItem[]; fetchedAt: string; expiresAt: string }
  >(),
};
...
async function fetchNewsForCountry(
  country: string,
): Promise<{ data: NewsArticle[]; source: "live" | "cached" | "fallback" }> {
...
async function fetchVideosForCountry(
  country: string,
): Promise<{ data: VideoItem[]; source: "live" | "cached" | "fallback" }> {
...
function newsToTrend(
  ...
  return {
    ...
    score: {
      ...
      tier,
      breakdown: {
        volume: Math.min(100, Math.floor(velocity * 0.9)),
        velocity,
        engagement: Math.floor(velocity * 0.8),
        growth: Math.floor(velocity * 0.85),
      },
    },
    prediction: {
      trajectory:
        velocity > 75 ? "rising" : velocity > 55 ? "stable" : "fading",
      confidence: Math.min(95, velocity + 10),
      peakIn: velocity > 80 ? "2-4h" : velocity > 65 ? "6-12h" : "24h+",
      reason: `Breaking news - ${sourceName}`,
      optimalWindow:
        velocity > 75 ? ["Now", "30min", "1h"] : ["2h", "4h", "8h"],
    },
    signals: [
      {
        id: `news-${index}`,
        source: "news",
        keyword: article.title.slice(0, 40),
        velocity,
        volume: Math.floor(50000 + Math.random() * 200000),
    ...
    thumbnail:
      article.urlToImage ||
      `https://ui-avatars.com/api/?name=${encodeURIComponent(sourceName)}&background=7c3aed&color=fff&size=640`,
    author: sourceName,
...
function videoToTrend(
  ...
  return {
    ...
    score: {
      ...
      tier,
      breakdown: {
        volume: Math.min(100, Math.floor(views / 100000)),
        velocity,
        engagement: Math.min(100, growthRate),
        growth: Math.floor(velocity * 0.9),
      },
    },
    prediction: {
      trajectory:
      ...
      confidence: Math.min(95, velocity + 10),
      peakIn: velocity > 80 ? "2-4h" : velocity > 65 ? "6-12h" : "24h+",
      reason: `${viewsFormatted} views on YouTube`,
      optimalWindow:
        velocity > 75 ? ["Now", "30min", "1h"] : ["2h", "4h", "8h"],
    },
    signals: [
      {
        id: `yt-${index}`,
        source: "youtube",
        keyword: video.snippet.title.slice(0, 40),
        velocity,
        volume: views,
        timestamp: publishedTime,
      },
...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-realtime-trends:${identifier}`, {
    limit: 45,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    const overallSource =
      newsSource === "live" && videoSource === "live"
        ? "live"
        : newsSource === "fallback" || videoSource === "fallback"
          ? "partial"
          : "cached";
    ...
    return NextResponse.json(
      {
        ...
        meta: {
          ...
          avgViralScore: Math.round(
            limitedTrends.reduce((acc, t) => acc + t.score.total, 0) /
              Math.max(1, limitedTrends.length),
          ),
          topTier: limitedTrends.filter(
            (t) => t.score.tier === "S" || t.score.tier === "A",
          ).length,
          sources: [
            {
              id: "youtube",
          ...
          liveSourcesCount:
            (newsSource === "live" ? 1 : 0) + (videoSource === "live" ? 1 : 0),
