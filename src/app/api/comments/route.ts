...
const MOCK_COMMENTS: Record<string, Comment[]> = {
  "content-1": [
      ...
      replies: [
        {
          ...
          user_avatar:
            "https://api.dicebear.com/7.x/avataaars/svg?seed=TrendWatcher",
          user_verified: false,
    ...
    },
    {
      ...
      user_avatar:
        "https://api.dicebear.com/7.x/avataaars/svg?seed=ViralHunter",
      user_verified: false,
...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-comments-get:${identifier}`, {
    limit: 90,
    windowMs: 60_000,
  });
...
export async function POST(request: NextRequest) {
  const identifier = getClientIdentifier(request);
  const rateLimit = checkRateLimit(`api-comments-post:${identifier}`, {
    limit: 24,
    windowMs: 60_000,
  });
  if (!rateLimit.success) {
