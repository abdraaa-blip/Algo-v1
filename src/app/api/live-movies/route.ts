...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-live-movies:${identifier}`, {
    limit: 60,
    windowMs: 60_000,
  });
  ...
  try {
    switch (type) {
      case "movie": {
        const moviesResult = sliceCachedData(
          await fetchTrendingMovies(timeWindow, region),
          listLimit,
        );
        return NextResponse.json({
          success: true,
      ...
      case "tv": {
        const tvResult = sliceCachedData(
          await fetchTrendingTV(timeWindow, region),
          listLimit,
        );
        return NextResponse.json({
          success: true,
      ...
      case "person": {
        const peopleResult = sliceCachedData(
          await fetchTrendingPeople(timeWindow),
          listLimit,
        );
        return NextResponse.json({
          success: true,
      ...
      case "now_playing": {
        const nowPlayingResult = sliceCachedData(
          await fetchNowPlaying(region),
          listLimit,
        );
        return NextResponse.json({
          success: true,
      ...
      case "upcoming": {
        const upcomingResult = sliceCachedData(
          await fetchUpcoming(region),
          listLimit,
        );
        return NextResponse.json({
          success: true,
