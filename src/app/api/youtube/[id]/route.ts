...
export async function GET(
  ...
  const rateLimit = checkRateLimit(`api-youtube-id:${identifier}`, {
    limit: 90,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    const [videoRes, relatedRes] = await Promise.all([
      ...
      fetch(
        `https://www.googleapis.com/youtube/v3/search?part=snippet&relatedToVideoId=${videoId}&type=video&maxResults=6&key=${apiKey}`,
      ),
    ]);
