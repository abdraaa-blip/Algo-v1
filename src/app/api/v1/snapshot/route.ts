...
import { fetchSnapshotBundle, parseSnapshotQuery } from '@/lib/ecosystem/snapshot-store'
...
export async function GET(request: NextRequest) {
  ...
  if (!parsed.ok) {
    return NextResponse.json(
      { success: false, error: parsed.error, ecosystemApiVersion: ALGO_ECOSYSTEM_API_VERSION },
      { status: 400 }
    )
  }
  ...
  return NextResponse.json(result.data, { headers: createRateLimitHeaders(rateLimit) })
}
