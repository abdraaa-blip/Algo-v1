...
export async function GET(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "Platform API keys not configured (ALGO_PLATFORM_API_KEY or ALGO_PLATFORM_API_KEYS)",
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Unauthorized",
        hint: "Use Authorization: Bearer <key> or X-ALGO-Platform-Key",
      },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    const headers: Record<string, string> = {
      ...Object.fromEntries(
        Object.entries(createRateLimitHeaders(rateLimit)).map(([k, v]) => [
          k,
          String(v),
        ]),
      ),
