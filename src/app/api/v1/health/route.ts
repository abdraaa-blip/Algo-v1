...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-v1-health:${identifier}`, { limit: 300, windowMs: 60_000 })
  if (!rateLimit.success) {
    return NextResponse.json(
      { ok: false, error: 'Rate limit exceeded', retryAfter: rateLimit.retryAfter, ecosystemApiVersion: ALGO_ECOSYSTEM_API_VERSION },
