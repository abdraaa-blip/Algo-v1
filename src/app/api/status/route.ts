...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-status:${identifier}`, {
    limit: 24,
    windowMs: 60_000,
  });
  ...
  const newsOk =
    results.apis.newsapi.test?.success &&
    results.apis.newsapi.test?.source === "live";
  const youtubeOk =
    results.apis.youtube.test?.success &&
    results.apis.youtube.test?.source === "live";
