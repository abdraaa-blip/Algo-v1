...
function mapDepartmentToCategory(
  ...
  if (dept.includes("directing") || dept.includes("production"))
    return "creator";
...
function transformToRisingStar(person: RealCelebrity) {
  ...
  return {
    ...
    avatar_url:
      person.profileUrl ||
      `https://ui-avatars.com/api/?name=${encodeURIComponent(person.name)}&background=8b5cf6&color=fff&size=300&bold=true`,
    ...
    best_video:
      person.knownFor.length > 0
        ? {
            ...
            thumbnail:
              person.profileUrl ||
              `https://placehold.co/480x360/1a1a2e/8b5cf6?text=${encodeURIComponent(person.name)}`,
    ...
    top_platforms: [
      {
        platform: "tmdb",
        followers: Math.floor(popularity * 50000),
    ...
    algo_prediction: isExploding
      ? "Trending on TMDB - high visibility"
      : "Rising popularity",
  };
}
...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-rising-stars:${identifier}`, {
    limit: 60,
    windowMs: 60_000,
  });
