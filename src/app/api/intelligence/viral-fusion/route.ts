...
const fusionCache = new Map<string, { expiresAtMs: number; payload: unknown }>()
...
export async function GET(request: NextRequest) {
  ...
  const requireProForYoutube = process.env.ALGO_FUSION_YOUTUBE_REQUIRE_PRO === '1'
  ...
  const cacheKey = requireProForYoutube ? `${region}:${snap?.plan ?? 'free'}` : region
  ...
  try {
    ...
    fusionCache.set(cacheKey, { expiresAtMs: nowMs + FUSION_CACHE_TTL_MS, payload })
    return NextResponse.json(
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'viral-fusion build failed',
      },
