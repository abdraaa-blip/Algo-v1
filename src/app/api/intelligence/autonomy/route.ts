...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`intelligence-autonomy-get:${identifier}`, {
    limit: 120,
    windowMs: 60_000,
  });
...
export async function PATCH(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(
    `intelligence-autonomy-patch:${identifier}`,
  ...
  try {
    ...
    const policy = updateAutonomyPolicy({
      ...
      ...(typeof body.killSwitch === "boolean"
        ...
        : {}),
      ...(typeof body.minConfidenceForAuto === "number"
        ? { minConfidenceForAuto: body.minConfidenceForAuto }
        : {}),
    });
