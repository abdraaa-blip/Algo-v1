...
export async function GET(request: NextRequest) {
  ...
  const alerts: Array<{ type: 'drift' | 'feedback' | 'stability'; severity: 'low' | 'medium' | 'high'; message: string }> = []
  ...
  if (latest) {
    if (latest.wrongRatio >= 0.35) {
      alerts.push({
        type: 'feedback',
        severity: 'high',
        message: 'Wrong feedback ratio is high. Consider tightening auto-execution threshold.',
      })
    } else if (latest.wrongRatio >= 0.2) {
      alerts.push({
        type: 'feedback',
        severity: 'medium',
        message: 'Wrong feedback ratio is increasing. Monitor autonomy policy drift.',
      })
    }
  }

  if (latest && prev) {
    const thresholdJump = Math.abs(latest.nextMinConfidence - prev.nextMinConfidence)
