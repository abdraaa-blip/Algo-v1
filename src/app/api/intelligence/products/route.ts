...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`intelligence-products:${identifier}`, {
    limit: 60,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    const products = PRODUCT_KEYWORDS.map((keyword) => {
      ...
      const potential = (
        score >= 75 ? "high" : score >= 55 ? "medium" : "emerging"
      ) as "high" | "medium" | "emerging";
      return {
        keyword,
        score,
        potential,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        locale,
        products,
        note: "Signals are derived from public trends and first-party engagement. No private marketplace data is collected without explicit access.",
        missionFr:
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to build product opportunities",
      },
      { status: 500 },
    );
  }
