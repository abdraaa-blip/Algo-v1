...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`intelligence-health:${identifier}`, {
    limit: 40,
    windowMs: 60_000,
  });
  ...
  try {
    const [newsH, musicH, videosH, trendsH, creatorH] = await Promise.all([
      fetch(`${origin}/api/live-news?country=${newsRegion}`, {
        cache: "no-store",
      }),
      fetch(`${origin}/api/live-music?type=all&country=${region}`, {
        cache: "no-store",
      }),
      fetch(`${origin}/api/live-videos?country=${region}`, {
        cache: "no-store",
      }),
      fetch(`${origin}/api/live-trends?country=${region}`, {
        cache: "no-store",
      }),
      fetch(`${origin}/api/live?limit=5`, { cache: "no-store" }),
    ]);

    const [newsJson, musicJson, videosJson, trendsJson, creatorJson] =
      await Promise.all([
        newsH.json(),
        musicH.json(),
        videosH.json(),
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to build intelligence health snapshot",
      },
      { status: 500 },
    );
  }
