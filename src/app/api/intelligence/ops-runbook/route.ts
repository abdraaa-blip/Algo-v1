...
import { pushIncident, persistIncident } from '@/lib/intelligence/ops-incidents'
...
function isAuthorized(request: NextRequest) {
  ...
  return (opsToken && providedOps === opsToken) || (cronSecret && authHeader === `Bearer ${cronSecret}`)
}
...
function applyRunbook() {
  ...
  if (snapshot.alerts.some((a) => a.id === 'learning-drift')) {
    updateAutonomyPolicy({
      mode: 'advisory',
      minConfidenceForAuto: Math.max(snapshot.policy.minConfidenceForAuto, 0.82),
    })
  ...
  if (snapshot.alerts.some((a) => a.id === 'resilience-open-circuit')) {
    updateAutonomyPolicy({
      minConfidenceForAuto: Math.max(snapshot.policy.minConfidenceForAuto, 0.78),
    })
  ...
  if (actions.length > 0) {
    const incident = pushIncident({
      severity: snapshot.alerts.some((a) => a.severity === 'high') ? 'high' : 'medium',
...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`intelligence-ops-runbook-get:${identifier}`, { limit: 60, windowMs: 60_000 })
  ...
  return NextResponse.json({ success: true, dryRun: true, snapshot: collectOpsStatus() })
}
...
export async function POST(request: NextRequest) {
  ...
  const identifier = getClientIdentifier(request)
  const rateLimit = checkRateLimit(`intelligence-ops-runbook-post:${identifier}`, { limit: 20, windowMs: 60_000 })
  if (!rateLimit.success) {
    return NextResponse.json(
      { success: false, error: 'Rate limit exceeded', retryAfter: rateLimit.retryAfter },
      { status: 429, headers: createRateLimitHeaders(rateLimit) }
