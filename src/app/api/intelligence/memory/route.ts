...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`intelligence-memory-get:${identifier}`, {
    limit: 60,
    windowMs: 60_000,
  });
...
export async function POST(request: NextRequest) {
  const identifier = getClientIdentifier(request);
  const rateLimit = checkRateLimit(`intelligence-memory-post:${identifier}`, {
    limit: 30,
    windowMs: 60_000,
  });
  if (!rateLimit.success) {
  ...
  try {
    ...
    if (!body?.region || !body?.domain || !body?.summary) {
      return NextResponse.json(
        { success: false, error: "region, domain, summary are required" },
        { status: 400 },
      );
    }
