...
export async function GET(request: NextRequest) {
  ...
  const videoUrl =
    searchParams.get("videoUrl")?.trim() ||
    searchParams.get("youtubeUrl")?.trim() ||
    undefined;
  ...
  try {
    ...
    bundleCache.set(cacheKey, {
      expiresAtMs: nowMs + BUNDLE_CACHE_TTL_MS,
      payload: body,
    });
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          error instanceof Error ? error.message : "viral-control build failed",
      },
