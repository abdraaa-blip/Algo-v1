...
const CONTENT_TYPES = new Set<RawContentInput["type"]>([
  "trend",
  "news",
  "video",
  "music",
...
function normalizePostedItem(
  raw: unknown,
  index: number,
): RawContentInput | null {
  if (!raw || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  const id = sanitizeInput(String(o.id || `item-${index}`)).slice(
    0,
    STR_MAX.id,
  );
  ...
  const type = String(
    o.type || "trend",
  ).toLowerCase() as RawContentInput["type"];
  ...
  return {
    ...
    description:
      o.description != null
        ? sanitizeInput(String(o.description)).slice(0, STR_MAX.description)
        : undefined,
    growthRate: num(o.growthRate),
    engagement: num(o.engagement),
    views: num(o.views),
    publishedAt:
      o.publishedAt != null ? String(o.publishedAt).slice(0, 40) : undefined,
    source:
      o.source != null
        ? sanitizeInput(String(o.source)).slice(0, STR_MAX.source)
        : undefined,
    category:
      o.category != null
        ? sanitizeInput(String(o.category)).slice(0, STR_MAX.category)
        : undefined,
    type,
  };
}
...
export async function GET(request: NextRequest) {
  ...
  const region = (
    request.nextUrl.searchParams.get("region") || "FR"
  ).toUpperCase();
  ...
  try {
    const items = await buildLiveRawItemsForCore(region, {
      maxTrends: 12,
      maxNews: 10,
    });
    ...
    return NextResponse.json(
      {
        success: false,
        error: e instanceof Error ? e.message : "Core intelligence failed",
      },
...
export async function POST(request: NextRequest) {
  const identifier = getClientIdentifier(request);
  const rateLimit = checkRateLimit(`core-post:${identifier}`, {
    limit: 30,
    windowMs: 60_000,
  });
  if (!rateLimit.success) {
  ...
  try {
    ...
    const userContext: AlgoCoreUserContext | undefined = ctx
      ? {
          locale: ctx.locale
            ? sanitizeInput(String(ctx.locale)).slice(0, 12)
            : undefined,
          regionHint: ctx.regionHint
            ? sanitizeInput(String(ctx.regionHint)).slice(0, 8).toUpperCase()
            : undefined,
          interestByCategory:
