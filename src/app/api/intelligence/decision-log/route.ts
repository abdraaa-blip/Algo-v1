...
type DurableDecisionProperties = Partial<SealedDecisionLogEntry> & {
  capturedAt?: string;
};
...
function requireOpsToken(request: NextRequest): NextResponse | null {
  ...
  if (!expectedToken) {
    return NextResponse.json(
      {
        success: false,
        error: "INTELLIGENCE_DASHBOARD_TOKEN is not configured",
      },
      { status: 503 },
    );
  }
...
function normalizeEntry(entry: DecisionLogEntry): DecisionLogEntry | null {
  if (!entry || !entry.at || !entry.region || !Array.isArray(entry.items))
    return null;
  return {
    at: new Date(entry.at).toISOString(),
    region: String(entry.region).toUpperCase(),
    viralityScore:
      typeof entry.viralityScore === "number" ? entry.viralityScore : null,
    ...
    items: entry.items
      ...
      .map((item) => ({
        ...
        approvalStatus:
          item.approvalStatus ||
          (item.requiresApproval ? "pending" : "not_required"),
...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(
    `intelligence-decision-log-post:${identifier}`,
  ...
  try {
    ...
    return NextResponse.json({
      ...
      capped: entries.length >= MAX_BATCH_SIZE,
    });
  } catch (error) {
    return NextResponse.json(
      {
...
function toCsv(entries: SealedDecisionLogEntry[]): string {
  const header =
    "at,region,viralityScore,confidence,level,title,reason,riskLevel,requiresApproval,approvalStatus,executionStatus,policyReason,prevHash,entryHash";
  const rows: string[] = [];
  for (const entry of entries) {
...
async function loadDurableEntries(
  limit: number,
): Promise<SealedDecisionLogEntry[]> {
  const supabase = getSupabaseClient();
  if (!supabase) return [];
  ...
  const parsed = data
    .map(
      (row) =>
        (row as { properties?: unknown }).properties as
          | DurableDecisionProperties
          | undefined,
    )
    .filter(Boolean)
    .map((props) => ({
...
export async function GET(request: NextRequest) {
  ...
  const identifier = getClientIdentifier(request);
  const rateLimit = checkRateLimit(
    `intelligence-decision-log-get:${identifier}`,
  ...
  const baseStore =
    memoryStore.length > 0 ? memoryStore : durableEntries.reverse();
  ...
  if (format === "csv") {
    return new NextResponse(toCsv(entries), {
      ...
      headers: {
        ...
        "Content-Disposition":
          'attachment; filename="intelligence-decision-log.csv"',
  ...
  response.headers.set(
    "X-Intelligence-Retention-Hours",
...
export async function DELETE(request: NextRequest) {
  ...
  const identifier = getClientIdentifier(request);
  const rateLimit = checkRateLimit(
    `intelligence-decision-log-delete:${identifier}`,
...
export async function PATCH(request: NextRequest) {
  ...
  const identifier = getClientIdentifier(request);
  const rateLimit = checkRateLimit(
    `intelligence-decision-log-patch:${identifier}`,
  ...
  try {
    ...
    if (!body?.at || !body?.itemId) {
      return NextResponse.json(
        { success: false, error: "at and itemId are required" },
        { status: 400 },
      );
    }
    ...
    if (!target) {
      return NextResponse.json(
        { success: false, error: "Entry not found in active memory window" },
        { status: 404 },
      );
    }
    ...
    if (body.executionStatus === "executed")
