...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const contents = ((data.results || []) as TMDBItem[]).slice(0, 10).map((item) => ({
      id: `tmdb-${item.id}`,
      ...
      badge: item.popularity > 500 ? 'Viral' : item.popularity > 200 ? 'Trend' : 'Early',
      ...
      creatorTips: 'Fais une réaction ou une analyse de ce contenu populaire.',
      insight: {
