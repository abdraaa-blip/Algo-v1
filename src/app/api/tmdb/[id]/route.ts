...
export async function GET(
  ...
  const rateLimit = checkRateLimit(`api-tmdb-id:${identifier}`, {
    limit: 90,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    const [detailsRes, videosRes, creditsRes, similarRes] = await Promise.all([
      fetch(
        `https://api.themoviedb.org/3/movie/${tmdbId}?api_key=${apiKey}&language=fr-FR&append_to_response=release_dates`,
      ),
      fetch(
        `https://api.themoviedb.org/3/movie/${tmdbId}/videos?api_key=${apiKey}&language=fr-FR`,
      ),
      fetch(
        `https://api.themoviedb.org/3/movie/${tmdbId}/credits?api_key=${apiKey}&language=fr-FR`,
      ),
      fetch(
        `https://api.themoviedb.org/3/movie/${tmdbId}/similar?api_key=${apiKey}&language=fr-FR&page=1`,
      ),
    ]);

    // If movie not found, try TV show
    if (!detailsRes.ok) {
      const [tvDetailsRes, tvVideosRes, tvCreditsRes, tvSimilarRes] =
        await Promise.all([
          fetch(
            `https://api.themoviedb.org/3/tv/${tmdbId}?api_key=${apiKey}&language=fr-FR`,
          ),
          fetch(
            `https://api.themoviedb.org/3/tv/${tmdbId}/videos?api_key=${apiKey}&language=fr-FR`,
          ),
          fetch(
            `https://api.themoviedb.org/3/tv/${tmdbId}/credits?api_key=${apiKey}&language=fr-FR`,
          ),
          fetch(
            `https://api.themoviedb.org/3/tv/${tmdbId}/similar?api_key=${apiKey}&language=fr-FR&page=1`,
          ),
        ]);
