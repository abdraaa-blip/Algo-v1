...
function insightForScore(viralScore: number): Insight {
  const postNow: PostProbability =
    viralScore >= 80 ? "high" : viralScore >= 55 ? "medium" : "low";
  return {
...
function buildContent(params: {
  ...
  return {
    ...
    sourceDistribution: [
      { platform: params.platform, percentage: 60, momentum: "high" },
    ],
