...
async function resolveUserIdForSubscription(
  ...
  const { data } = await admin.from('profiles').select('id').eq('stripe_subscription_id', sub.id).maybeSingle()
...
export async function POST(req: NextRequest) {
  ...
  if (!secret) {
    return NextResponse.json({ ok: false, error: 'STRIPE_WEBHOOK_SECRET manquant' }, { status: 503 })
  }

  const stripe = getStripeServer()
  const admin = createSupabaseAdminClient()
  if (!stripe || !admin) {
    return NextResponse.json({ ok: false, error: 'Stripe ou Supabase admin non configuré' }, { status: 503 })
  }
