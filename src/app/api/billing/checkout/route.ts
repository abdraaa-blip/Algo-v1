    ...
    return NextResponse.json(
      {
        ...
        configured: false,
        error:
          "Stripe non configuré (STRIPE_SECRET_KEY + STRIPE_PRICE_PRO_MONTHLY).",
      },
      ...
      return NextResponse.json(
        {
          ok: false,
          error: "auth_required",
          message: "Connecte-toi pour souscrire au plan Pro.",
        },
    ...
    return NextResponse.json(
      {
        ...
        message: "Session indisponible. Réessaie ou reconnecte-toi.",
      },
      { status: 503, headers: createRateLimitHeaders(rateLimit) },
    );
  }
