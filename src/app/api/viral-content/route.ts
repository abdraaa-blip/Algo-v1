 ...
 * - YouTube Trending (API Data v3 si YOUTUBE_API_KEY, sinon Invidious)
 * - Reddit Hot
 * - Hacker News Top
 * - RSS News Feeds
 */
...
import {
  ...
  fetchYouTubeTrendingWithDiagnostics,
  fetchRedditHot,
  isYoutubeApiKeyPresent,
  type ViralContent,
  type YoutubeTrendingDiagnostics,
...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`api-viral-content:${identifier}`, {
    limit: 50,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    let youtubeDiagnostics: YoutubeTrendingDiagnostics | undefined;

    // Fetch based on filters
    if (platform === "youtube") {
      const { videos, diagnostics } =
        await fetchYouTubeTrendingWithDiagnostics(country);
      youtubeDiagnostics = diagnostics;
    ...
    const meta: {
      ...
      lastUpdated: string;
      country: string;
      /** Présence de `YOUTUBE_API_KEY` côté serveur (toutes les variantes d’appel). */
      youtubeKeyPresent: boolean;
      /** Pipeline détaillé uniquement si `?platform=youtube` */
      youtube?: YoutubeTrendingDiagnostics;
    } = {
      totalItems: content.length,
      sources: [...new Set(content.map((c) => c.platform))],
      lastUpdated: new Date().toISOString(),
      country,
      youtubeKeyPresent: isYoutubeApiKeyPresent(),
    };

    if (platform === "youtube" && youtubeDiagnostics) {
      meta.youtube = youtubeDiagnostics;
    }
