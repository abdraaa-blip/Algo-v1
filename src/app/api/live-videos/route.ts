...
async function fetchYouTubeVideos(country: string): Promise<VideoData[]> {
  ...
  try {
    ...
    const videos: VideoData[] = (data.items || []).map(
      (
        item: {
          id: string;
          snippet: {
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const rateLimit = checkRateLimit(`api-live-videos:${identifier}`, {
    limit: 60,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    if (all) {
      ...
      const results = await Promise.all(
        countries.map((c) => fetchYouTubeVideos(c)),
      );
      videos = results.flat();
    } else {
