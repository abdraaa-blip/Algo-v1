...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`analytics-events-get:${identifier}`, {
    limit: 300,
    windowMs: 60_000,
  });
  ...
  const frictionEvents =
    (stats.eventTypes.error || 0) + (stats.eventTypes.api_error || 0);
  const adaptiveSignals = {
    engagementRate:
      stats.totalEvents > 0
        ? Number((engagementEvents / stats.totalEvents).toFixed(4))
        : 0,
    frictionRate:
      stats.totalEvents > 0
        ? Number((frictionEvents / stats.totalEvents).toFixed(4))
        : 0,
  };
