...
const chartsMeta = {
  ...
  provider: "Last.fm",
  note: "Sans LASTFM_API_KEY : données démo locales (voir offline-media-demos). Charts Spotify optionnels : GET /api/spotify (SPOTIFY_CLIENT_ID + SPOTIFY_CLIENT_SECRET).",
} as const;

export async function GET(request: NextRequest) {
  const identifier = getClientIdentifier(request);
  const rateLimit = checkRateLimit(`api-live-music:${identifier}`, {
    limit: 60,
    windowMs: 60_000,
  });
