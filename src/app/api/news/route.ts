...
function scoresFromArticle(i: number, title: string, publishedAt: string): {
  viralScore: number
    ...
    try {
      ...
      if (res.ok) {
        ...
        const news = ((data.articles || []) as NewsAPIArticle[]).map((a, i: number) => {
          ...
          const { viralScore, importanceScore, speedScore } = scoresFromArticle(i, title, publishedAt)
          return {
          id: `n${i + 1}`,
          title,
        ...
        return NextResponse.json({
          ...
          meta: {
            ...
            scoresNote: 'Scores déterministes (hash titre + date), pas des métriques NewsAPI.',
          },
        })
      }
    } catch {
