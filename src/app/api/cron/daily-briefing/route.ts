...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const topContent = [
      ...
      ...(newsData.data || []).map((n: { title: string; viralScore?: number; source?: string }) => ({
        title: n.title,
        category: 'Actualites',
    ...
    ].sort((a, b) => b.viralScore - a.viralScore).slice(0, 30)
      ...
      await Promise.all(
        batch.map(async (profile) => {
          try {
            ...
            console.error(`[ALGO Cron] Failed to generate briefing for user ${profile.id}:`, error)
          }
        })
      )
    ...
    console.log(`[ALGO Cron] Daily briefings completed in ${executionTime}ms. Generated: ${generated}`)
