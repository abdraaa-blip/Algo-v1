...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const allContent = [
      ...
      ...(newsData.data || []).map(
        (n: { title: string; viralScore?: number }) => ({
          title: n.title,
          category: "news",
    ...
    const detectedTrends: TrendEntity[] = clusters.map((cluster, i) => ({
      ...
      status:
        cluster.evolutionStage === "emerging"
          ? "emerging"
          : cluster.evolutionStage === "growing"
            ? "growing"
            : cluster.evolutionStage === "mainstream"
              ? "mainstream"
              : "declining",
      contentCount: allContent.filter((c) =>
        c.title
          .toLowerCase()
          .includes(cluster.name.toLowerCase().split(" ")[0]),
      ).length,
