...
export async function POST(req: NextRequest) {
  ...
  if (!rateLimit.success) {
    ...
    addLog({
      ...
      metadata: { durationMs: Date.now() - startedAt, httpStatus: 429, route: 'POST /api/ai/ask' },
    })
  ...
  try {
    ...
    addLog({
      ...
      metadata: { durationMs: Date.now() - startedAt, route: 'POST /api/ai/ask' },
    })
    return NextResponse.json(
      {
        success: true,
  ...
  const context = body.context as { currentTrends?: string[]; userCountry?: string } | undefined
  ...
  const serverEnrich = typeof body.serverEnrich === 'boolean' ? body.serverEnrich : undefined
  ...
  try {
    const { answer, structured, meta, standard, quality, route, systemConfidence } =
    ...
    const trends = context?.currentTrends?.filter((t) => typeof t === 'string' && t.trim()) ?? []
    ...
    addLog({
      ...
      metadata: { durationMs: Date.now() - startedAt, route: 'POST /api/ai/ask', systemRoute: route },
    })
    return NextResponse.json(
      {
        success: true,
