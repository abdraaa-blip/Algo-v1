...
import { 
  Music, Mic2, Play, ExternalLink, RefreshCw, Users, 
  Headphones, Radio
} from 'lucide-react'
      ...
      {isLoading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="h-20 bg-[var(--color-card)] rounded-xl animate-pulse" />
          ))}
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {((activeTab === 'tracks' && tracks.length === 0) || 
            ...
            <div className="text-center py-12">
              <Radio size={48} className="mx-auto mb-4 text-[var(--color-text-muted)]" />
              <p className="text-[var(--color-text-secondary)] mb-2">Aucune donnee musicale disponible</p>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-[var(--color-text-secondary)] truncate">{track.artist}</p>
      </div>
...
function ArtistCard({ artist }: { artist: Artist }) {
  return (
    ...
    >
      ...
      <ExternalLink size={14} className="text-[var(--color-text-muted)] group-hover:text-[var(--color-text-tertiary)]" />
    </a>
  )
}
