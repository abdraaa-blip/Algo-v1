...
Exemples intelligence (toujours indicatifs, même famille que le radar UI) :

| Route | Limite | Rôle |
|-------|--------|------|
| `GET /api/intelligence/radar-history` | IP | Historique score / confiance / anomalies. |
| `GET /api/intelligence/viral-control` | IP | Bundle cockpit + option `videoUrl` YouTube (voir `docs/integrations/YOUTUBE_VIRAL_CONTROL.md`). |
| `GET /api/social/youtube/video-metrics` | IP | Métriques publiques + série d’observations serveur · requiert `YOUTUBE_API_KEY`. |
